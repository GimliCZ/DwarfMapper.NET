...
| 36 | [`guides/36_Views.cs`](guides/36_Views.cs) — Zero-copy views | reading a DTO shape straight off the source, with no allocation and no copy |
<!-- endtable -->
...
*guides/36_Views.cs* — reading a DTO shape straight off the source, with no allocation and no copy

`[GenerateView<TSource, TTarget>]` emits a nested `readonly ref struct` on the mapper whose properties
evaluate the same member resolution `Map` would — lazily, on access, against the source you hand it. Nothing
is allocated and nothing is copied. **The `ref struct` is the contract, not a limitation:** the compiler will
not let a view be stored in a field, boxed, captured by a lambda or held across an `await`, so it cannot
outlive the source it borrows — which is what makes reading straight through to a live object safe. Consume a
view at once — serialize it, render it, compare it — and use `Map` wherever the result is stored, returned or
kept. It is a window, not a snapshot: mutate the source afterwards and the view shows the new value.

Measured against map-then-consume (`Issues/round29/plan3-results.md`): **0.20× at 1k, 0.04× at 100k, zero
bytes allocated**. The zero is the view's own — a converter it calls may still allocate on its own account (an
