...
// </snippet>
    ...
    [DocExample(27,
        Tier.Advanced,
        "Patch-merge: a null source member leaves the destination alone",
    ...
    public static class Example
    {
        public static void Run()
        {
            ...
            Console.WriteLine(
                $"27 Patch-merge        -> patch keeps \"{patched.Inscription}\" ({patched.Weight}kg); " + $"replace clears it to {Show(replaced.Inscription)} ({replaced.Weight}kg)");
        }

        private static Anvil Stored()
        {
            return new Anvil
            {
                Owner = "Narvi",
