...
{
    Console.WriteLine("  INSTABILITY: " + msg);
    failures++;
}
...
foreach (var n in new[] { 0, 1, 2, 3, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 1000, 1001 })
{
    ...
    for (var i = 0; i < n; i++)
        if (dst[i] != src[i])
        {
            Fail($"widen n={n} i={i}: {dst[i].ToString(ci)} != {((long)src[i]).ToString(ci)}");
            break;
        }
}
...
for (var r = 0; r < 20_000; r++)
{
    ...
    try
    {
        new DepthMapper().Map(head);
        Fail($"depth guard did not throw at run {r}");
        break;
    }
...
for (var r = 0; r < 50_000; r++)
{
    ...
    {
        Fail($"setnull back-edge not nulled at run {r}");
        break;
    }
}
...
if (!DwarfMapperRegistry.IsProvided(typeof(FlatSrc), typeof(FlatDst)))
{
...
void Report(string name, int iters, Action body)
{
    ...
    Console.WriteLine(
        $"  {name,-14} {med.ToString("F1", ci),9} [{min.ToString("F1", ci)}..{max.ToString("F1", ci)}]{note}");
}
