...
// ── F35 [MapConstructor<S,T>] pair-scoped factory ────────────────────────────
    ...
    {
        private static F35D Create(F35S s)
        {
            return new F35D(s.Text, s.Number);
        }
    }
...
// the target directly and never calls the factory. Green build, and `Map(item)` and `Map(list)[0]` disagreed.
    ...
    public partial class F42M
    {
        ...
        private static F42ItemDto Create(F42Item i)
        {
            return new F42ItemDto(i.V + 100);
        }
    }
...
// it emits nothing, and exists so DWARF085 can compare the two pairs' RESOLVED mappings.
    ...
    [MapProperty<F46AliasCommand, F46AliasCommandDto>(nameof(F46Command.Raw),
        nameof(F46CommandDto.Text),
        Use = nameof(Clean))]
