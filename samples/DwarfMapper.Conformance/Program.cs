...
// F01 flat
R.Check("F01 flat by-name",
    new F01M().Map(new F01S
    {
        Id = 1,
        Name = "a"
    }) is { Id: 1, Name: "a" });
...
// F04 enum by value
R.Check("F04 enum by-value",
    new F04M().Map(new F04S
    {
        E = Src4.B
    }).E ==
    Dst4.Y);
...
// F05 nested
R.Check("F05 nested object",
    new F05M().Map(new F05S
    {
        Inner = new Inner5
        {
            V = 8
...
R.Check("F07 null-collection AsNull",
    new F07M().Map(new F07S
    {
        Nums = null
    }).Nums is null);
...
R.Check("F10 Use= converter",
    new F10M().Map(new F10S
    {
        Total = 9.5m
    }).Total ==
    9.5m.ToString("C", CultureInfo.GetCultureInfo("en-US")));

// F11 null substitute
R.Check("F11 NullSubstitute",
    new F11M().Map(new F11S
    {
        Nickname = null
    }).Nickname ==
    "(none)");
...
R.Check("F12 When= (false)",
    new F12M().Map(new F12S
    {
        Score = 50,
        Active = false
    }).Score ==
    0);
R.Check("F12 When= (true)",
    new F12M().Map(new F12S
    {
        Score = 50,
        Active = true
...
// F14 record ctor
R.Check("F14 record target",
    new F14M().Map(new F14S
    {
        Id = 3,
        Name = "r"
    }) ==
...
R.Check("F15 AfterMap hook",
    new F15M().Map(new F15S
    {
        Id = 1
    }).Touched ==
    99);
...
// F16 reverse map
R.Check("F16 ReverseMap fwd",
    new F16M().Forward(new F16A
    {
        FullName = "Ada"
    }).Name ==
    "Ada");
R.Check("F16 ReverseMap back",
    new F16M().Backward(new F16B
    {
        Name = "Bob"
    }).FullName ==
    "Bob");
...
// F17 ignore
R.Check("F17 MapIgnore",
    new F17M().Map(new F17S
    {
        Id = 1
    }).Secret is null);

// F18 [MapTo] registry extension
R.Check("F18 [MapTo] extension",
    new F18S
    {
        Id = 9,
        Name = "z"
    }.MapTo<F18D>() is { Id: 9, Name: "z" });

// F19 polymorphism ([MapDerivedType])
Animal19 a19 = new Dog19
{
    Id = 1,
    Breed = "Lab"
};
...
R.Check("F22 NameConvention flexible",
    new F22M().Map(new F22S
    {
        user_name = "ada"
    }).UserName ==
    "ada");
...
R.Check("F23 [DwarfMapperConstructor]",
    new F23M().Map(new F23S
    {
        Id = 5,
        Name = "x"
    }) is { Id: 5, Name: "x" });
...
R.Check("F24 [GenerateMap<S,D>]",
    new F24M().Map(new F24S
    {
        Id = 7
    }).Id ==
    7);
...
R.Check("F25 source-required+ignore",
    new F25M().Map(new F25S
    {
        Id = 3,
        Internal = "x"
    }).Id ==
...
R.Check("F26 FlattenGraph",
    td.Label == "T" && td.Nodes.Count >= 3 && td.Nodes.OfType<FileNDto>().Any(n => n.Size == 2),
    $"nodes={td.Nodes.Count}");

// F27 Reinterpret array blit (Px[]->Qx[] by layout: A->X, B->Y)
var rd = new F27M().Map(new ReSrc
{
    V = new[]
    {
        new Px
...
R.Check("F28 OnCycle=SetNull",
    cnb.Id == 1 && (cnb.Next is null || cnb.Next.Next is null),
...
R.Check("F29 ambient resolve+map",
    ((AmbDst)DwarfMapperRegistry.Map(new AmbSrc
        {
            V = 41
        },
        typeof(AmbDst))).V ==
...
R.Check("D2 narrowing overflow",
    R.Throws<OverflowException>(() => new DovM().Map(new OvS
    {
        V = long.MaxValue
    })));

// D3 unparseable string->int -> FormatException
R.Check("D3 bad parse throws",
    R.Throws<FormatException>(() => new DpsM().Map(new PsS
    {
        V = "abc"
    })));
...
R.Check("D5 nested null -> null",
    new DnnM().Map(new NnS
    {
        Inner = null
    }).Inner is null);

// D6 default null-collection -> empty (AsEmpty)
R.Check("D6 null coll -> empty",
    new DecM().Map(new EcS
    {
        Xs = null
    }).Xs is { Count: 0 });

// D7 enum by-value undefined numeric -> raw value passes through
R.Check("D7 enum raw value",
    (int)new DreM().Map(new ReS
    {
        Code = 999
    }).Code ==
    999);
...
R.Check("D8 unicode passthrough",
    new DuniM().Map(new UniS
    {
        Text = "héllo 🐉 世界"
    }).Text ==
    "héllo 🐉 世界");
R.Check("D8 empty string",
    new DuniM().Map(new UniS
    {
        Text = ""
    }).Text.Length ==
...
var f31Patched = new F31D
{
    Name = "kept",
    Note = "kept"
};
var f31Replaced = new F31D
{
    Name = "kept",
    Note = "kept"
};
new F31M().Patch(new F31S
    {
        Name = "new"
    },
    f31Patched);
new F31M().Replace(new F31S
    {
        Name = "new"
    },
    f31Replaced);
...
R.Check("F32 AllowNonPublic ctor",
    new F32M().Map(new F32S
    {
        Id = 11
    }).Id ==
    11);
...
R.Check("F34 obsolete member skipped",
    new F34M().Map(new F34S
    {
        Id = 4
    }) is { Id: 4, LegacyId: 0 });
...
R.Check("F36 assembly defaults read",
    new F36M().Map(new F36S
    {
        itemcount = 6
    }).ItemCount ==
    6);
...
R.Check("F38 [MapCollectionKey] merge",
    f38Dest.Lines.Count == 3 && f38Dest.Lines.Single(l => l.Id == 1).Text == "new" && f38Dest.Lines.Single(l => l.Id == 2).Text == "keep" && f38Dest.Lines.Single(l => l.Id == 3).Text == "added");

// F39 [AutoNest(false)] — the nested pair is declared by hand, and nothing is invented
R.Check("F39 [AutoNest(false)]",
    new F39M().Map(new F39S
    {
        Inner = new F39Inner
        {
            V = 8
...
var f40 = new F40M().Map(new F40Envelope<F40Payload>
{
    Value = new F40Payload
    {
        Id = 2
    },
    Trace = "t"
...
var f42M = new F42M();
var f42Direct = f42M.Map(new F42Item
{
    V = 1
});
var f42Element = f42M.Map([
    new F42Item
    {
        V = 2
    }
]);
var f42Nested = f42M.Map(new F42Box
{
    Only = new F42Item
    {
        V = 3
...
var f44 = new F44M().ToDto(new F44AliasCommand
{
    Name = "aka",
    Alias = "a"
});
...
var f45Annotated = new F45AnnotatedM().Map(new F45Dispatch
{
    Source = F45DispatchChannel.NextDay
});
var f45Identity = new F45IdentityM().Map(new F45Dispatch
{
    Source = F45DispatchChannel.NextDay
});
...
R.Check("F45 EnumStringSource=Identifier reads it back",
    new F45IdentityM().Map(new F45DispatchDoc
    {
        Source = "NextDay"
    }).Source ==
    F45DispatchChannel.NextDay);
...
var f46 = new F46M();
R.Check("F46 [RestatesBase] base pair converts",
    f46.Map(new F46Command
    {
        Raw = "  hi  "
    }).Text ==
    "hi");
R.Check("F46 [RestatesBase] restated pair converts too",
    f46.Map(new F46AliasCommand
    {
        Raw = "  hi  ",
        Alias = "a"
    }) is { Text: "hi", Alias: "a" });
...
R.Check("F48 element map still registered",
    ((F48D)DwarfMapperRegistry.Map(new F48S
        {
            Id = 5
        },
        typeof(F48D))).Id ==
