...
if (count != 5)
{
    Console.WriteLine("ERROR: chain5 count wrong");
    return 1;
}
...
if (tA.V != 10)
{
    Console.WriteLine("ERROR: preserve cycle tA.V wrong");
    return 1;
}

if (tA.Next.V != 20)
{
    Console.WriteLine("ERROR: preserve cycle tA.Next.V wrong");
    return 1;
}
...
Console.WriteLine(
    $"setnull cycle: A.V={snResult.V}, A.Next.V={snResult.Next!.V}, A.Next.Next={(snResult.Next.Next is null ? "null" : "NOT-null")}");
if (snResult.V != 10)
{
    Console.WriteLine("ERROR: setnull A.V wrong");
    return 1;
}

if (snResult.Next.V != 20)
{
    Console.WriteLine("ERROR: setnull A.Next.V wrong");
    return 1;
}
...
{
    Console.WriteLine("ERROR: flatten-graph node count wrong");
    return 1;
}

if (fgResult.Label != "mine")
{
    Console.WriteLine("ERROR: flatten-graph root label wrong");
    return 1;
}

// Edges must be null (topology degraded)
if (fgResult.Nodes.Any(n => n.Next is not null))
{
    Console.WriteLine("ERROR: flatten-graph edge not nulled");
    return 1;
}
...
{
    Console.WriteLine("ERROR: poly Dog name wrong");
    return 1;
}
...
if (heteroResult.Nodes.Count != 4)
{
    Console.WriteLine($"ERROR: hetero-flatten node count wrong (got {heteroResult.Nodes.Count})");
    return 1;
}

if (heteroResult.Tag != "plan22")
{
    Console.WriteLine("ERROR: hetero-flatten root tag wrong");
    return 1;
}
...
if (folderDtos.Count != 2)
{
    Console.WriteLine($"ERROR: hetero-flatten expected 2 FolderDto, got {folderDtos.Count}");
    return 1;
}

if (fileDtos.Count != 2)
{
    Console.WriteLine($"ERROR: hetero-flatten expected 2 FileDto, got {fileDtos.Count}");
    return 1;
}

// Verify edges are degraded (null) — topology intentionally lost
if (folderDtos.Any(f => f.Children is not null))
{
    Console.WriteLine("ERROR: hetero-flatten FolderDto.Children not null (topology not degraded)");
    return 1;
}
...
if (file1Dto is null)
{
    Console.WriteLine("ERROR: hetero-flatten file1 not found by name");
    return 1;
}

if (file1Dto.Size != 42L)
{
    Console.WriteLine($"ERROR: hetero-flatten file1 Size wrong (got {file1Dto.Size})");
    return 1;
}
...
{
    Console.WriteLine($"ERROR: hetero-flatten cycle node count wrong (got {cycleResult.Nodes.Count})");
    return 1;
}
...
Console.WriteLine(
    $"update-into: same instance, Id={updated.Id} Label={updated.Label} Score={updated.Score} (AOT-safe)");
...
try
{
    Span<long> tiny = stackalloc long[1];
    spanMapper.Map(spanSrc, tiny);
}
catch (ArgumentException)
{
    spanThrew = true;
...
if (!spanThrew)
{
    Console.WriteLine("ERROR: span map should throw on too-small destination");
    return 1;
}
...
for (var i = 0; widenOk && i < widenSrc.V.Length; i++) widenOk = widenDst.V[i] == widenSrc.V[i];
if (!widenOk)
{
    Console.WriteLine("ERROR: SIMD widen result != scalar widen");
    return 1;
}
...
static async IAsyncEnumerable<AotAsyncSrc> AotAsyncSource()
{
    for (var i = 1; i <= 3; i++)
    {
        await Task.Yield();
...
var asyncCollected = new List<AotAsyncDst>();
