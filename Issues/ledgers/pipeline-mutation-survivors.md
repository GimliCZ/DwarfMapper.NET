...
- **Line 217, `tgtName.IndexOf('.') >= 0` → `> 0`, and line 284, `srcName.IndexOf('.') >= 0` → `> 0`.**
  *Resolved 2026-09-26 by a SOURCE FIX (`3f51061`), not by a test.* This entry said the mutant's refusal was *the more legible one* and declined to pin the original. That was the right call pointing at the wrong remedy: when the mutant is better than the original, the fix is to adopt it. Both sites now read `> 0`, so a name that merely STARTS with a dot is not a path — `[MapProperty(".Name", "Full")]` reports DWARF009 naming `'.Name'` instead of DWARF043 blaming an empty member, and the destination half reports DWARF008 naming `'.Full'` instead of DWARF045 about an empty unflatten intermediate *plus* a spurious DWARF001. Two regression tests in `DeepSourcePathGeneratorTests` pin both, watched RED against the unfixed source; the sibling test pinning a REAL path keeping DWARF043 states the other side of the boundary. The 2026-09-26 leg confirms both mutants Killed, and the leg is now AT its ceiling — 15 survivors against 15 proven rows. The THIRD copy of this comparison (`ApplySkipNullSourceMembers`, line 59) stays proven-equivalent and its proof is now stronger: it rests on no SourceName beginning with '.', and such a name is refused earlier than it was.
