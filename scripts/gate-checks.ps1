...
#   Windows  DwarfMapper.1.0.2-rc.1.nupkg  325,131 B -> floor(325131/1024) = 317 KB   (was 308)
#            DwarfMapper.Testing...nupkg    51,044 B -> floor( 51044/1024) =  49 KB   (UNCHANGED)
...
# bound, and applying the same ~520 B offset puts ubuntu at ~324,610 B, which truncates to the SAME 317 KB.
...
# 316,128 B against the 316,155 B recorded above — a 27-byte drift between two Windows packs of the same
# commit. The headroom below is 501 B, which is larger than that drift but not by much.
#
...
#   analyzers/.../DwarfMapper.Generator.dll   639,488 / 211,436 -> 665,600 / 219,718   +8,282
#   lib/net10.0/DwarfMapper.xml               168,571 /  38,638 -> 169,960 /  38,977     +339
#   lib/net10.0/DwarfMapper.dll                42,496 /  18,066 ->  43,520 /  18,399     +333
#   DwarfMapper.nuspec                          1,370 /     746 ->   1,417 /     776      +30
#   README.md                                  80,105 /  27,595 ->  80,108 /  27,605      +10
#   analyzers/.../DwarfMapper.CodeFixes.dll    36,864 /  17,227 ->  36,864 /  17,235       +8
#   _rels/.rels                                   505 /     284 ->     505 /     285       +1
#   (total deflated +9,003, and 316,128 + 9,003 = 325,131 exactly; the .psmdcp part is NuGet's random
...
# package.
#
# Headroom to the first red byte (318 KB = 325,632 B): 501 B on Windows. That is TIGHT, and deliberately
# so — R1 forbids a cushion. It is not theoretical: within this same task a ~60-line generator fix moved
# the package from 324,259 to 325,131 B and turned a freshly measured 316 KB ceiling red. Expect the next
# generator change of any size to move this number again; the gate is a tripwire on what ships, not a
# budget.
# For DwarfMapper.Testing (50 KB = 51,200 B): 156 B, and its 49 KB is re-measured, not merely carried.
# ─────────────────────────────────────────────────────────────────────────────────────────────────────
