    ...
    internal static class AcceptedDivergences
    {
        ...
        [
            ...
            new("EnumToString",
                Oracles.AutoMapper,
                "<root>.Status",
                "Same axis. AutoMapper 14 uses .ToString(), i.e. the identifier — which is precisely why a " + "migration off it walks into this: see docs/howto/migrate-from-automapper.md row 3.")
        ];

        public static bool IsAccepted(string shape, string oracle, string path)
        {
            return All.Any(a => string.Equals(a.Shape, shape, StringComparison.Ordinal) && string.Equals(a.Oracle, oracle, StringComparison.Ordinal) && path.StartsWith(a.PathPrefix, StringComparison.Ordinal));
        }
    }
