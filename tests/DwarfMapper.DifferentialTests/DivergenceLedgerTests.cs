    ...
    public class DivergenceLedgerTests
    {
        ...
        public void Every_accepted_divergence_states_a_reason()
        {
            ...
            Assert.True(unexplained.Count == 0,
                "Accepted divergence(s) with no real explanation:\n  " + string.Join("\n  ", unexplained) + "\n\nName the AXIS on which the two mappers differ and why DwarfMapper's side is right (or " + "deliberately different). A one-word reason is how an allowlist becomes a way of making red " + "things green.");
        }
        ...
        public void Every_accepted_divergence_names_a_real_oracle()
        {
            var known = new[]
            {
                Oracles.Mapperly, Oracles.AutoMapper
            };
            ...
            Assert.True(unknown.Count == 0,
                "Accepted divergence(s) naming an oracle this harness does not run: " + string.Join(", ", unknown));
        }
        ...
        public void Every_accepted_divergence_is_actually_exercised()
        {
            ...
            foreach (var difference in MemberComparer.Differences(comparison.Dwarf, comparison.OracleValue))
            {
                ...
                foreach (var accepted in AcceptedDivergences.All)
                    if (string.Equals(accepted.Shape, comparison.Shape, StringComparison.Ordinal) && string.Equals(accepted.Oracle, comparison.Oracle, StringComparison.Ordinal) && path.StartsWith(accepted.PathPrefix, StringComparison.Ordinal))
                    {
            ...
            Assert.True(unused.Count == 0,
                "Accepted divergence(s) that no comparison actually hit:\n  " + string.Join("\n  ", unused) + "\n\nEither the difference is gone — delete the entry, that is the ratchet tightening — or no " + "shape exercises it, in which case the entry is documenting something untested.");
        }
        ...
        public void Every_declared_pair_is_actually_compared()
        {
            ...
            Assert.True(uncompared.Count == 0,
                "Pair(s) declared on the oracle but never compared:\n  " + string.Join("\n  ", uncompared) + "\n\nAdd them to ShapeCatalog.All(). A shape that exists but is not in the catalogue is a shape " + "nobody is checking, wearing the appearance of coverage.");
        }

        private static string Key(AcceptedDivergence a)
        {
            return $"{a.Shape}/{a.Oracle} at {a.PathPrefix}";
        }
    }
}
