...
// fixture type names carry the IEk (interface-edge-kind) scenario prefix
...
// Interface/base-typed edge traversal (homo)
    ...
    public class FlattenGraphEdgeKindMatrixTests
    {
        ...
        {
            var child = new IEkConcrete
            {
                Name = "Child",
                Extra = 7
            };
            var parent = new IEkConcrete
            {
                Name = "Parent",
                Extra = 99
            };
            ...
            child.Parent = parent;
            var result = _im.Map(new IEkRoot
            {
                Entry = child
            });
            Assert.Equal(2, result.Nodes.Count);
...
// ── Golden fixtures ────────────────────────────────────────────────────────────
    ...
    public class FlattenGraphGoldenFixtureTests
    {
        ...
        public void WideGraph_root_20children_5grandchildren_each_121_total()
        {
            ...
            var result = _m.MapColl(new FGCollRoot
            {
                Entry = rootNode
            });
