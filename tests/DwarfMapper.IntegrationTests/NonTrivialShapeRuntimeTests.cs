...
// ── readonly struct (ctor-only) ───────────────────────────────────────────────
    public readonly struct NtPoint
    {
        public NtPoint(int x, int y)
        {
            X = x;
            Y = y;
        }
...
// ── deep nullability ──────────────────────────────────────────────────────────
    public class NtDeepSrc
    {
        public List<int?> Keep { get; set; } = new();
    }

    public class NtDeepDst
    {
        public List<int?> Keep { get; set; } = new();
    } // null-preserving element
    ...
    public class NonTrivialShapeRuntimeTests
    {
        ...
        {
            var d = new NonTrivialMapper().Map(new NtSrc
            {
                A = 1,
                B = "b",
                C = 3
        ...
        {
            var dest = new NtInitDst
            {
                A = 10,
                B = "orig",
                C = 0
            };
            new NonTrivialMapper().Update(new NtSrc
                {
                    A = 99,
                    B = "new",
                    C = 42
        ...
        {
            var p = new NonTrivialMapper().Map(new NtPointSrc
            {
                X = 4,
                Y = 5
            });
        ...
        {
            var r = new NonTrivialMapper().Map(new NtRecSrc
            {
                X = 7,
                Y = "z"
            });
        ...
        {
            var d = new NonTrivialMapper().Map(new NtDeepSrc
            {
                Keep = new List<int?>
                {
                    1,
