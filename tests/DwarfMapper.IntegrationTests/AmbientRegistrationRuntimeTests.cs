...
// manifest entry. These tests prove that end-to-end (Phase 2 / 3 of the ambient registry).
    ...
    public partial class AmbientSampleMapper
    {
        ...
        public partial IReadOnlyList<AmbientDst> Map(List<AmbientSrc> source);

        // An async-stream map — IAsyncEnumerable boxes to object, so it is ambient-registerable as well.
        public partial IAsyncEnumerable<AmbientDst> Map(IAsyncEnumerable<AmbientSrc> source);
    }
    ...
    public sealed class AmbientRegistrationRuntimeTests
    {
        ...
        {
            var dst = DwarfMapperFacade.Instance.Map<AmbientDst>(new AmbientSrc
            {
                V = 5
            });
            ...
            var direct = new AmbientSampleMapper().Map(new AmbientSrc
            {
                V = 9
            });
        ...
        public void Collection_conversion_map_resolves_through_the_ambient_facade()
        {
            ...
            Assert.True(DwarfMapperRegistry.IsProvided(typeof(List<AmbientSrc>),
                typeof(IReadOnlyList<AmbientDst>)));

            var result = DwarfMapperFacade.Instance.Map<IReadOnlyList<AmbientDst>>(
                new List<AmbientSrc>
                {
                    new()
                    {
        ...
        public void Async_stream_map_is_ambient_registered()
        {
            Assert.True(DwarfMapperRegistry.IsProvided(
                typeof(IAsyncEnumerable<AmbientSrc>),
                typeof(IAsyncEnumerable<AmbientDst>)));
        }
