...
public sealed class RegistryPropertyTests
{
    ...
    // each closed type.
    private sealed class PSrc<T> { public T? Tag { get; set; } }

    private sealed class PDst<T> { public int Marker { get; set; } public T? Tag { get; set; } }
    ...
    [
        (new PoolEntry(typeof(PSrc<R1>), typeof(PDst<R1>), () => new PSrc<R1>(), 1), _ => new PDst<R1> { Marker = 1 }),
        (new PoolEntry(typeof(PSrc<R2>), typeof(PDst<R2>), () => new PSrc<R2>(), 2), _ => new PDst<R2> { Marker = 2 }),
        (new PoolEntry(typeof(PSrc<R3>), typeof(PDst<R3>), () => new PSrc<R3>(), 3), _ => new PDst<R3> { Marker = 3 }),
        (new PoolEntry(typeof(PSrc<R4>), typeof(PDst<R4>), () => new PSrc<R4>(), 4), _ => new PDst<R4> { Marker = 4 })
    ];
    ...
    static RegistryPropertyTests()
    {
        ...
        foreach (var entry in DuplicatePool)
        {
            ...
            DwarfMapperRegistry.Register(entry.Source, entry.Destination,
    ...
    {
        if (destination == typeof(PDst<D1>)) return new PDst<D1> { Marker = marker };
        if (destination == typeof(PDst<D2>)) return new PDst<D2> { Marker = marker };
        if (destination == typeof(PDst<D3>)) return new PDst<D3> { Marker = marker };
        if (destination == typeof(PDst<D4>)) return new PDst<D4> { Marker = marker };
        if (destination == typeof(PDst<K1>)) return new PDst<K1> { Marker = marker };
        if (destination == typeof(PDst<K2>)) return new PDst<K2> { Marker = marker };
        if (destination == typeof(PDst<K3>)) return new PDst<K3> { Marker = marker };
        if (destination == typeof(PDst<K4>)) return new PDst<K4> { Marker = marker };
    ...
    {
        Gen.Select(Gen.Int[0, IdentityPool.Length - 1], Gen.Int[0, IdentityPool.Length - 1]).Sample(t =>
        {
            var (i, j) = t;
