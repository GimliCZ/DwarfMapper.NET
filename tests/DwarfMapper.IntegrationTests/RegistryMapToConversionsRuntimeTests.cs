...
namespace RegistryProto.Conv
{
    ...
    public class RegistryMapToConversionsRuntimeTests
    {
        ...
        public void Maps_scalars_nested_and_collections()
        {
            ...
            Assert.Equal((string?)"3.5", (string?)dto.Score); // format (invariant)
            ...
            Assert.Equal(new List<long>
                ...
                dto.Marks); // collection element widen
            Assert.Equal(2, dto.Contacts.Count); // collection of nested
            Assert.Equal((string?)"Brno", (string?)dto.Contacts[0].City);
            Assert.Equal((string?)"Ostrava", (string?)dto.Contacts[1].City);
        }
