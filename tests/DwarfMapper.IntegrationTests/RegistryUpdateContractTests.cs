...
namespace DwarfMapper.IntegrationTests
{
    ...
    public sealed class RegistryUpdateContractTests
    {
        ...
        {
            var forSource = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Update(null!, new UNullDst(), typeof(UNullSrc), typeof(UNullDst)));
            Assert.Equal("source", forSource.ParamName);

            var forDestination = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Update(new UNullSrc(), null!, typeof(UNullSrc), typeof(UNullDst)));
            Assert.Equal("destination", forDestination.ParamName);

            var forSourceType = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Update(new UNullSrc(), new UNullDst(), null!, typeof(UNullDst)));
            Assert.Equal("sourceType", forSourceType.ParamName);

            var forDestinationType = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Update(new UNullSrc(), new UNullDst(), typeof(UNullSrc), null!));
        ...
        public void A_duplicate_update_registration_is_first_wins_and_marked_ambiguous()
        {
            ...
            Assert.True(DwarfMapperRegistry.IsUpdateAmbiguous(typeof(USrc), typeof(UDst)),
                "A duplicate update registration must be MARKED, exactly as the create table marks its " + "duplicates. Silently dropping the loser is the asymmetry the create table already solved.");
        }
        ...
        /// <summary>
        ///     A single registration is not ambiguous — without this the marking assertions could pass on a set that is
        ///     always non-empty.
        /// </summary>
        [Fact]
        ...
        public void The_update_table_does_not_alias_the_map_table()
        {
            ...
            Assert.False(DwarfMapperRegistry.IsUpdateProvided(typeof(UAliasSrc), typeof(UAliasDst)),
                "A Register on the create table must not surface as an update registration — a caller would get " + "a create where they asked for an in-place update.");
            Assert.False(DwarfMapperRegistry.IsUpdateAmbiguous(typeof(UAliasSrc), typeof(UAliasDst)),
                "The create table's duplicate must not surface as an update-table duplicate either. The " + "separation has to hold in BOTH directions, or one accessor is still reading the other's set.");
        }

        private sealed class USrc
        {
            public int Id { get; set; }
        }

        private sealed class UDst
        {
            public int Id { get; set; }
        }

        private sealed class UMarkSrc
        {
            public int Id { get; set; }
        }

        private sealed class UMarkDst
        {
            public int Id { get; set; }
        }

        private sealed class UAliasSrc
        {
            public int Id { get; set; }
        }

        private sealed class UAliasDst
        {
            public int Id { get; set; }
        }

        private sealed class USoloSrc
        {
            public int Id { get; set; }
        }

        private sealed class USoloDst
        {
            public int Id { get; set; }
        }

        private sealed class UNullSrc
        {
            public int Id { get; set; }
        }

        private sealed class UNullDst
        {
            public int Id { get; set; }
        }
    }
