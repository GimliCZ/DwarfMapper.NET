...
public sealed class IfSrcB { public int X { get; set; } }
public sealed class IfDstB { public int X { get; set; } }

public sealed class IfSrcC { public int X { get; set; } }
public sealed class IfDstC { public int X { get; set; } }

public sealed class IfSrcD { public int X { get; set; } }
public sealed class IfDstD { public int X { get; set; } }

public sealed class IfSrcE { public int X { get; set; } }
public sealed class IfDstE { public int X { get; set; } }

public sealed class IfSrcF { public int X { get; set; } }
public sealed class IfDstF { public int X { get; set; } }
...
public sealed class RegistryInterfaceLookupTests
{
    ...
    public void A_map_registered_on_an_interface_serves_a_concrete_implementation()
    {
        ...
        DwarfMapperRegistry.Register(typeof(IEnumerable<IfSrcA>), typeof(List<IfDstA>),
            o => ((IEnumerable<IfSrcA>)o).Select(s => new IfDstA { X = s.X }).ToList());

        var list = new List<IfSrcA> { new() { X = 1 }, new() { X = 2 } };
    ...
    {
        DwarfMapperRegistry.Register(typeof(IEnumerable<IfSrcB>), typeof(List<IfDstB>),
            o => ((IEnumerable<IfSrcB>)o).Select(s => new IfDstB { X = s.X }).ToList());

        var mapped = (List<IfDstB>)DwarfMapperRegistry.Map(
            new[] { new IfSrcB { X = 7 } }, typeof(List<IfDstB>));
    ...
    public void A_lazy_LINQ_iterator_is_served_too()
    {
        ...
        DwarfMapperRegistry.Register(typeof(IEnumerable<IfSrcC>), typeof(List<IfDstC>),
            o => ((IEnumerable<IfSrcC>)o).Select(s => new IfDstC { X = s.X }).ToList());

        var lazy = new List<IfSrcC> { new() { X = 1 }, new() { X = 9 } }.Where(s => s.X > 5);
    ...
    public void An_exact_registration_still_wins_over_an_interface_one()
    {
        ...
        DwarfMapperRegistry.Register(typeof(IEnumerable<IfSrcD>), typeof(List<IfDstD>),
            _ => new List<IfDstD> { new() { X = -1 } });
        DwarfMapperRegistry.Register(typeof(List<IfSrcD>), typeof(List<IfDstD>),
            _ => new List<IfDstD> { new() { X = 42 } });
    ...
    public void Two_matching_interfaces_throw_rather_than_picking_one()
    {
        ...
        DwarfMapperRegistry.Register(typeof(IEnumerable<IfSrcE>), typeof(List<IfDstE>),
            _ => new List<IfDstE> { new() { X = 1 } });
        DwarfMapperRegistry.Register(typeof(IReadOnlyCollection<IfSrcE>), typeof(List<IfDstE>),
            _ => new List<IfDstE> { new() { X = 2 } });

        var ex = Assert.Throws<DwarfMapMissingException>(
            () => DwarfMapperRegistry.Map(new List<IfSrcE>(), typeof(List<IfDstE>)));
    ...
    {
        var ex = Assert.Throws<DwarfMapMissingException>(
            () => DwarfMapperRegistry.Map(new IfSrcF(), typeof(IfDstF)));
    ...
    public void A_lazy_iterator_with_no_map_at_all_is_told_to_materialize()
    {
        ...
        var lazy = new List<IfSrcG> { new() }.Where(_ => true);
