...
namespace DwarfMapper.IntegrationTests
{
    ...
    ///             <item>
    ///                 replacing it with check-then-act (a far narrower window) → caught in <b>22–26 of 60</b>
    ///                 rounds, failing on every one of three consecutive runs.
    ///             </item>
    ///         </list>
    ...
    ///         <list type="bullet">
    ///             <item>
    ///                 last-writer-wins → caught in <b>58 of 60</b> rounds even before the round count was
    ///                 raised;
    ///             </item>
    ///             <item>
    ///                 check-then-act → <b>23, 54 and 58 of 240</b> on three consecutive runs. At 60 rounds the
    ///                 same mutant scored 1, 6 and 7 — it still failed every run, but a margin of one round is a
    ///                 coin-flip away from greening a broken registry, which is why the count is 240 and not 60.
    ///             </item>
    ///         </list>
    ...
    ///         (<see cref="DeepPopulation.TortureCreateRounds" /> /
    ///         <see cref="DeepPopulation.TortureUpdateRounds" />).
    ///     </para>
    /// </summary>
    ...
    public sealed class RegistryConcurrencyTortureTests
    {
        ...
        private static readonly Type[] Tags =
        {
            typeof(K1), typeof(K2), typeof(K3), typeof(K4), typeof(K5), typeof(K6), typeof(K7), typeof(K8)
        };

        private static readonly ConcurrentDictionary<Type, Func<object, object>> Delegates = new();

        private static readonly Type[] UpdateTags =
        {
            typeof(U2), typeof(U3), typeof(U4)
        };

        private static readonly ConcurrentDictionary<Type, Action<object, object>> Stamps = new();

        private static Type SrcOf(Type tag)
        {
            return typeof(Src<>).MakeGenericType(tag);
        }

        private static Type DstOf(Type tag)
        {
            return typeof(Dst<>).MakeGenericType(tag);
        }
...
#pragma warning restore CA1031
        ...
        public void Racing_registrations_of_one_key_leave_exactly_one_winner()
        {
            ...
            Assert.True(replacementsSeen == 0,
                $"a reader saw the delegate for a key REPLACED in {replacementsSeen} of {rounds} rounds — " + "a registration became visible and was then overwritten, so TryAdd is no longer what decides " + "the winner.");
        }
        ...
        {
            return Delegates.GetOrAdd(dst, d => _ => Activator.CreateInstance(d)!);
        }
        ...
        public void Enumerating_Provided_while_registering_does_not_throw()
        {
            var churnTags = new[]
            {
                typeof(float), typeof(double), typeof(decimal), typeof(char), typeof(bool), typeof(string)
            };
        ...
        public void Racing_update_registrations_of_one_key_leave_exactly_one_winner()
        {
            ...
            for (var round = 0; round < rounds; round++)
            {
                ...
                Assert.False(DwarfMapperRegistry.IsAmbiguous(src, typeof(UpdateBox)),
                    $"round {round}: an update-table duplicate was recorded against the CREATE table, which has " + "no map for this pair at all.");
            }

            Assert.Empty(allFailures);
            Assert.True(replacementsSeen == 0,
                $"a reader saw the update delegate for a key REPLACED in {replacementsSeen} of {rounds} rounds — " + "a registration became visible and was then overwritten, so TryAdd is no longer what decides " + "the winner on the update table.");
        }
        ...
        private sealed class Src<T>
        {
            public T? Tag { get; set; }
        }

        private sealed class Dst<T>
        {
            public T? Tag { get; set; }
        }
