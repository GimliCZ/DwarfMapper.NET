...
// the values actually survive the round trip.
    ...
    public class FlagsEnumRuntimeTests
    {
        ...
        {
            var result = new FlagsMapper().Map(new FlagSrc
            {
                Perm = input
            });
        ...
        {
            var forward = new FlagsMapper().Map(new FlagSrc
            {
                AsText = PermSrc.Read | PermSrc.Exec
            });
            ...
            var back = new FlagsMapper().MapFromText(new FlagTextSrc
            {
                Perm = forward.AsText
            });
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new FlagsMapper().MapFromText(new FlagTextSrc
            {
                Perm = "Read, Nonsense"
            }));
        }
