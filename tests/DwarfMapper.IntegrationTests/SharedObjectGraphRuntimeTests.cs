...
namespace DwarfMapper.IntegrationTests
{
    ...
    public class TNode
    {
        ...
        public List<TNode> Children { get; set; } = new();
    }

    public class TNodeDto
    {
        public string Name { get; set; } = "";

        public List<TNodeDto> Children { get; set; } = new();
    }

    [DwarfMapper(ReferenceHandling = ReferenceHandlingStrategy.Preserve, MaxDepth = 64)]
    public partial class TreeGraphMapper
    {
        public partial TNodeDto Map(TNode n);
    }

// Two parents sharing the SAME list instance.
    public class SharingRoot
    {
        public Holder P1 { get; set; } = new();

        public Holder P2 { get; set; } = new();
    }
    ...
    public class SharingRootDto
    {
        public HolderDto P1 { get; set; } = new();

        public HolderDto P2 { get; set; } = new();
    }
    ...
    public partial class SharingMapper
    {
        public partial SharingRootDto Map(SharingRoot r);
    }
...
// under Preserve its mapper must still receive ctx so the shared instance dedupes.
    ...
    public class PlainDiamondRoot
    {
        public PlainLeaf? Left { get; set; }

        public PlainLeaf? Right { get; set; }
    }

    public class PlainLeafDto
    {
        public int V { get; set; }
    }

    public class PlainDiamondRootDto
    {
        public PlainLeafDto? Left { get; set; }

        public PlainLeafDto? Right { get; set; }
    }

    [DwarfMapper(ReferenceHandling = ReferenceHandlingStrategy.Preserve)]
    public partial class PlainDiamondMapper
    {
        public partial PlainDiamondRootDto Map(PlainDiamondRoot r);
    }
    ...
    public class SharedObjectGraphRuntimeTests
    {
        ...
        public void Shared_list_maps_to_one_target_instance()
        {
            ...
            var root = new SharingRoot
            {
                P1 = holder,
                P2 = holder
            }; // P1 and P2 are the SAME holder
        ...
        {
            var leaf = new PlainLeaf
            {
                V = 42
            };
            var root = new PlainDiamondRoot
            {
                Left = leaf,
                Right = leaf
            }; // SAME instance — no back-edge
