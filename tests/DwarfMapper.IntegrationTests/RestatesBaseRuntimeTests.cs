...
// is that check. It emits nothing — these tests exist to prove that the guarded mapping still behaves.
    ...
    [MapProperty<RbAliasCommand, RbAliasCommandDto>(nameof(RbCommand.Raw),
        nameof(RbCommandDto.Text),
        Use = nameof(Clean))]
...
// keeps every other member guarded rather than switching the check off wholesale.
    ...
    public class RestatesBaseRuntimeTests
    {
        ...
        {
            Assert.Equal("hi",
                new RbMapper().Map(new RbCommand
                {
                    Raw = "  hi  "
                }).Text);
        }
        ...
        public void The_restated_pair_maps_the_same_way()
        {
            ...
            var mapped = new RbMapper().Map(new RbAliasCommand
            {
                Raw = "  hi  ",
                Alias = "a"
            });
        ...
        public void A_declared_override_really_does_map_differently()
        {
            ...
            var mapped = new RbOverrideMapper().Map(new RbAliasCommand
            {
                Raw = "  hi  ",
                Alias = "a"
            });
            ...
            Assert.Equal("  hi  ", mapped.Text);
            Assert.Equal("hi",
                new RbOverrideMapper().Map(new RbCommand
                {
                    Raw = "  hi  "
                }).Text);
        }
