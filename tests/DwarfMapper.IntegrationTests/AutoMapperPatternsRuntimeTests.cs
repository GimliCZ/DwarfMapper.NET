...
// that calls them. All compile-time, AOT-safe.
    ...
    public partial class FcAlertsMapper
    {
        ...
        {
            if (s.ClientAlerts.TryGetValue(FcPlatform.YouTube, out var yt) && yt is FcYt y)
            {
                d.YouTube = MapYt(y);
            }

            if (s.ClientAlerts.TryGetValue(FcPlatform.Primary, out var tw) && tw is FcPrimary t)
            {
                d.Primary = MapTw(t);
            }
        }
        ...
        {
            if (s.YouTube is not null)
            {
                d.ClientAlerts[FcPlatform.YouTube] = MapYt(s.YouTube);
            }

            if (s.Primary is not null)
            {
                d.ClientAlerts[FcPlatform.Primary] = MapTw(s.Primary);
            }
        }
    }
...
// ───────────────────────── Pattern 4: polymorphic Data (src.Data as DiscordBasicNotification) ───────────
    ...
    public sealed class AutoMapperPatternsRuntimeTests
    {
        ...
        public void Pattern1_UpdateInto_SameType_mutates_existing_instance()
        {
            ...
            var existing = new FcStoreItem
            {
                Name = "old",
                Price = 1
            };
            mapper.Update(new FcStoreItem
                {
                    Name = "new",
                    Price = 42
                },
            ...
            var target = new FcStoreItem
            {
                Name = "x",
                Price = 0
            };
            var returned = mapper.UpdateFluent(new FcStoreItem
                {
                    Name = "y",
                    Price = 7
                },
        ...
        public void Pattern2_CustomEnumerable_source_maps_via_IEnumerable_param()
        {
            ...
            var ints = new FcConcurrentList<int>
            {
                1,
                2,
                3
            }; // custom IEnumerable<int>
        ...
        public void Pattern3_EnumKeyedDictionary_roundtrips_through_flat_document()
        {
            ...
            var model = new FcAlerts
            {
                ClientAlerts =
                {
                    [FcPlatform.YouTube] = new FcYt
                    {
                        Color = "red"
                    },
                    [FcPlatform.Primary] = new FcPrimary
                    {
                        Color = "purple"
                    }
                }
        ...
        public void Pattern4_Polymorphic_base_source_dispatches_to_derived_map()
        {
            ...
            FcNote basic = new FcBasic
            {
                Msg = "hello"
            };
            FcNote embed = new FcEmbed
            {
                Title = "title"
            };
