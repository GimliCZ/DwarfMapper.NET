...
namespace DwarfMapper.IntegrationTests
{
    ...
    public sealed class DwarfRefContextOnStackGuardTests
    {
        ...
        public void Outside_SetNull_mode_TryEnterNode_is_a_no_op_that_always_returns_true()
        {
            ...
            Assert.True(none.TryEnterNode(node), "None mode has no on-stack guard: re-entering the same " + "reference must still report a fresh enter, never a cycle.");

            var preserve = new DwarfRefContext(10, true);
            Assert.True(preserve.TryEnterNode(node));
            Assert.True(preserve.TryEnterNode(node), "Preserve mode reconstructs cycles via the identity map; " + "the on-stack guard must stay inert there too.");
        }
        ...
        public void In_SetNull_mode_TryEnterNode_detects_reentry_and_ExitNode_releases_the_node()
        {
            ...
            Assert.False(ctx.TryEnterNode(parent), "The same reference re-entering while on the stack is a " + "re-entrant back-edge and must report the cycle.");
            Assert.True(ctx.TryEnterNode(sibling), "A distinct node must be unaffected by another node " + "being on the stack.");

            ctx.ExitNode(parent);
            Assert.True(ctx.TryEnterNode(parent), "After ExitNode the reference has left the stack: a shared " + "but acyclic node (a diamond) is mapped again, not nulled — only true ancestor cycles break.");
        }
        ...
        public void Value_equal_records_are_distinct_nodes_on_the_stack()
        {
            ...
            Assert.True(ctx.TryEnterNode(second), "… but distinct references: the guard must use reference " + "identity, or value-equal records collapse into a phantom cycle.");
        }
