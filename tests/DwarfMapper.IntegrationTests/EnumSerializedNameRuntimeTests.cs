...
// wins over the member name. Separate one-way mappers keep each direction's fixture clean.
    ...
    public sealed class EsnEnumSrc
    {
        public EsnStatus Value { get; set; }
    }

    public sealed class EsnStringDst
    {
        public string Value { get; set; } = "";
    }

    public sealed class EsnStringSrc
    {
        public string Value { get; set; } = "";
    }

    public sealed class EsnEnumDst
    {
        public EsnStatus Value { get; set; }
    }
    ...
    public class EnumSerializedNameRuntimeTests
    {
        ...
        {
            var dto = new EnumSerializedNameMapper().ToText(new EsnEnumSrc
            {
                Value = input
            });
        ...
        {
            var dto = new EnumSerializedNameMapper().FromText(new EsnStringSrc
            {
                Value = input
            });
            Assert.Equal(expected, dto.Value);
        ...
        public void The_pair_round_trips_through_the_serialized_name()
        {
            ...
            foreach (var s in new[]
                     {
                         EsnStatus.InProgress, EsnStatus.Complete, EsnStatus.Cancelled
                     })
            {
                var text = mapper.ToText(new EsnEnumSrc
                {
                    Value = s
                }).Value;
                var back = mapper.FromText(new EsnStringSrc
                {
                    Value = text
                }).Value;
                Assert.Equal(s, back);
