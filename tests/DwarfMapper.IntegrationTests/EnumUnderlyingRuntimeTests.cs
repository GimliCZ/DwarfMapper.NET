...
// ── Tests ─────────────────────────────────────────────────────────────────────
    ...
    public class EnumUnderlyingRuntimeTests
    {
        ...
        public void ByName_byte_underlying_to_long_underlying_preserves_value()
        {
            ...
            Assert.Equal(EDstLong.A,
                mapper.Map(new EnumSrcByteHolder
                {
                    V = ESrcByte.A
                }).V);
            Assert.Equal(EDstLong.B,
                mapper.Map(new EnumSrcByteHolder
                {
                    V = ESrcByte.B
                }).V);
            Assert.Equal(EDstLong.C,
                mapper.Map(new EnumSrcByteHolder
                {
                    V = ESrcByte.C
                }).V);
        ...
        public void ByName_long_underlying_to_byte_underlying_preserves_value()
        {
            ...
            Assert.Equal(ESrcByte.A,
                mapper.Map(new EnumSrcLongHolder
                {
                    V = EDstLong.A
                }).V);
            Assert.Equal(ESrcByte.B,
                mapper.Map(new EnumSrcLongHolder
                {
                    V = EDstLong.B
                }).V);
            Assert.Equal(ESrcByte.C,
                mapper.Map(new EnumSrcLongHolder
                {
                    V = EDstLong.C
                }).V);
        ...
        public void ByName_ulong_underlying_large_value_preserved_by_name()
        {
            ...
            Assert.Equal(BigULong.Lo,
                mapper.Map(new BigSrcHolder
                {
                    V = BigULong.Lo
                }).V);
            Assert.Equal(BigULong.Hi,
                mapper.Map(new BigSrcHolder
                {
                    V = BigULong.Hi
                }).V);
        ...
        {
            var result = new EnumByteToIntMapper().Map(new EByteHolder
            {
                V = EByte.X
            });
        ...
        {
            var result = new EnumIntToLongMapper().Map(new EIntHolder
            {
                V = EInt.Y
            });
        ...
        public void Enum_long_to_int_narrowing_throws_on_overflow()
        {
            ...
            Assert.Throws<OverflowException>(() => mapper.Map(new ELongBigHolder
            {
                V = ELongBig.Big
            }));
        }
        ...
        {
            var result = new EnumLongSmallToIntMapper().Map(new ELongSmallHolder
            {
                V = ELongSmall.Small
            });
        ...
        public void EnumByValue_long_to_byte_narrowing_throws_on_overflow()
        {
            ...
            Assert.Throws<OverflowException>(() => mapper.Map(new LValHolder
            {
                V = LValEnum.X
            }));
        }
        ...
        {
            var result = new EnumByValNarrowMapper().Map(new LValHolder
            {
                V = LValEnum.Y
            });
