    ...
    public class SimdWidenRuntimeTests
    {
        ...
        public void All_seven_widen_pairs_map_correctly()
        {
            var src = new SwSrc
            {
                ...
                Floats = new[]
                {
                    float.MinValue, -1.5f, 0f, 1.5f, float.MaxValue, float.NaN, float.PositiveInfinity
                }
            };
        ...
        public void Vectorized_equals_scalar_oracle_for_every_length(int len)
        {
            ...
            var dst = new SwIntMapper().Map(new SwIntSrc
            {
                V = ints
            });
        ...
        {
            var d0 = new SwIntMapper().Map(new SwIntSrc
            {
                V = Array.Empty<int>()
            });
            Assert.Empty(d0.V);
            // null source array → empty target (None-collection default), never NRE.
            var dn = new SwIntMapper().Map(new SwIntSrc
            {
                V = null!
            });
            Assert.NotNull(dn.V);
