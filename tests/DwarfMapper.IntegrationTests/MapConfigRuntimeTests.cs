...
// within one compilation.
    public sealed class McS
    {
        public int MessagesCount { get; set; }
    }

    public sealed class McT
    {
    ...
    {
        private static void Cfg(MapConfig<McS, McT> c)
        {
            c.Map(t => t.NumberOfMessages, s => s.MessagesCount);
        }
    }
...
// `t => t.X`) using a fresh pair so it doesn't collide with McS/McT above (all fixtures share one compilation).
    public sealed class PmS
    {
        public int MessagesCount { get; set; }
    }

    public sealed class PmT
    {
        public int NumberOfMessages { get; set; }
    }
    ...
    {
        private static void Cfg(MapConfig<PmS, PmT> c)
        {
            c.Map(t => t.NumberOfMessages, s => s.MessagesCount);
        }
    }
...
// object — already works via TryReadMemberPath's chain-walk, with no collector change needed.
    ...
    public sealed class FlS
    {
        public Author Author { get; set; } = new();
    }

    public sealed class FlT
    {
    ...
    {
        private static void Cfg(MapConfig<FlS, FlT> c)
        {
            c.Map(t => t.AuthorName, s => s.Author.Name);
        }
    }
...
// handled by the existing 3-arg branch, with no collector change needed.
    public sealed class CvS
    {
        public string? Raw { get; set; }
    }

    public sealed class CvT
    {
    ...
    {
        private static void Cfg(MapConfig<CvS, CvT> c)
        {
            c.Map(t => t.Clean, s => s.Raw, Norm);
        }
...
// Op 3 fixtures: proves `.MapWhen` — conditional assignment gated by a predicate method group over the source.
    ...
    {
        private static void Cfg(MapConfig<WhS, WhT> c)
        {
            c.MapWhen(t => t.V, s => s.V, Gate);
        }

        private static bool Gate(WhS s)
        {
            return s.Ok;
        }
    }
...
// to build with DWARF001 — that build failure IS the RED for this op.
    public sealed class IgS
    {
        public int A { get; set; }
    }

    public sealed class IgT
    {
        public int A { get; set; }
    ...
    {
        private static void Cfg(MapConfig<IgS, IgT> c)
        {
            c.Ignore(t => t.Extra);
        }
    }
...
// only the factory method group can plausibly build a CnAlias, so this genuinely exercises `.Construct`.
    ...
    public sealed class CnAlias : IEquatable<CnAlias>
    {
        public CnAlias(string display)
        {
            Display = display;
        }
        ...
        public bool Equals(CnAlias? other)
        {
            return other is not null && Display == other.Display;
        }

        public override bool Equals(object? obj)
        {
            return Equals(obj as CnAlias);
        }

        public override int GetHashCode()
        {
            return Display.GetHashCode(StringComparison.Ordinal);
        }
    }
    ...
    {
        private static void Cfg(MapConfig<CnS, CnAlias> c)
        {
            c.Construct(Make);
        }

        private static CnAlias Make(CnS s)
        {
            return new CnAlias($"{s.Fmt}:{s.Name}");
        }
    }
...
// Op 7 fixtures: proves `.MapOr` value-type (int?) null-substitute parity against [MapProperty(NullSubstitute=)].
    public sealed class MoS
    {
        public int? V { get; set; }
    }

    public sealed class MoT
    {
        public int V { get; set; }
    }
    ...
    {
        private static void Cfg(MapConfig<MoS, MoT> c)
        {
            c.MapOr(t => t.V, s => s.V, 5);
        }
    }
...
// gap where the class-constrained MapOr overload was untested.
    public sealed class MoRS
    {
        public string? Name { get; set; }
    }

    public sealed class MoRT
    {
        public string? Name { get; set; }
    }
    ...
    {
        private static void Cfg(MapConfig<MoRS, MoRT> c)
        {
            c.MapOr(t => t.Name, s => s.Name, "none");
        }
    }
...
// Op 8 fixtures: proves `.Value` constant parity against [MapValue<T>(target, constant)].
    public sealed class VcS
    {
        public int A { get; set; }
    }

    public sealed class VcT
    {
        public int A { get; set; }
    ...
    {
        private static void Cfg(MapConfig<VcS, VcT> c)
        {
            c.Value(t => t.Tag, 99);
        }
    }
...
// Op 8 fixtures (compute overload): proves `.Value` method-group parity against [MapValue<T>(target){ Use = }].
    ...
    {
        private static void Cfg(MapConfig<VcS, VcT2> c)
        {
            c.Value(t => t.Tag, Make);
        }
...
// Op: rename (`.Map` 2-arg) — MxRnX via config, MxRnY via attribute, in the same MixedMapper.
    ...
    {
        private static void Cfg(MapConfig<MxRnS, MxRnT> c)
        {
            c.Map(t => t.X, s => s.A).Map(t => t.Y, s => s.B);
        }
    }
...
// Op: flatten (`.Map` dotted source selector) — MxFlAName via config, MxFlBNick via attribute.
    public sealed class MxFlA
    {
        public string? Name { get; set; }
    }

    public sealed class MxFlB
    {
        public string? Nick { get; set; }
    }

    public sealed class MxFlS
    {
        public MxFlA A { get; set; } = new();

        public MxFlB B { get; set; } = new();
    }

    public sealed class MxFlT
    {
    ...
    {
        private static void Cfg(MapConfig<MxFlS, MxFlT> c)
        {
            c.Map(t => t.AName, s => s.A.Name).Map(t => t.BNick, s => s.B.Nick);
        }
    }
...
// Op: convert (`.Map` 3-arg converter method group) — Clean1 via config, Clean2 via attribute.
    ...
    public sealed class MxCvT
    {
        public string? Clean1 { get; set; }

        public string? Clean2 { get; set; }
    }
    ...
    {
        private static void Cfg(MapConfig<MxCvS, MxCvT> c)
        {
            c.Map(t => t.Clean1, s => s.Raw1, Norm).Map(t => t.Clean2, s => s.Raw2, Norm);
        }
    ...
    public partial class MxCvMixedMapper
    {
        private static void Cfg(MapConfig<MxCvS, MxCvT> c)
        {
            c.Map(t => t.Clean1, s => s.Raw1, Norm);
        }
...
// Op: when (`.MapWhen`) — V1 gated via config, V2 gated via attribute.
    ...
    {
        private static void Cfg(MapConfig<MxWhS, MxWhT> c)
        {
            c.MapWhen(t => t.V1, s => s.V1, Gate1).MapWhen(t => t.V2, s => s.V2, Gate2);
        }

        private static bool Gate1(MxWhS s)
        {
            return s.Ok1;
        }

        private static bool Gate2(MxWhS s)
        {
            return s.Ok2;
        }
    }
...
// Op: ignore (`.Ignore`) — Extra1 ignored via config, Extra2 ignored via attribute.
    public sealed class MxIgS
    {
        public int A { get; set; }
    }

    public sealed class MxIgT
    {
        public int A { get; set; }
    ...
    {
        private static void Cfg(MapConfig<MxIgS, MxIgT> c)
        {
            c.Ignore(t => t.Extra1).Ignore(t => t.Extra2);
        }
    }
...
// IgnoreSource_on_config_method_silences_DWARF039 generator test's note above).
    ...
    {
        private static void Cfg(MapConfig<MxIsS, MxIsT> c)
        {
            c.IgnoreSource(s => s.Unused1).IgnoreSource(s => s.Unused2);
        }
...
// Op: value (`.Value` constant) — Tag1 via config, Tag2 via attribute.
    public sealed class MxVcS
    {
        public int A { get; set; }
    }

    public sealed class MxVcT
    {
        public int A { get; set; }
    ...
    {
        private static void Cfg(MapConfig<MxVcS, MxVcT> c)
        {
            c.Value(t => t.Tag1, 99).Value(t => t.Tag2, 77);
        }
    }
...
// the combined MixedMapper.
    ...
    public sealed class CnAlias2 : IEquatable<CnAlias2>
    {
        public CnAlias2(string display)
        {
            Display = display;
        }
        ...
        public bool Equals(CnAlias2? other)
        {
            return other is not null && Display == other.Display;
        }
    ...
    {
        private static CnAlias2 Make2(CnS2 s)
        {
            return new CnAlias2($"{s.Fmt}:{s.Name}");
        }
    }
...
// Op: nullsub (`.MapOr`) — V1 (value-type overload) via config, V2 (reference-type overload) via attribute.
    ...
    {
        private static void Cfg(MapConfig<MxMoS, MxMoT> c)
        {
            c.MapOr(t => t.V1, s => s.V1, 5).MapOr(t => t.V2, s => s.V2, "none");
        }
    }
...
// float/double/decimal target cast path through the MapConfig collector (widening int -> float).
    public sealed class NwS
    {
        public int A { get; set; }
    }

    public sealed class NwT
    {
        public int A { get; set; }
    ...
    {
        private static void Cfg(MapConfig<NwS, NwT> c)
        {
            c.Value(t => t.F, 5);
        }
    }
    ...
    public class MapConfigRuntimeTests
    {
        ...
        {
            var configResult = new McConfigMapper().Map(new McS
            {
                MessagesCount = 42
            });
            var attrResult = new McAttrMapper().Map(new McS
            {
                MessagesCount = 42
            });
        ...
        {
            var configResult = new PmConfigMapper().Map(new PmS
            {
                MessagesCount = 42
            });
            var attrResult = new PmAttrMapper().Map(new PmS
            {
                MessagesCount = 42
            });
        ...
        {
            var configResult = new CvConfigMapper().Map(new CvS
            {
                Raw = "  x  "
            });
            var attrResult = new CvAttrMapper().Map(new CvS
            {
                Raw = "  x  "
            });
        ...
        {
            var configOff = new WhConfigMapper().Map(new WhS
            {
                V = 9,
                Ok = false
            });
            var attrOff = new WhAttrMapper().Map(new WhS
            {
                V = 9,
                Ok = false
            });
            ...
            var configOn = new WhConfigMapper().Map(new WhS
            {
                V = 9,
                Ok = true
            });
            var attrOn = new WhAttrMapper().Map(new WhS
            {
                V = 9,
                Ok = true
            });
        ...
        {
            var result = new IgConfigMapper().Map(new IgS
            {
                A = 3
            });
        ...
        {
            var configNull = new MoConfigMapper().Map(new MoS
            {
                V = null
            });
            var attrNull = new MoAttrMapper().Map(new MoS
            {
                V = null
            });
            ...
            var configVal = new MoConfigMapper().Map(new MoS
            {
                V = 9
            });
            var attrVal = new MoAttrMapper().Map(new MoS
            {
                V = 9
            });
        ...
        {
            var configNull = new MoRConfigMapper().Map(new MoRS
            {
                Name = null
            });
            var attrNull = new MoRAttrMapper().Map(new MoRS
            {
                Name = null
            });
            ...
            var configVal = new MoRConfigMapper().Map(new MoRS
            {
                Name = "x"
            });
            var attrVal = new MoRAttrMapper().Map(new MoRS
            {
                Name = "x"
            });
        ...
        {
            var configResult = new VcConfigMapper().Map(new VcS
            {
                A = 1
            });
            var attrResult = new VcAttrMapper().Map(new VcS
            {
                A = 1
            });
        ...
        {
            var configResult = new VcComputeConfigMapper().Map(new VcS
            {
                A = 1
            });
            var attrResult = new VcComputeAttrMapper().Map(new VcS
            {
                A = 1
            });
        ...
        {
            var configResult = new NwConfigMapper().Map(new NwS
            {
                A = 1
            });
            var attrResult = new NwAttrMapper().Map(new NwS
            {
                A = 1
            });
