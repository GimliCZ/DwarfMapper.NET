...
namespace DwarfMapper.IntegrationTests
{
    ...
    public sealed class AmbientRegistryTests
    {
        ...
        {
            DwarfMapperRegistry.Register(typeof(S1),
                typeof(D1),
                s => new D1
                {
                    V = ((S1)s).V + 1
            ...
            var result = DwarfMapperRegistry.Map(new S1
                {
                    V = 41
                },
                typeof(D1));
        ...
        {
            DwarfMapperRegistry.Register(typeof(S2),
                typeof(D2),
                s => new D2
                {
                    V = ((S2)s).V * 2
            ...
            var byDest = mapper.Map<D2>(new S2
            {
                V = 5
            });
            var byBoth = mapper.Map<S2, D2>(new S2
            {
                V = 6
            });
        ...
        {
            DwarfMapperRegistry.Register(typeof(Base9),
                typeof(D9),
                s => new D9
                {
                    Tag = "base:" + ((Base9)s).V
                });
            DwarfMapperRegistry.Register(typeof(Derived9),
                typeof(D9),
                s => new D9
                {
                    Tag = "derived:" + ((Derived9)s).V
                });

            var mapper = DwarfMapperFacade.Instance;
            Base9 instance = new Derived9
            {
                V = 3
            };
        ...
        {
            DwarfMapperRegistry.Register(typeof(Base3),
                typeof(D3),
                s => new D3
                {
                    V = ((Base3)s).V
            ...
            var result = DwarfMapperRegistry.Map(new Derived3
                {
                    V = 7
                },
                typeof(D3));
        ...
        {
            var forSource = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Map(null!, typeof(D1)));
            Assert.Equal("source", forSource.ParamName);

            var forDestination = Assert.Throws<ArgumentNullException>(() => DwarfMapperRegistry.Map(new S1(), null!));
