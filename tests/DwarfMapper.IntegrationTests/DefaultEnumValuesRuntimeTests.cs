...
// gate (RuntimeCoverageScanTests.R3) had no real reference. These declare each default and assert what it does.
    ...
    public enum DevColor
    {
        Red = 1,
        Green = 2
    }

    public enum DevColorDto
    {
        Red = 1,
        Green = 2
    }

    public sealed class DevEnumSrc
    {
        public DevColor C { get; set; }
    }

    public sealed class DevEnumDst
    {
        public DevColorDto C { get; set; }
    }

    public sealed class DevNullSrc
    {
        public int? V { get; set; }
    }

    public sealed class DevNullDst
    {
        public int V { get; set; }
    }

    public sealed class DevNode
    {
        public int V { get; set; }

        public DevNode? Next { get; set; }
    }

    public sealed class DevNodeDto
    {
        public int V { get; set; }

        public DevNodeDto? Next { get; set; }
    }

    public sealed class DevPlainSrc
    {
    ...
    public sealed class DevPlainDst
    {
        public int A { get; set; }
    ...
    public partial class DevEnumMapper
    {
        public partial DevEnumDst Map(DevEnumSrc s);
    }

    [DwarfMapper(NullStrategy = NullStrategy.Throw)]
    public partial class DevNullMapper
    {
        public partial DevNullDst Map(DevNullSrc s);
    }

    [DwarfMapper(OnCycle = OnCycleStrategy.Throw, ReferenceHandling = ReferenceHandlingStrategy.None, MaxDepth = 16)]
    public partial class DevCycleMapper
    {
        public partial DevNodeDto Map(DevNode n);
    }
...
// mapping" defaults, named on one mapper that just maps correctly.
    ...
    public partial class DevPlainMapper
    {
        public partial DevPlainDst Map(DevPlainSrc s);
    }
    ...
    public class DefaultEnumValuesRuntimeTests
    {
        ...
        {
            var dto = new DevEnumMapper().Map(new DevEnumSrc
            {
                C = DevColor.Green
            });
        ...
        public void NullStrategy_Throw_throws_on_a_null_value_source()
        {
            ...
            Assert.ThrowsAny<Exception>(() => new DevNullMapper().Map(new DevNullSrc
            {
                V = null
            }));
            ...
            Assert.Equal(7,
                new DevNullMapper().Map(new DevNullSrc
                {
                    V = 7
                }).V);
        }
        ...
        {
            var dto = new DevPlainMapper().Map(new DevPlainSrc
            {
                A = 3,
                B = "x"
            });
