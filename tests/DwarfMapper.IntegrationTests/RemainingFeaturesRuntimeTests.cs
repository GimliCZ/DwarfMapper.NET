...
public class Rf_ScSrc { public int Id { get; set; } public string Audit { get; set; } = ""; }
public class Rf_ScDst { public int Id { get; set; } }
[DwarfMapper(RequiredMapping = RequiredMappingStrategy.Both)]
[MapIgnoreSource(nameof(Rf_ScSrc.Audit))]
public partial class Rf_ScMapper { public partial Rf_ScDst Map(Rf_ScSrc s); }

// ── [Reinterpret] — force a layout-compatible blit on differently-named struct fields ──
public struct Rf_Px { public int A; public int B; }
public struct Rf_Qx { public int X; public int Y; }
public class Rf_ReSrc { public Rf_Px[] V { get; set; } = System.Array.Empty<Rf_Px>(); }
public class Rf_ReDst { public Rf_Qx[] V { get; set; } = System.Array.Empty<Rf_Qx>(); }
[DwarfMapper] public partial class Rf_ReMapper
{
...
public class Rf_CtorSrc { public int Id { get; set; } }
public class Rf_CtorDst
{
    public int Id { get; }
    public Rf_CtorDst() => Id = -1;
    [DwarfMapperConstructor] public Rf_CtorDst(int id) => Id = id;
}
[DwarfMapper(CaseInsensitive = true)] public partial class Rf_CtorMapper { public partial Rf_CtorDst Map(Rf_CtorSrc s); }

// ── Method-level [AutoNest] ──────────────────────────────────────────────────
public class Rf_Inner { public int X { get; set; } }
public class Rf_InnerD { public int X { get; set; } }
public class Rf_AnSrc { public Rf_Inner Inner { get; set; } = new(); }
public class Rf_AnDst { public Rf_InnerD Inner { get; set; } = new(); }
[DwarfMapper] public partial class Rf_AnMapper
{
    [AutoNest(true)]
...
public class RemainingFeaturesRuntimeTests
{
    ...
    public void Reinterpret_blits_layout_compatible_struct_array()
    {
        ...
        Assert.Equal(1, d.V[0].X); Assert.Equal(2, d.V[0].Y); // A→X, B→Y positionally
        Assert.Equal(3, d.V[1].X); Assert.Equal(4, d.V[1].Y);
    }
