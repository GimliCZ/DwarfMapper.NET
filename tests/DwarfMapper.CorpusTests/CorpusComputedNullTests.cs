...
public class Cc_Person { public string FirstName { get; set; } = ""; public string LastName { get; set; } = ""; }
public class Cc_PersonDto { public string FullName { get; set; } = ""; }
[DwarfMapper] public partial class Cc_FullNameMapper
{
    [MapIgnore(nameof(Cc_PersonDto.FullName))]
    public partial Cc_PersonDto Map(Cc_Person s);
    [AfterMap] private static void Fill(Cc_Person s, Cc_PersonDto d) => d.FullName = $"{s.FirstName} {s.LastName}";
}

// #34 Computed aggregate total (dotnet/eShop OrderSummary)
public class Cc_OrderItem { public decimal UnitPrice { get; set; } public int Units { get; set; } }
public class Cc_Order { public List<Cc_OrderItem> Items { get; set; } = new(); }
public class Cc_OrderDto { public decimal Total { get; set; } }
[DwarfMapper] public partial class Cc_TotalMapper
{
    [MapIgnore(nameof(Cc_OrderDto.Total))]
    public partial Cc_OrderDto Map(Cc_Order s);
    [AfterMap] private static void Fill(Cc_Order s, Cc_OrderDto d) => d.Total = s.Items.Sum(i => i.UnitPrice * i.Units);
}

// #35 Constant target field (eShopOnWeb Status="Pending")
public class Cc_Req { public int Id { get; set; } }
public class Cc_Vm { public int Id { get; set; } public string Status { get; set; } = ""; }
[DwarfMapper] public partial class Cc_ConstMapper
{
...
public class Cc_PriceSrc { public decimal Price { get; set; } }
public class Cc_PriceDto { public string Price { get; set; } = ""; }
[DwarfMapper] public partial class Cc_CurrencyMapper
{
    ...
    private static string ToCurrency(decimal v) => v.ToString("C", CultureInfo.GetCultureInfo("en-US"));
}

// #42 Null substitution (AutoMapper NullSubstitute("N/A"))
public class Cc_NsSrc { public string Name { get; set; } = ""; public string? MiddleName { get; set; } }
public class Cc_NsDst { public string Name { get; set; } = ""; public string MiddleName { get; set; } = ""; }
[DwarfMapper] public partial class Cc_NullSubMapper
{
...
public class Cc_CreateCmd { public string Title { get; set; } = ""; public int ListId { get; set; } }
public class Cc_Todo { public string Title { get; set; } = ""; public int ListId { get; set; } public bool Done { get; set; } public DateTime CreatedUtc { get; set; } }
[DwarfMapper] public partial class Cc_AuditMapper
{
    ...
    [AfterMap] private static void Stamp(Cc_Todo d) => d.CreatedUtc = PinnedClock;
}
...
public sealed class Cc_Colour
{
    ...
    public Cc_Colour(string hex) => _hex = hex;
    public static implicit operator string(Cc_Colour c) => c._hex;
}
public class Cc_Item { public string Name { get; set; } = ""; }
public class Cc_ItemDto { public string Name { get; set; } = ""; }
public class Cc_TodoList { public string Title { get; set; } = ""; public Cc_Colour Colour { get; set; } = new("#000000"); public List<Cc_Item> Items { get; set; } = new(); }
public record Cc_TodoListDto(string Title, string Colour, IReadOnlyCollection<Cc_ItemDto> Items);
[DwarfMapper] public partial class Cc_RecordMapper
{
    ...
    private static string ColourToString(Cc_Colour c) => c;
}
...
public class CorpusComputedNullTests
{
    ...
    public void S33_Computed_full_name()
        => Assert.Equal("Margaret Hamilton", new Cc_FullNameMapper().Map(new Cc_Person { FirstName = "Margaret", LastName = "Hamilton" }).FullName);

    [Fact]
    public void S34_Computed_aggregate_total()
    {
        var d = new Cc_TotalMapper().Map(new Cc_Order { Items = new List<Cc_OrderItem> { new() { UnitPrice = 10, Units = 2 }, new() { UnitPrice = 5, Units = 3 } } });
    ...
    public void S42_Null_substitution()
    {
        ...
        Assert.Equal("Brewster", new Cc_NullSubMapper().Map(new Cc_NsSrc { Name = "Grace", MiddleName = "Brewster" }).MiddleName);
    }
    ...
    {
        var src = new Cc_TodoList { Title = "Work", Colour = new Cc_Colour("#0000FF"), Items = new List<Cc_Item> { new() { Name = "task" } } };
