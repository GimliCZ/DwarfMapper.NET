    ...
    public class CorpusComputedNullTests
    {
        ...
        public void S33_Computed_full_name()
        {
            Assert.Equal("Margaret Hamilton",
                new Cc_FullNameMapper().Map(new Cc_Person
                {
                    FirstName = "Margaret",
                    LastName = "Hamilton"
                }).FullName);
        ...
        public void S35_Constant_field()
        {
            Assert.Equal("Pending",
                new Cc_ConstMapper().Map(new Cc_Req
                {
                    Id = 1
                }).Status);
        }
        ...
        public void S40_Decimal_to_currency_string()
        {
            Assert.Equal("$1,234.50",
                new Cc_CurrencyMapper().Map(new Cc_PriceSrc
                {
                    Price = 1234.50m
                }).Price);
        }
        ...
        {
            Assert.Equal("N/A",
                new Cc_NullSubMapper().Map(new Cc_NsSrc
                {
                    Name = "Grace",
                    MiddleName = null
                }).MiddleName);
        ...
        {
            var d = new Cc_AuditMapper().Map(new Cc_CreateCmd
            {
                Title = "Write tests",
                ListId = 7
            });
        ...
        public void S48_Record_init_ctor_with_value_object()
        {
            ...
            {
                Title = "Work",
                Colour = new Cc_Colour("#0000FF"),
                Items = new List<Cc_Item>
                {
                    new()
                    {
                        Name = "task"
