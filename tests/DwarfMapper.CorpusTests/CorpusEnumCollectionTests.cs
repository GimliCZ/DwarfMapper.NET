...
public class CorpusEnumCollectionTests
{
    ...
    public void S21_Enum_by_name_maps_across_different_numeric_values()
    {
        Assert.Equal(Ce_ColorB.Black,
            new Ce_ByNameMapper().Map(new Ce_NameSrc { C = Ce_ColorA.Black }).C); // 1 → 3 (by name)
        Assert.Equal(Ce_ColorB.Blue,
            new Ce_ByNameMapper().Map(new Ce_NameSrc { C = Ce_ColorA.Blue }).C); // 2 → 4 (by name)
    }
    ...
    {
        var src = new Ce_Car
            { Tires = new List<Ce_Tire> { new() { Description = "Winter" }, new() { Description = "Summer" } } };
    ...
    public void S29_Element_map_drops_extra_member()
    {
        var src = new Ce_Cart
        {
            Items = new List<Ce_LineItem>
                { new() { Sku = "A1", Qty = 2, Price = 9.99m }, new() { Sku = "B2", Qty = 1, Price = 4.00m } }
        };
