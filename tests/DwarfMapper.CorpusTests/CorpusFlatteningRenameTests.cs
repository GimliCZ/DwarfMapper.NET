...
// #1 Basic nested flatten Customer.Name → CustomerName (eShopOnWeb Order→OrderDto; AutoMapper flattening)
    ...
    {
        public Cf_Customer Customer { get; set; } = new();

        public decimal Amount { get; set; }
    }

    public class Cf_OrderDto
    {
    ...
    [DwarfMapper]
    public partial class Cf_OrderMapper
    {
...
// #2 PascalCase flatten Make.Id→MakeId, Make.Name→MakeName (Mapperly Car flattening)
    ...
    public class Cf_Car
    {
        public Cf_Make Make { get; set; } = new();
    }
    ...
    [DwarfMapper]
    public partial class Cf_CarMapper
    {
...
// #4 Deep multi-level flatten Customer.Address.City → CustomerAddressCity (AutoMapper PascalCase traversal)
    ...
    public class Cf_Cust2
    {
        public Cf_Address Address { get; set; } = new();
    }

    public class Cf_DeepSrc
    {
        public Cf_Cust2 Customer { get; set; } = new();
    }

    public class Cf_DeepDst
    {
        public string CustomerAddressCity { get; set; } = "";
    }

    [DwarfMapper]
    public partial class Cf_DeepMapper
    {
...
// #9 Single-level unflatten MakeId→Make.Id, MakeName→Make.Name (Mapperly: explicit only)
    [DwarfMapper]
    public partial class Cf_UnflattenMapper
    {
...
// #11 Bidirectional rename round-trip Id ↔ Code (Mapster TwoWays)
    public class Cf_Poco
    {
        public int Id { get; set; }
    }

    public class Cf_CodeDto
    {
        public int Code { get; set; }
    }

    [DwarfMapper]
    public partial class Cf_TwoWayMapper
    {
        [ReverseMap]
...
// #13 Field rename Type→Name (eShopOnWeb CatalogType→CatalogTypeDto)
    ...
    public class Cf_CatalogTypeDto
    {
        public int Id { get; set; }
    ...
    [DwarfMapper]
    public partial class Cf_CatalogTypeMapper
    {
...
// #14/#15 Drop nav props + ignore sensitive field (eShopOnWeb CatalogItem; nopCommerce user)
    ...
    [DwarfMapper]
    public partial class Cf_CatalogItemMapper
    {
        public partial Cf_CatalogItemDto Map(Cf_CatalogItem s);
    }
    ...
    public class CorpusFlatteningRenameTests
    {
        ...
        {
            var d = new Cf_OrderMapper().Map(new Cf_Order
            {
                Customer = new Cf_Customer
                {
                    Name = "Ada Lovelace",
                    Email = "ada@x.io"
                },
        ...
        {
            var d = new Cf_CarMapper().Map(new Cf_Car
            {
                Make = new Cf_Make
                {
                    Id = 7,
        ...
        {
            var d = new Cf_DeepMapper().Map(new Cf_DeepSrc
            {
                Customer = new Cf_Cust2
                {
                    Address = new Cf_Address
                    {
                        City = "Prague",
                        Country = "CZ"
                    }
        ...
        {
            var car = new Cf_UnflattenMapper().Map(new Cf_CarDto
            {
                MakeId = 7,
                MakeName = "Audi"
            });
        ...
        {
            var d = new Cf_CatalogTypeMapper().Map(new Cf_CatalogType
            {
                Id = 3,
                Type = "Mug"
            });
        ...
        {
            var src = new Cf_CatalogItem
            {
                Id = 1,
                Name = "Mug",
                Price = 9.99m,
                CatalogBrandId = 4,
                CatalogBrand = new Cf_Brand
                {
                    BrandName = "ACME"
                },
