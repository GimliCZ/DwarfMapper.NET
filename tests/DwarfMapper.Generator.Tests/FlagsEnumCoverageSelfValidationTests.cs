...
namespace DwarfMapper.Generator.Tests
{
    ...
    ///     <list type="number">
    ///         <item>
    ///             <description>no schema ever declared a <c>[Flags]</c> enum, so the shape was never generated; and</description>
    ///         </item>
    ///         <item>
    ///             <description>
    ...
    public class FlagsEnumCoverageSelfValidationTests
    {
        ...
        public void ObjectFactory_produces_COMBINED_flag_values_not_just_declared_ones()
        {
            ...
            Assert.True(combined.Count > 0,
                ...
                "breaks it.\nProduced: " +
                string.Join(", ", produced.Distinct().OrderBy(v => (int)v)));
        }
        ...
        public void The_combinatorial_schema_declares_a_Flags_enum()
        {
            ...
            Assert.True(cells.Count > 0,
                "The combinatorial matrix contains no [Flags] enum basic type, so no cell can exercise combined " + "flag values against the other axes (widening, cycle mode, update-into, null strategy).");
