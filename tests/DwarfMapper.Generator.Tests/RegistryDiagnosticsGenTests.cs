    ...
    public sealed class RegistryDiagnosticsGenTests
    {
        ...
        public void The_registry_extension_class_is_internal_unless_the_assembly_opts_in()
        {
            ...
            Assert.Contains("internal static class __DwarfRegistry_Src",
                GeneratorTestHarness.RunAll(s).GeneratedSource,
        ...
        public void Every_registry_message_format_actually_formats()
        {
            ...
            foreach (var (name, descriptor) in descriptors)
            {
                ...
                {
                    broken.Add($"{descriptor.Id} ({name}): MessageFormat does not format — {ex.Message}. A " + "literal '{' or '}' in message text must be doubled ('{{' / '}}'); Roslyn " + "catches this and shows the caller the raw format string.");
                    continue;
                }

                if (!rendered.Contains("ARG-ZERO", StringComparison.Ordinal))
                {
                    broken.Add($"{descriptor.Id} ({name}): renders no argument at all, so nothing in the message " + "identifies WHICH declaration it is about.");
                }
            }
        ...
        public void Every_registry_diagnostic_is_triggered_and_documented()
        {
            ...
            Assert.True(ids.Count >= 6,
                $"Only {ids.Count} DWARFR descriptors reflected from RegistryDiagnostics — the gate would be vacuous. " + "The class moved or its field shape changed.");
            ...
            Assert.True(untested.Count == 0,
                "Registry diagnostic id(s) with NO triggering test in this file (assert `d.Id == \"DWARFR##\"`): " + string.Join(", ", untested) + ". Add a fact that drives the diagnostic from a minimal [MapTo] source.");

            var undocumented = ids.Where(id => !docsText.Contains(id, StringComparison.Ordinal)).ToList();
            Assert.True(undocumented.Count == 0,
                "Registry diagnostic id(s) missing from docs/diagnostics.md: " + string.Join(", ", undocumented) + ". Document each DWARFR code in the registry-diagnostics table.");
        }

        private static string SourcePath([CallerFilePath] string path = "")
        {
            return path;
        }
