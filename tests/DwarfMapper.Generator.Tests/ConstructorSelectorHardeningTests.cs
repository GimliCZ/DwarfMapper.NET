...
public class ConstructorSelectorHardeningTests
{
    ...
    {
        var (diags, _) = GeneratorTestHarness.Run(Internal("", flag: false));
    ...
    {
        var errors = GeneratorTestHarness.RunAndGetCompilationErrors(Internal("internal", flag: true));
        Assert.Empty(errors);
    }
    ...
    {
        var errors = GeneratorTestHarness.RunAndGetCompilationErrors(Internal("protected internal", flag: true));
        Assert.Empty(errors);
    }
    ...
    public void Private_ctor_with_flag_still_reports_DWARF026()
    {
        ...
        var (diags, _) = GeneratorTestHarness.Run(OnlyCtor("private", flag: true));
        Assert.Contains(diags, d => d.Id == "DWARF026");
    }
    ...
    {
        var (diags, _) = GeneratorTestHarness.Run(OnlyCtor("protected", flag: true));
        Assert.Contains(diags, d => d.Id == "DWARF026");
    }
    ...
    public void Most_params_ctor_is_auto_selected()
    {
        ...
        Assert.Contains("x:", source, System.StringComparison.Ordinal);
        Assert.Contains("y:", source, System.StringComparison.Ordinal); // the 2-arg ctor was chosen, not the 1-arg one
    }

    private static string Internal(string ctorAccessibility, bool flag) => OnlyCtor(ctorAccessibility, flag,
        extraParameterless: ctorAccessibility.Length == 0);
