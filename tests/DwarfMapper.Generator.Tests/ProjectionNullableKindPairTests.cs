...
namespace DwarfMapper.Generator.Tests
{
    ...
    public class ProjectionNullableKindPairTests
    {
        ...
        public static TheoryData<string, string> KindPairs()
        {
            return new TheoryData<string, string>
            {
                {
                    "struct", "struct"
                }, // control — always lifted
                {
                    "class", "class"
                }, // control — always lifted
                {
                ...
                {
                    "class", "struct"
                } // I14, the reverse genre
            };
        }

        private static string Declare(string kind, string name)
        {
            return kind switch
            {
        ...
        public void Project_lifts_null_across_every_kind_pair_and_agrees_with_Map(string srcKind, string dstKind)
        {
            ...
            Assert.True(assembly is not null,
                $"{srcKind}->{dstKind} did not emit: " + string.Join(", ", errors.Select(e => e.Id + " " + e.GetMessage(CultureInfo.InvariantCulture))));
        ...
        public void Project_lifts_a_null_ELEMENT_across_a_rekinded_pair(string srcKind, string dstKind)
        {
            ...
            var (assembly, errors) = GeneratorTestHarness.EmitAssembly(code);
            Assert.True(assembly is not null,
                $"{srcKind}->{dstKind} elements did not emit: " + string.Join(", ", errors.Select(e => e.Id + " " + e.GetMessage(CultureInfo.InvariantCulture))));
        ...
        public void Project_still_refuses_a_target_that_cannot_hold_the_null(string srcMember, string dstMember)
        {
            ...
            Assert.Contains("cannot hold null",
                refusal.GetMessage(CultureInfo.InvariantCulture),
        ...
        public void Project_refuses_rather_than_emitting_CS0037_for_a_value_type_nested_target()
        {
            ...
            var refusal = Assert.Single(diagnostics.Where(d => d.Id == "DWARF028"));
            Assert.Contains("value-type target",
                refusal.GetMessage(CultureInfo.InvariantCulture),
                StringComparison.Ordinal);
        ...
        private static object? Nested(object dto)
        {
            return dto.GetType().GetProperty("M")!.GetValue(dto);
        }
