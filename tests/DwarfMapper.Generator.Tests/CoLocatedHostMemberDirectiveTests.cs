    ...
    public sealed class CoLocatedHostMemberDirectiveTests
    {
        ...
        public void A_DwarfMapper_class_declaring_a_pair_refuses_a_directive_on_its_own_member()
        {
            ...
            Assert.True(d088.Count == 1,
                $"expected one DWARF088 for [MapIgnore] on the mapper's own member; got {d088.Count}. " + "B18: this placement used to be reported by nothing at all.");
            Assert.Contains("Mappers.Scratch",
                d088[0].GetMessage(CultureInfo.InvariantCulture),
