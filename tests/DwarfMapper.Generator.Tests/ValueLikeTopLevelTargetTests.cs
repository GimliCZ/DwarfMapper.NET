...
public class ValueLikeTopLevelTargetTests
{
    ...
    public void A_Guid_target_is_not_silently_object_constructed()
    {
        ...
        var refused = diagnostics.Any(d => d.Severity == Microsoft.CodeAnalysis.DiagnosticSeverity.Error);
        if (refused) return;
    ...
    public void A_DateTimeOffset_target_is_not_silently_object_constructed()
    {
        ...
        if (diagnostics.Any(d => d.Severity == Microsoft.CodeAnalysis.DiagnosticSeverity.Error)) return;
    ...
    public void A_member_less_struct_target_is_refused_rather_than_returned_empty()
    {
        ...
        Assert.True(refused || generated.Contains("Value =", StringComparison.Ordinal)
                            || generated.Contains("new global::Demo.Money(", StringComparison.Ordinal),
            "A member-less struct target was neither refused nor genuinely populated — which means it was "
            + "constructed empty and handed back as if it held the source's value.\n\n" + generated);
    }
}
