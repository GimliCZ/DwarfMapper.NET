...
namespace DwarfMapper.Generator.Tests
{
    ...
    public class UpdateIntoPreserveParityTests
    {
        ...
        public void Update_into_carries_the_preserve_context_exactly_as_create_does()
        {
            ...
            Assert.Empty(createDiag.Where(d => d.Severity == DiagnosticSeverity.Error));
            Assert.Empty(updateDiag.Where(d => d.Severity == DiagnosticSeverity.Error));

            var createCtx = CountContextHelpers(create);
            var updateCtx = CountContextHelpers(update);

            Assert.True(createCtx > 0,
                "Preserve mode emitted no context-carrying helper for the CREATE endpoint, so this test cannot " + "tell the two endpoints apart and would pass no matter what update did.");

            Assert.True(updateCtx > 0,
                $"Preserve mode emitted {createCtx} context-carrying helper call(s) for .Map and {updateCtx} for " + ".Update on the same mapper. ReferenceHandling is not reaching the update-into resolver, so " + "shared references are preserved through one endpoint and not the other.");
        }

        private static int CountContextHelpers(string generated)
        {
            return generated.Split("DwarfRefContext").Length - 1;
        }
