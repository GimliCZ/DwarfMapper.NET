...
namespace DwarfMapper.Generator.Tests
{
    ...
    internal static class GeneratorAssert
    {
        ...
        /// </summary>
        public static string CompilesClean(
            string source,
        ...
        /// </summary>
        public static string EmitsCompilableCode(
            string source,
            NullableContextOptions nullable = NullableContextOptions.Disable)
        {
        ...
        /// </summary>
        public static IReadOnlyList<Diagnostic> Reports(
            string source,
            string diagnosticId,
            NullableContextOptions nullable = NullableContextOptions.Disable)
        {
            ...
            Assert.True(matches.Count > 0,
                $"Expected diagnostic {diagnosticId}, but the generator reported " + (diagnostics.Length == 0 ? "nothing." : "only:\n  " + Describe(diagnostics)) + "\n\n--- source ---\n" + source);

            return matches;
        }

        /// <summary>Asserts the generator does NOT report <paramref name="diagnosticId" />.</summary>
        public static void DoesNotReport(
            string source,
            string diagnosticId,
            NullableContextOptions nullable = NullableContextOptions.Disable)
        {
        ...
        {
            return string.Join("\n  ",
                diagnostics.Select(d => d.Id + " (" + d.Severity + "): " + d.GetMessage(CultureInfo.InvariantCulture)));
        }
