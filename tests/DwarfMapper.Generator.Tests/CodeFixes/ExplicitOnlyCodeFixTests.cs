...
namespace DwarfMapper.Generator.Tests.CodeFixes
{
    ...
    public class ExplicitOnlyCodeFixTests
    {
        ...
        private static async Task<string> ApplyAsync(int actionIndex)
        {
            ...
            var d = diags.First(x => x.Id == "DWARF072" && x.GetMessage(CultureInfo.InvariantCulture).Contains("IsAdmin", StringComparison.Ordinal));
        ...
        public async Task Map_action_inserts_MapProperty_and_resolves_the_diagnostic()
        {
            ...
            Assert.Contains("MapProperty", fixedText, StringComparison.Ordinal);
            Assert.Contains("IsAdmin", fixedText, StringComparison.Ordinal);

            var (afterDiags, _) = GeneratorTestHarness.Run(fixedText);
            Assert.DoesNotContain(afterDiags,
                x => x.Id == "DWARF072" &&
                     x.GetMessage(CultureInfo.InvariantCulture).Contains("IsAdmin", StringComparison.Ordinal));
