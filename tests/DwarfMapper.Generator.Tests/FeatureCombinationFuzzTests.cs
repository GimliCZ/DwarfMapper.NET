...
#pragma warning disable CA1062
    ...
    public class FeatureCombinationFuzzTests
    {
        ...
        {
            ("rename", "public int F1o { get; set; }", "public int F1 { get; set; }", "[MapProperty(\"F1o\", \"F1\")]", ""), ("mapvalue", "", "public string F2 { get; set; } = \"\";", "[MapValue(\"F2\", \"v2\")]", ""), ("unflatten", "public string F3c { get; set; } = \"\";", "public FzAddr F3 { get; set; } = new();",
            ...
            ("enum", "public FzEnA F9 { get; set; }", "public FzEnB F9 { get; set; }", "", ""), ("nested", "public FzN F10 { get; set; } = new();", "public FzNDto F10 { get; set; } = new();", "", ""), ("narrow", "public long F11 { get; set; }", "public int F11 { get; set; }", "", ""),
            ("parse", "public string F12 { get; set; } = \"0\";", "public int F12 { get; set; }", "", ""), ("nullable", "public int? F13 { get; set; }", "public int? F13 { get; set; }", "", ""), ("ignore", "", "public int F14 { get; set; }", "[MapIgnore(\"F14\")]", ""), ("flatten", "public FzFl F15 { get; set; } = new();", "public int F15x { get; set; }", "[Flatten(\"F15\")]",
                ""),
        ...
        {
            ("none", "[DwarfMapper]", true), ("preserve", "[DwarfMapper(ReferenceHandling = ReferenceHandlingStrategy.Preserve)]", true), ("setnull", "[DwarfMapper(OnCycle = OnCycleStrategy.SetNull)]", true),
            ...
            ("strict", "[DwarfMapper(ImplicitConversions = false)]", true), ("caseinsensitive", "[DwarfMapper(CaseInsensitive = true)]", false), ("flexible", "[DwarfMapper(NameConvention = NameConvention.Flexible)]", false), ("byvalue", "[DwarfMapper(EnumStrategy = EnumStrategy.ByValue)]", false), ("setdefault", "[DwarfMapper(NullStrategy = NullStrategy.SetDefault)]", false),
        ...
        private static readonly int[] LossyFeatures =
        {
            10, 11
        };
        ...
        public void Full_power_set_compiles_on_every_emit_path()
        {
            ...
            Assert.True(failures.IsEmpty,
                failures.Count + " power-set combination(s) produced non-compiling code, e.g.: " + string.Join(" | ", failures.Take(8)));
        }
        ...
        private static void CompileInto(ConcurrentBag<string> failures, string kind, string mode, int[] sub)
        {
            ...
            if (errs.Length > 0)
            {
                failures.Add(kind +
                             ...
                             mode +
                             ":" +
                             string.Join("+", sub.Select(i => Features[i].Name)) +
                             " -> " +
                             errs[0].Id);
            }
        }
        ...
        private static string BuildSource(string kind, string mode, int[] featureIdx)
        {
            ...
            foreach (var i in featureIdx)
            {
                ...
                if (f.Attr.Length > 0)
                {
                    attrs.Append("    ").Append(f.Attr).Append('\n');
                }

                if (f.Helper.Length > 0)
                {
                    helpers.Append("    ").Append(f.Helper).Append('\n');
                }
            }
