...
namespace DwarfMapper.Generator.Tests
{
    ...
    public class ProjectionImplicitConversionsTests
    {
        ...
        public static TheoryData<string, string> LossyShapes()
        {
            return new TheoryData<string, string>
            {
                {
                    "plain member", "public class Src { public long M { get; set; } } public class Dst { public double M { get; set; } }"
                },
                {
                    "nested member", "public class I1 { public long M { get; set; } } public class I2 { public double M { get; set; } } " + "public class Src { public I1 C { get; set; } } public class Dst { public I2 C { get; set; } }"
                },
                {
                    "collection element", "public class Src { public System.Collections.Generic.List<long> A { get; set; } } " + "public class Dst { public System.Collections.Generic.List<double> A { get; set; } }"
                },
                {
                    "constructor parameter", "public class Src { public long M { get; set; } } " + "public class Dst { public Dst(double m) { M = m; } public double M { get; set; } }"
                }
            };
        }
        ...
        public void Project_refuses_a_lossy_conversion_under_strict_exactly_as_Map_does(string shape, string types)
        {
            ...
            Assert.True(d038.Count == 1,
                $"[{shape}] expected exactly one DWARF038 from the projection under " + $"ImplicitConversions = false, got {d038.Count}. I20: this endpoint reported none at all — " + "the strictness gate was silently off here.");
            Assert.True(d038[0].Severity == DiagnosticSeverity.Error,
                $"[{shape}] DWARF038 is {d038[0].Severity}, expected Error. The endpoints must agree on " + "WHETHER THE BUILD BREAKS — that is the only thing this option promises.");
        ...
        public void Under_the_default_both_endpoints_warn_once_and_neither_refuses(string shape, string types)
        {
            ...
            Assert.True(d038.Count == 2,
                $"[{shape}] expected one DWARF038 per method (2), got {d038.Count}. Both endpoints apply the " + "conversion, so both must say so — and neither may say it twice.");
        ...
        {
            const string types =
                "public class Src { public long Lossy { get; set; } public int Widening { get; set; } } " + "public class Dst { public double Lossy { get; set; } public long Widening { get; set; } }";
            ...
            Assert.Equal(long.MaxValue, projected[0].Lossy);
