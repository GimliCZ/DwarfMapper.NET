...
public class BlittableProofCoverageTests
{
    ...
    public void Fuzz_seeded_symmetric_layout_identical_struct_pairs_return_true()
    {
        var pairs = new[]
        {
            ...
            ("namespace T { public struct Sa { public float X; public float Y; float Z; } public struct Sb { public float X; public float Y; float Z; } }", "Sa", "Sb"),
            ("namespace T { public struct Sa { public long L; public int I; } public struct Sb { public long L; public int I; } }", "Sa", "Sb"),
        };
    ...
    public void CanReinterpret_structs_with_different_pointer_field_types_returns_false()
    {
        ...
        var model = compilation.GetSemanticModel(tree);
        var root  = tree.GetRoot();
        var types = new System.Collections.Generic.Dictionary<string, INamedTypeSymbol>(StringComparer.Ordinal);
        foreach (var decl in root.DescendantNodes()
            .OfType<Microsoft.CodeAnalysis.CSharp.Syntax.TypeDeclarationSyntax>())
        {
            var sym = model.GetDeclaredSymbol(decl);
