...
namespace DwarfMapper.Generator.Tests
{
    ...
    ///             endpoints</b>
    ///         — the assertion is always "Project matches the documented value" AND "Map matches
    ...
    public class ProjectionNullCollectionsTests
    {
        ...
        public static TheoryData<string, bool, string, bool> Cells()
        {
            ...
            foreach (var kind in new[]
                     {
                         "List<int>", "int[]", "IEnumerable<int>", "IReadOnlyList<int>"
                     })
            {
            ...
            bool expectNull)
        {
            ...
            Assert.True(assembly is not null,
                $"[{option}|nullable={targetNullable}|{kind}] did not emit: " + string.Join(", ", errors.Select(e => e.Id + " " + e.GetMessage(CultureInfo.InvariantCulture))));
            ...
            var cell = $"[NullCollections{(string.IsNullOrEmpty(option) ? "=AsEmpty (default)" : option)}" + $" | target {(targetNullable ? "nullable" : "non-nullable")} | {kind}] ";
            ...
            if (expectNull)
            {
                ...
                Assert.True(projectedNull is null,
                    cell + "Project did not propagate the null under AsNull over a null-capable target — it " + "produced " + Describe(projectedNull) + ". Reading NullCollections is not enough: the " + "AsNull branch has to survive the read.");
            }
            else
            {
                ...
                Assert.True(projectedNull is not null,
                    cell + "I19: Project returned null for a null source collection where the documented " + "behaviour is an EMPTY one (AsEmpty, or AsNull degraded because the target cannot hold " + "the null). Map returned " + Describe(mappedNull) + ".");
        ...
        private static object Materialise(string kind, int[] values)
        {
            return kind switch
            {
                "int[]" => values,
        ...
        private static string Describe(object? value)
        {
            return value is null
                ? "null"
                : value.GetType().Name + " with " + ((IEnumerable)value).Cast<object?>().Count().ToString(CultureInfo.InvariantCulture) + " element(s)";
        }
    }
}
