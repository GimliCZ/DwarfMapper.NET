...
namespace DwarfMapper.Generator.Tests.Framework
{
    ...
    internal static class GeneratorCacheAssert
    {
        ...
        public static void FullyCachedOnRerun(
        ...
        public static void CachedAfterUnrelatedEdit(
            IIncrementalGenerator generator,
            string source,
            IReadOnlyList<string> trackingNames)
        {
        ...
        public static void NoSymbolsInPipeline(
            IIncrementalGenerator generator,
            string source,
            IReadOnlyList<string> trackingNames)
        {
            ...
            foreach (var name in trackingNames)
            {
                ...
                foreach (var value in steps.SelectMany(s => s.Outputs).Select(o => o.Value))
                {
                    ...
                    if (leak is not null)
                    {
                        ...
                        Assert.Fail(where + " — pipeline models must not hold ISymbol, SyntaxNode, Location or " + "Compilation. They are never equatable (so caching dies) and they root old compilations, " + "forcing Roslyn to retain memory it could free.");
                    }
                    ...
                    {
                        Assert.Fail($"Symbol-leak walk could not inspect {truncation.SkippedMembers.Count} " + $"member(s) while checking step '{name}' — a getter threw, so the walk skipped it " + "instead of seeing its value. A member that could not be inspected is not evidence of " + "a clean model:\n  " + string.Join("\n  ", truncation.SkippedMembers));
                    }
                    ...
                    if (truncation.Hit)
                    {
                        Assert.Fail($"Symbol-leak walk hit its depth cap ({MaxWalkDepth}) at {truncation.Path} " + $"while checking step '{name}'. A truncated branch is NOT evidence of a clean model " + "— the model graph grew deeper than the walk. Raise MaxWalkDepth and re-run.");
                    }
                }
            }

            Assert.True(missing.Count == 0,
                "Declared tracking name(s) never appeared in TrackedSteps: " + string.Join(", ", missing) + ". Either the fixture does not trigger that pipeline, or the name in AllStepNames is stale. A step " + "that never runs is not evidence of freedom from leaked symbols — it is an unasserted step.");
        }
        ...
        [SuppressMessage("Design",
            ...
            Justification = "A property getter here is arbitrary third-party/generator model code; any exception " + "it throws must be swallowed so this test helper fails for the leak it's checking, not for an " + "unrelated getter side effect.")]
        private static SymbolLeak? FindSymbolLeak(
            object? value,
            string path,
            int depth,
            ...
            TruncationTracker truncation)
        {
            ...
            foreach (var property in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                var propertyLeak = FindSymbolLeak(propertyValue,
                    $"{path}.{property.Name}",
                    depth + 1,
                    visited,
                    truncation);
                if (propertyLeak is not null)
                {
                    return propertyLeak;
                }
            }
                    ...
                    step => Assert.All(step.Outputs,
                        output =>
                            Assert.True(
                                ...
                                $"Step '{name}' was {output.Reason} after {what}; expected Cached/Unchanged. A " + "non-value-equatable field (raw ImmutableArray, leaked symbol, unstable collection) " + "likely broke the model's equality and disabled incremental caching.")));
            }

            Assert.True(missing.Count == 0,
                "Declared tracking name(s) never appeared in TrackedSteps: " + string.Join(", ", missing) + ". Either the fixture does not trigger that pipeline, or the name in AllStepNames is stale. A step " + "that never runs is not evidence of cacheability — it is an unasserted step.");
        }

        /// <summary>
        ///     Where a leaked <see cref="ISymbol" />/<see cref="SyntaxNode" />/etc. was found, relative to
        ///     the tracked step's own output value, and the runtime type name of the offending value.
        /// </summary>
