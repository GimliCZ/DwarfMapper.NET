    ...
    public class GoldenCorpusTests
    {
        ...
        public void Generated_output_matches_the_golden_manifest()
        {
            ...
            if (Environment.GetEnvironmentVariable(GoldenManifest.UpdateEnvVar) == "1")
            {
                Assert.False(IsRunningInCi(),
                    $"{GoldenManifest.UpdateEnvVar}=1 was set in a CI environment. CI must not be able to bless a " + "regenerated manifest — that silently rubber-stamps whatever the generator currently produces. " + "Regenerate the manifest locally, review the diff, and commit it deliberately.");
            ...
            Assert.True(expected.Count > 0,
                $"No golden manifest at {GoldenManifest.Path}. It is never auto-created — generate it deliberately " + $"with {GoldenManifest.UpdateEnvVar}=1 and commit the result after reviewing it.");
        ...
        public void The_corpus_has_not_silently_shrunk()
        {
            ...
            Assert.True(cases.Count >= MinimumCases,
                $"Golden corpus has {cases.Count} cases, floor is {MinimumCases}. A shrinking corpus passes every " + "other test while covering less — lower the floor only deliberately.");
        ...
        {
            return !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("CI")) || !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("TF_BUILD")) || !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("GITHUB_ACTIONS"));
        }

        private static void AssertAtLeast(int actual, int floor, string axisName)
        {
            Assert.True(actual >= floor,
                $"Axis {axisName} has {actual} cases, floor is {floor}. One axis collapsing hides behind a " + "healthy total count — lower the floor only deliberately.");
        }
        ...
        private static string BuildFailure(List<string> added, List<string> removed, List<string> changed)
        {
            ...
            if (added.Count > 0)
            {
                sb.Append("ADDED (new case needs review):\n  ").AppendJoin("\n  ", added).Append('\n');
            }

            if (removed.Count > 0)
            {
                sb.Append("REMOVED (corpus shrank):\n  ").AppendJoin("\n  ", removed).Append('\n');
            }
