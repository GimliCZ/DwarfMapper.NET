...
public class ConversionPolicyGeneratorTests
{
    ...
    public void The_option_answers_the_same_across_every_Nullable_permutation_of_a_pair(
        string srcType, string tgtType, bool lossy)
    {
        foreach (var strict in new[] { false, true })
        {
            ...
            if (!lossy)
            {
                Assert.True(d is null,
                    $"{where}: the conversion loses nothing and must not be flagged, but DWARF038 fired "
                    + $"as {d?.Severity}. A classifier widened too far is the failure mode this row guards.");
                Assert.DoesNotContain(diags, x => x.Severity == DiagnosticSeverity.Error);
                continue;
            }

            Assert.True(d is not null,
                $"{where}: no DWARF038 at all. The option promises to report this conversion, and a "
                + "Nullable<> wrapper is not a reason for it to go quiet (I20).");
            var expected = strict ? DiagnosticSeverity.Error : DiagnosticSeverity.Warning;
            Assert.True(d!.Severity == expected,
                $"{where}: DWARF038 is {d.Severity}, expected {expected}. Under "
                + "ImplicitConversions = false the build must BREAK — a Warning means the strict setting "
                + "was read as permissive somewhere on the way down (I20).");
        }
    }
