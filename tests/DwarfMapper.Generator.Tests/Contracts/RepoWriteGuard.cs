...
public static class RepoWriteGuard
{
    ...
    /// </summary>
    public static bool WriteBack(string path, string contents) => WriteBack(path, contents, IsMutationRun);
    ...
    public static bool WriteBackLines(string path, IReadOnlyCollection<string> lines) =>
        WriteBackLines(path, lines, IsMutationRun);
    ...
    internal static string? FindMutationMarker(string baseDirectory)
    {
        ...
        {
            if (string.Equals(dir.Name, "StrykerOutput", StringComparison.OrdinalIgnoreCase)
                || string.Equals(dir.Name, ".stryker-tmp", StringComparison.OrdinalIgnoreCase))
                return dir.FullName;
        }
