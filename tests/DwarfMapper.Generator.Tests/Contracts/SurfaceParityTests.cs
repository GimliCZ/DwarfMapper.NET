...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    ///             not a correct generator.</b>
    ///         <c>SurfaceProbe.Classify</c> returns <c>Honoured</c> when the emitted
    ...
    public sealed class SurfaceParityTests
    {
        ...
        ///             a product defect.</b>
        ///         A11 gave the <c>Struct</c> site a slot and measured <c>[MapTo]</c> there
        ...
        ///             <c>AsyncStream</c></b>
        ///         — <c>CS7036</c> in the emitted mapper, because <c>Preserve</c> adds a
        ...
        ///         reclassification of <c>D17</c>:
        ///         <c>
        ///             [assembly: DwarfMapperDefaults(RegisterCollectionShapes =
        ///             false)]
        ///         </c>
        ///         at the <c>[MapTo]</c> registry front door. The reason is a MEASUREMENT, not a
        ...
        ///         <b>That is
        ///             history now, in both halves.</b>
        ///         R4 is fixed, so a <c>CS8795</c> behind a blocking DWARF error
        ...
        ///         this one did not resolve to a single verdict — measured,
        ///         <b>2 cells are Honoured and 18
        ...
        ///     </para>
        ///     <para>
        ///         62 → <b>54</b> when <c>D11</c> closed: eight cells, one verdict —
        ///         <b>8 Refused
        ...
        ///     </para>
        ///     <para>
        ///         54 → <b>38</b> when <c>D8</c> closed: sixteen cells, one verdict —
        ///         <b>16 Refused
        ...
        ///     </para>
        ///     <para>
        ///         12 → <b>9</b> when <c>D6</c> and <c>D7</c> closed: three cells, one verdict —
        ///         <b>3 Refused
            ...
            Endpoint endpoint)
        {
            ...
            if (effect is SurfaceEffect.NoSuchSite or SurfaceEffect.NotCompilable
                or SurfaceEffect.EmittedInvalidCode)
            {
                return;
            }
            ...
            if (claimed)
            {
                if (effect is SurfaceEffect.Honoured or SurfaceEffect.Refused
                    or SurfaceEffect.UnhonouredButLoud)
                {
                    return;
                }
                ...
                if (StructurallyInapplicableOption(c.Axis, endpoint) is not null)
                {
                    return;
                }
            ...
            {
                if (effect is SurfaceEffect.Silent or SurfaceEffect.UnhonouredButLoud)
                {
                    return;
                }

                Assert.Fail(
                    $"{c.Rendered} on a {site} does NOT claim {endpoint} ({ClaimSource(element, c.Site)}) but is " + $"{effect} there ({detail}). The claim under-reaches: this cell is live and the matrix was " + $"told to skip it. Add {endpoint} to the claim that governs the {site} site.");
            }
        }
        ...
        {
            return element.SiteClaims.Any(sc => (sc.Site & site) == site)
        ...
        public void The_matrix_is_not_vacuous()
        {
            ...
            Assert.True(acting >= 10,
                $"Only {acting} elements visibly act at CreateMap. Either the fixtures stopped triggering their " + "elements or Classify() is not observing correctly — in both cases the matrix is passing " + "without testing anything.");
        }
        ...
        public void The_cells_that_pose_no_question_are_declared_and_counted()
        {
            ...
            AssertRatchet(unaskable,
                UnaskableCellCeiling,
                "cells pose the generator no question",
                "Close a hole instead: give the case a fixture that triggers it with " + "[DwarfSurfaceProbe(ProbeKey = ...)], or an argument list that bites.");
        }
        ...
        public void The_cells_whose_generated_code_does_not_compile_are_counted()
        {
            ...
            AssertExactPin(broken,
                EmittedInvalidCodeCellCeiling,
                "cells carry a compiler error against code the GENERATOR emitted",
                "There is one way to close one of these and it is not a bookkeeping move: make the generator " + "emit valid C#, or refuse the shape with a diagnostic before it emits anything. A cell here is " + "strictly worse than a refusal — the consumer gets a broken build in a file they never wrote, " + "and no statement of what to change.");
        }
        ...
        public void The_cells_the_compiler_rejects_are_counted()
        {
            ...
            AssertExactPin(rejected,
                NotCompilableCellCeiling,
        ...
        public void The_cells_that_pass_both_claim_branches_are_counted()
        {
            ...
            AssertRatchet(undecided,
                UnhonouredButLoudCellCeiling,
                "cells pass both the claimed and the unclaimed branch, so they decide nothing",
                "Close one by making the fixture's BASELINE compile, so the element under test is the only " + "difference between the two compilations and its verdict is legible again.");
        }
        ...
        public void The_cells_with_no_declaration_site_are_counted_by_cause()
        {
            ...
            foreach (var (cause, pinned) in NoSuchSiteCausePins)
            {
                ...
                AssertExactPin(cells,
                    pinned,
                    $"cells have no declaration site under '{cause}'",
        ...
        private static string? StructurallyInapplicableOption(string axis, Endpoint endpoint)
        {
            ...
            return DeclaredDivergences.StructurallyInapplicable
                .TryGetValue((axis[..eq], endpoint), out var why)
                ? why
                : null;
        }
        ...
        public void The_cells_excused_as_structural_are_counted()
        {
            ...
            foreach (var endpoint in EndpointSources.All)
            {
                if ((SurfaceCatalog.ClaimFor(element, c.Site) & ToFlag(endpoint)) == SurfaceEndpoints.None)
                {
                    continue;
                }

                if (SurfaceProbe.Classify(c, endpoint).Effect is not SurfaceEffect.Silent)
                {
                    continue;
                }

                if (StructurallyInapplicableOption(c.Axis, endpoint) is not { } why)
                {
                    continue;
                }
            ...
            AssertRatchet(excused,
                StructurallyExcusedCellCeiling,
                "cells are excused because one option of a bag has no surface at that endpoint",
                "Close one by giving the endpoint the surface the option configures, and delete the " + "StructurallyInapplicable row — nothing else will notice that it became stale.");
        }
        ...
        public void The_recorded_divergences_are_counted()
        {
            ...
            const string maintainerDecision =
                "A new divergence is a MAINTAINER DECISION, not a way past a red build: it says the generator " + "has a defect that will ship. Fix the generator, or refuse the directive with a diagnostic. " + "Only if neither is possible now does a row belong here — and then someone raises this number " + "deliberately, in a commit that says why.";

            AssertExactPin(findings,
                DivergenceFindingCeiling,
                "divergences are recorded as unfixed",
                maintainerDecision);

            var cells = DeclaredDivergences.AllDeclaredCells()
                .Select(x => $"  {x.Id}: {x.Cell.UsageName}`{x.Cell.Arity}({x.Cell.Axis}) on a {x.Cell.Site} " + $"@ {x.Endpoint}")
                .OrderBy(s => s, StringComparer.Ordinal)
                .ToList();

            AssertExactPin(cells,
                DivergentCellCeiling,
                "cells are covered by a recorded divergence",
                maintainerDecision);
        }
        ...
        public void Every_declared_divergence_is_still_a_divergence()
        {
            ...
            foreach (var (id, _, cell, endpointFlag) in DeclaredDivergences.AllDeclaredCells())
            {
                ...
                var element = SurfaceCatalog.CrossProductElements.FirstOrDefault(e => string.Equals(e.UsageName, cell.UsageName, StringComparison.Ordinal) && Arity(e) == cell.Arity);
                var probed = element is null
                    ? null
                    : SurfaceCatalog.CasesFor(element).FirstOrDefault(x => string.Equals(x.Axis, cell.Axis, StringComparison.Ordinal) && x.Site == cell.Site);

                if (probed is null)
                {
                    stale.Add($"{where} — names no cell the matrix measures. The element, its axis or its site " + "changed; the row now governs nothing while reading as a reviewed decision.");
                    continue;
                }
                ...
                {
                    stale.Add($"{where} — the element no longer CLAIMS this endpoint, so the cell is not judged " + "here any more. The divergence was declared away rather than fixed; delete the " + "row so one cell is not recorded twice in two contradictory ways.");
                    continue;
                }
        ...
        public void Every_finding_covers_cells_and_no_cell_is_covered_twice()
        {
            ...
            Assert.True(empty.Count == 0,
                "Finding(s) covering no cell at all: " + string.Join(", ", empty) + ". A finding that pins nothing is a note, not a record — delete it or name its cells.");
            ...
            Assert.True(duplicates.Count == 0,
                "Cell(s) covered by more than one finding:\n  " + string.Join("\n  ", duplicates) + "\n\nOne cell is evidence for one finding. Two rows over one cell means one of them can be " + "deleted without the cell becoming visible again.");
            ...
            foreach (var (id, divergence) in DeclaredDivergences.Reasons)
            {
                ...
                Assert.False(string.IsNullOrWhiteSpace(divergence.Why),
                    $"{id} states no reason. The reason is the whole difference between a recorded defect and an " + "exemption: it must say what a caller who wrote this reasonably expects.");
        ...
        private static void AssertEvidenceLinkResolves(string id, string section)
        {
            ...
            Assert.True(section.StartsWith("Issues/", StringComparison.Ordinal) && hash > "Issues/".Length,
                $"{id} does not link to a section of the findings write-up ('{section}'). The reason field " + "states what a caller expects; the write-up carries the evidence, and a record with nowhere " + "to read the evidence is an assertion.");
            ...
            Assert.True(File.Exists(path),
                $"{id}'s evidence link points at '{relative}', which does not exist. The write-up was moved or " + "renamed and the record now cites nothing.");

            var text = File.ReadAllText(path);
            var resolved = text.Contains($"<a id=\"{anchor}\"", StringComparison.Ordinal) || HeadingSlugs(text).Contains(anchor);

            Assert.True(resolved,
                $"{id}'s evidence link '{section}' names an anchor that '{relative}' does not define. Neither " + $"an <a id=\"{anchor}\"> nor a heading slugging to '{anchor}' is in the file, so the link " + "lands at the top of the document and the reader has to hunt for the evidence the record " + "claims is written up.");
        }
        ...
        private static void AssertRatchet(List<string> cells, int ceiling, string what, string howToClose)
        {
            Assert.True(ceiling > 10,
                $"AssertRatchet was called with a ceiling of {ceiling}. Its shrink side is a ten-wide tolerance " + "band and cannot see churn in a population that small — every count from zero upward would " + $"pass, while its doc comment promises otherwise. Use {nameof(AssertExactPin)}.");
            ...
            Assert.True(cells.Count >= ceiling - 10,
                $"Only {cells.Count} {what}, well under the ceiling of {ceiling}. Lower the ceiling to lock the " + "improvement in.");
        }
        ...
        private static void AssertExactPin(List<string> cells, int pinned, string what, string howToClose)
        {
            Assert.True(cells.Count <= pinned,
                $"{cells.Count} {what}, above the pinned count of {pinned}:\n" + string.Join("\n", cells) + "\n\nThis number may only shrink, and it is pinned EXACTLY: the " + "room a closed cell frees is not available to a new one. " + howToClose);

            Assert.True(cells.Count >= pinned,
                $"Only {cells.Count} {what}, under the pinned count of {pinned}. If a cell was genuinely closed, " + "lower the pin in the same commit to lock the improvement in — this population is pinned " + "exactly, so an improvement does not silently fund a replacement.");
        }
        ...
        public void The_theory_key_names_exactly_one_case()
        {
            ...
            Assert.True(duplicates.Count == 0,
                "The (name, arity, axis, site) key is not unique: " + string.Join(", ", duplicates) + ". Add a discriminator to Cells() rather than letting one row stand in for two cases.");
        }
        ...
        private static (SurfaceElement, SurfaceCase) Resolve(string usageName, int arity, string axis, string site)
        {
            var element = SurfaceCatalog.CrossProductElements
                .Single(e => string.Equals(e.UsageName, usageName, StringComparison.Ordinal) && Arity(e) == arity);
            var c = SurfaceCatalog.CasesFor(element)
                .Single(x => string.Equals(x.Axis, axis, StringComparison.Ordinal) && string.Equals(x.Site.ToString(), site, StringComparison.Ordinal));
            return (element, c);
        }

        private static int Arity(SurfaceElement element)
        {
            return element.Type.GetGenericArguments().Length;
        }

        private static SurfaceEndpoints ToFlag(Endpoint e)
        {
            return Enum.Parse<SurfaceEndpoints>(e.ToString());
        }
    }
}
