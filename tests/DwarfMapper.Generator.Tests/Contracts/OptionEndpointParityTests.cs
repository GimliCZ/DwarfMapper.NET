...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    public class OptionEndpointParityTests
    {
        ...
        public void An_option_that_acts_at_CreateMap_does_not_go_silent_at_another_endpoint(
            ...
            Endpoint endpoint)
        {
            var cell = OptionCatalog.Options.Single(c => string.Equals(c.Name, option, StringComparison.Ordinal) && string.Equals(c.ValueLabel, value, StringComparison.Ordinal));
            ...
            if (reference.Effect == OptionEffect.Silent)
            {
                return;
            }

            var actual = OptionProbe.Classify(endpoint, cell.NonDefault, cell.Types);
            if (actual.Effect != OptionEffect.Silent)
            {
                return;
            }
            ...
            if (DeclaredDivergences.CoversOption(option, endpoint))
            {
                return;
            }
        ...
        public void The_parity_property_is_not_vacuous()
        {
            ...
            Assert.True(acting >= 6,
                $"Expected several options to visibly act at CreateMap, but only {acting} did. Either the " + "fixtures stopped triggering their options or Classify() is not observing correctly — in " + "both cases the parity theory is passing without testing anything.");
        }
        ...
        public void DWARF077_does_not_fire_when_the_mapper_is_not_explicit_only()
        {
            ...
            foreach (var endpoint in new[]
                     {
                         Endpoint.SpanMap, Endpoint.AsyncStream
                     })
            {
