...
public class SurfaceMultiplicityAxisTests
{
    ...
    private static readonly Dictionary<string, string> IdenticalByConstruction = new(StringComparer.Ordinal)
    {
        ...
        ["MapDerivedType/0"] = "declared Arguments are two typeofs (the polymorphic-hierarchy fixture's "
                               + "derived pair); rotating would need a SECOND derived pair in the fixture",
    ...
    public void The_multiplicity_axis_renders_two_different_applications()
    {
        ...
        {
            if (!string.Equals(c.Axis, "×2", StringComparison.Ordinal)) continue;

            var applications = c.Rendered.Split('\n');
            Assert.True(applications.Length == 2,
                $"{Key(element)}: the ×2 case rendered {applications.Length} application(s), not 2 — the "
                + "axis stopped being the multiplicity axis.");
        ...
        var unexpected = identical
            .Where(line => !IdenticalByConstruction.ContainsKey(line.Split("  ", StringSplitOptions.None)[0]))
            .ToList();
        Assert.True(unexpected.Count == 0,
            "These ×2 cases render two BYTE-IDENTICAL applications, so the multiplicity axis asks nothing "
            + "the single-application axis did not already ask (B37):\n  " + string.Join("\n  ", unexpected)
            + "\nRotate the argument by variant, or pin it in IdenticalByConstruction with the reason it "
            + "cannot be.");
        ...
        var pinnedKeys = identical.Select(line => line.Split("  ", StringSplitOptions.None)[0])
    ...
    public void Variant_one_still_renders_the_declaration_exactly_as_written()
    {
        ...
        foreach (var member in SurfaceCatalog.MemberPlaceholders(claim.Arguments!))
        {
            ...
            var singleApplicationCases = SurfaceCatalog.CasesFor(element)
                .Where(c => c.Axis.StartsWith("ctor(", StringComparison.Ordinal)
                            && c.Axis == $"ctor({claim.ConstructorArity})")
                .ToList();

            foreach (var c in singleApplicationCases.Where(c =>
                         !c.Rendered.Contains(expected, StringComparison.Ordinal)))
                offenders.Add($"{Key(element)} {c.Axis} @{c.Site}: declared {expected}, rendered "
                              + c.Rendered);

            // …and the FIRST application of the ×2 case is variant 1 too.
            foreach (var c in SurfaceCatalog.CasesFor(element)
                         .Where(c => string.Equals(c.Axis, "×2", StringComparison.Ordinal)
                                     && claim.ConstructorArity == FirstCtorArity(element)))
            {
        ...
        Assert.True(offenders.Count == 0,
            "A declared argument list no longer renders as written at variant 1. The rotation belongs to "
            + "the SECOND application only — axes 1 and 2 render at variant 1, so this moves every "
            + "constructor and property cell of the element:\n  " + string.Join("\n  ", offenders));
    }

    private static string Key(SurfaceElement element) =>
        element.UsageName + "/" + element.Type.GetGenericArguments().Length;

    private static int FirstCtorArity(SurfaceElement element) =>
        element.Type.GetConstructors(BindingFlags.Public | BindingFlags.Instance)
    ...
    private static IEnumerable<SurfaceProbeClaim> ClaimsWithArguments(SurfaceElement element) =>
        element.ProbeClaims.Where(c => c.Arguments is not null);
}
