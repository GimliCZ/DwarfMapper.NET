...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    internal static class DeclaredDivergences
    {
        ...
        public static readonly Dictionary<(string Option, Endpoint Endpoint), string> StructurallyInapplicable =
            new()
            {
                ...
                [("RegisterCollectionShapes", Endpoint.UpdateInto)] =
                    "update-into is not ambient-registerable (two arguments, mutates an existing instance), so it " + "has no registration rows for this option to add",
                ...
                [("RegisterCollectionShapes", Endpoint.AsyncStream)] =
                    "an async-stream map is already a sequence map; wrapping it in another collection shape is not " + "a shape any call site asks for",
        ...
        ///     </para>
        /// </summary>
        public static KeyValuePair<string, Divergence>? For(
            string usageName,
            int arity,
            string axis,
            AttributeTargets site,
            SurfaceEndpoints endpoint)
        {
            var matches = Reasons
                .Where(entry => entry.Value.Cells.Any(cell =>
                    string.Equals(cell.UsageName, usageName, StringComparison.Ordinal) && cell.Arity == arity && string.Equals(cell.Axis, axis, StringComparison.Ordinal) && cell.Site == site && (cell.Endpoints & endpoint) != SurfaceEndpoints.None))
                .ToList();
        ...
        {
            return Reasons.Values.Any(d =>
                string.Equals(d.OptionName, optionName, StringComparison.Ordinal) && d.Cells.Any(c => (c.Endpoints & ToSurfaceFlag(endpoint)) != SurfaceEndpoints.None));
        }
