...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    internal static class SurfaceProbe
    {
        ...
        public static (SurfaceEffect Effect, string Detail) Classify(SurfaceCase c, Endpoint endpoint)
        {
            ...
            if (types is not null && !EndpointSources.DeliversFixture(endpoint))
            {
                return (SurfaceEffect.Unasked,
                    $"the '{c.ProbeKey}' fixture never reached this endpoint's source; the {endpoint} template " + "declares its own DTO pair");
            }

            if (c.MapperOptions is not null && !source.Contains(c.MapperOptions, StringComparison.Ordinal))
            {
                return (SurfaceEffect.Unasked,
                    $"the ambient options this case needs ({c.MapperOptions}) never reached this endpoint's " + $"source; the {endpoint} template declares no mapper class to carry them");
            }
        ...
        public static bool PosesNoQuestion(SurfaceCase c, SurfaceEffect effect)
        {
            ...
            return effect is SurfaceEffect.Unasked || (c.Unmeasured is not null && effect is SurfaceEffect.Silent);
        }
            ...
            Baseline(Endpoint endpoint, string? probeKey, string? types, string? options)
        {
            ...
            return Baselines.GetOrAdd(cacheKey,
                _ =>
                {
                    ...
                    var compilerErrorCounts = GeneratorTestHarness.RunAndGetCompilationErrors(baselineSource)
                        .Where(x => x.Severity == DiagnosticSeverity.Error && x.Id.StartsWith("CS", StringComparison.Ordinal))
        ...
        ///     </para>
        /// </summary>
        internal static bool IsGeneratorRefusal(
            IReadOnlyList<string> newCompilerErrorIds,
            IReadOnlyList<Diagnostic> addedDiagnostics)
        {
            ...
            return addedDiagnostics.Any(d => d.Severity == DiagnosticSeverity.Error && !string.Equals(d.Id, "DWARF078", StringComparison.Ordinal)) && newCompilerErrorIds.Count > 0 && newCompilerErrorIds.All(id => AbsentEmissionErrorIds.Contains(id, StringComparer.Ordinal));
        }
        ...
        ///     </para>
        /// </summary>
        internal static IReadOnlyList<string> NewOccurrences(
            IReadOnlyDictionary<string, int> withCounts,
