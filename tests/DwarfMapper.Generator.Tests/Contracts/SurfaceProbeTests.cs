    ...
    public sealed class SurfaceProbeTests
    {
        ...
        public void A_MapIgnore_on_a_create_map_is_observed_as_honoured()
        {
            var element = SurfaceCatalog.Elements.Single(e =>
                string.Equals(e.UsageName, "MapIgnore", StringComparison.Ordinal) && e.Type.GetGenericArguments().Length == 0);
            ...
            Assert.True(effect is SurfaceEffect.Honoured or SurfaceEffect.Refused,
                $"[MapIgnore] at CreateMap classified as {effect} ({detail}). If this reads Silent, the probe is " + "not observing correctly and every cell built on it is vacuous.");
        }
        ...
        public void BuildAt_never_returns_a_source_that_omits_the_rendered_attribute_text()
        {
            ...
            foreach (var site in sites)
            {
                ...
                if (source is null)
                {
                    continue; // NoSuchSite: no cell claimed, nothing to check.
                }
            ...
            Assert.True(offenders.Count == 0,
                "BuildAt returned a non-null source that omits the rendered attribute text for: " + string.Join(", ", offenders) + ". A non-null result claims a cell exists; add the missing slot " + "to the endpoint's template in Endpoints.cs, or return null (NoSuchSite) if there genuinely is " + "no such site.");
        }
        ...
        ///     <c>namespace Demo;</c> (CS0246 otherwise).
        ///     <see cref="BuildAt_never_returns_a_source_that_omits_the_rendered_attribute_text" />
        ...
        public void No_two_declaration_sites_of_an_element_are_measured_as_the_same_source()
        {
            ...
            Assert.Contains(multiSite, x => x.Sites.Contains(AttributeTargets.Property) && x.Sites.Contains(AttributeTargets.Field));
            Assert.Contains(multiSite, x => x.Sites.Contains(AttributeTargets.Class) && x.Sites.Contains(AttributeTargets.Struct));
            ...
            foreach (var endpoint in EndpointSources.All)
            {
                ...
                for (var j = i + 1; j < built.Count; j++)
                {
                    ...
                    {
                        offenders.Add($"{element.UsageName}({c.Axis}) @ {endpoint}: {built[i].Site} and " + $"{built[j].Site} are the same source");
                    }
                }
            }

            Assert.True(compared > 0, "No pair of sites was compared at all — the case space or the site " + "enumeration collapsed and this guard is measuring nothing.");
            Assert.True(offenders.Count == 0,
                "Two declaration sites produce byte-identical source for: " +
        ...
        public void Fixtures_without_a_member_slot_are_counted_not_silently_absent()
        {
            var missing = SurfaceFixtures.All
                .Where(kv => !kv.Value.Contains(EndpointSources.PropertySlotMarker, StringComparison.Ordinal) && !kv.Value.Contains(EndpointSources.FieldSlotMarker, StringComparison.Ordinal))
                .Select(kv => kv.Key)
            ...
            Assert.True(missing.Count == baseline,
                ...
                string.Join(", ", missing) +
                ". Each one means every Property/Field-site case that needs that " +
        ...
        public void NewOccurrences_flags_an_id_the_baseline_already_carries_once_when_the_case_doubles_it()
        {
            ...
            var withCounts = new Dictionary<string, int>
            {
                ["CS8795"] = 2
            };
        ...
        /// <summary>
        ///     The correct-suppression direction, for contrast: an id whose count is unchanged from the
        ///     baseline is not new, however many times it already occurs.
        /// </summary>
        [Fact]
        ...
        public void IsGeneratorRefusal_is_false_when_the_case_introduced_no_compiler_error()
        {
            ...
            {
                Dwarf("DWARF005", DiagnosticSeverity.Error)
            };
        ...
        public void IsGeneratorRefusal_ignores_an_escalated_DWARF078_even_when_unfiltered_by_the_caller()
        {
            var added = new List<Diagnostic>
            {
                Dwarf("DWARF078", DiagnosticSeverity.Error)
            };

            Assert.False(SurfaceProbe.IsGeneratorRefusal(["CS8795"], added));
        }

        private static Diagnostic Dwarf(string id, DiagnosticSeverity severity)
        {
            return Diagnostic.Create(
                new DiagnosticDescriptor(id, id, id, "Dwarf", severity, true),
                Location.None);
        }
        ...
        {
            var element = SurfaceCatalog.CrossProductElements.Single(e => string.Equals(e.UsageName, usageName, StringComparison.Ordinal));
            return SurfaceCatalog.CasesFor(element).Single(x => string.Equals(x.Axis, axis, StringComparison.Ordinal) && x.Site == site);
        }
