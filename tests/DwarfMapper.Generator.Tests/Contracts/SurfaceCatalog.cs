...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    internal static class SurfaceCatalog
    {
        ...
        public static IReadOnlyList<SurfaceCase> CasesFor(SurfaceElement element)
        {
            lock (CaseCache)
            {
                if (CaseCache.TryGetValue(element, out var cached))
                {
                    return cached;
                }
        ...
        private static List<SurfaceElement> Build()
        {
            return typeof(DwarfMapperAttribute).Assembly.GetExportedTypes()
                ...
                .Select(t => (Type: t, Surface: t.GetCustomAttribute<DwarfSurfaceAttribute>(false)))
                .Where(x => x.Surface is not null)
                .Select(x =>
                {
                    var usage = x.Type.GetCustomAttribute<AttributeUsageAttribute>(true);
        ...
        {
            return type.GetCustomAttributes<DwarfSurfaceSiteAttribute>(false)
        ...
        {
            return type.GetCustomAttributes<DwarfSurfaceProbeAttribute>(false)
                .Select(a => new SurfaceProbeClaim(a.Property,
                    a.ConstructorArity,
                    a.ProbeKey,
                    a.Value,
                    a.Arguments,
        ...
        {
            return type.GetCustomAttributes<DwarfSurfaceOptionAttribute>(false)
        ...
        internal static IReadOnlyList<string> ValidateOptionClaims(
            string name,
            IReadOnlySet<string> options,
            SurfaceCategory elementCategory,
            IReadOnlyList<SurfaceOptionClaim> claims)
        {
            ...
            foreach (var claim in claims)
            {
                ...
                {
                    problems.Add($"{name}: {where} names no writable property of the element (it has: " + $"{string.Join(", ", options.OrderBy(o => o, StringComparer.Ordinal))}), so it " + "redirects nothing. The option it was written for still carries the element's " + "obligation, and nothing says so.");
                }
                else if (!seen.Add(claim.Option))
                {
                    problems.Add($"{name}: two [DwarfSurfaceOption] claims both redirect '{claim.Option}'. One is " + "never consulted, and which one depends on declaration order.");
                }

                if (string.IsNullOrWhiteSpace(claim.Because))
                {
                    problems.Add($"{name}: {where} states no reason. An unexplained redirect is an allowlist " + "entry with a category name on it.");
                }

                if (claim.Category == elementCategory)
                {
                    problems.Add($"{name}: {where} restates the element's own category ({elementCategory}). It " + "redirects nothing while reading as a reviewed decision — delete it, or name " + "the category whose obligation this option actually satisfies.");
                }
            }
        ...
        {
            return element.ProbeClaims.FirstOrDefault(p => string.Equals(p.Property, property, StringComparison.Ordinal));
        }
        ...
        {
            return element.ProbeClaims.FirstOrDefault(p => p.Property is null && p.ConstructorArity == arity);
        }
        ...
        internal static IReadOnlyList<string> ValidateSiteClaims(
            string name,
            AttributeTargets validOn,
            SurfaceEndpoints defaultAppliesTo,
            IReadOnlyList<SurfaceSiteClaim> claims)
        {
            ArgumentNullException.ThrowIfNull(claims);
            var problems = new List<string>();
...
#pragma warning disable MA0099
            ...
            foreach (var claim in claims)
            {
                if (claim.Site == 0)
                {
                    problems.Add($"{name}: a [DwarfSurfaceSite] names no site at all, so it narrows nothing and " + "reads as a reviewed decision.");
                }

                var illegal = claim.Site & ~validOn;
                if (illegal != 0)
                {
                    problems.Add($"{name}: [DwarfSurfaceSite({claim.Site})] names {illegal}, which " + $"AttributeUsage does not permit (ValidOn = {validOn}). The element cannot be " + "written there at all, so the narrowing applies to nothing — this is how a " + "claim quietly stops applying after a site is removed from AttributeUsage.");
                }

                var overlap = claim.Site & seen;
                if (overlap != 0)
                {
                    problems.Add($"{name}: two [DwarfSurfaceSite] claims both cover {overlap}. One of them would " + "never be consulted, and which one depends on declaration order.");
                }
...
#pragma warning restore MA0099
                ...
                if (string.IsNullOrWhiteSpace(claim.Because))
                {
                    problems.Add($"{name}: [DwarfSurfaceSite({claim.Site})] states no reason. The reason must be " + "about the SHAPE of the site; 'the generator does not read it there' is a " + "divergence to report, not a claim to encode.");
                }

                if (claim.AppliesTo == defaultAppliesTo)
                {
                    problems.Add($"{name}: [DwarfSurfaceSite({claim.Site})] restates the element's own default " + $"({defaultAppliesTo}). It narrows nothing while reading as a reviewed " + "decision — delete it, or narrow it.");
                }
            }
        ...
        /// </param>
        internal static IReadOnlyList<string> ValidateProbeClaims(
            string name,
            IReadOnlyDictionary<string, int> domainSizes,
            IReadOnlyList<int> arities,
            string? elementProbeKey,
            ...
            Func<string?, (IReadOnlySet<string> Members, IReadOnlySet<string> Types)?> fixtureNames)
        {
            ...
            foreach (var claim in claims)
            {
                ...
                if (claim.Property is not null)
                {
                    ...
                    {
                        problems.Add($"{name}: {where} names no readable/writable non-indexed property of the " + "element, so it governs no case at all. Every cell it was written for is " + "still probed against a shape that cannot ask it anything.");
                    }
                    else if (!seenProperties.Add(claim.Property))
                    {
                        problems.Add($"{name}: two [DwarfSurfaceProbe] claims both refine '{claim.Property}'. One " + "of them is never consulted, and which one depends on declaration order.");
                    }

                    if (claim.Arguments is not null)
                    {
                        problems.Add($"{name}: {where} states Arguments, which only a CONSTRUCTOR case has. A " + "property case is rendered as a named argument, so this value goes nowhere.");
                    }

                    if (claim.Value is not null && domainSizes.TryGetValue(claim.Property, out var size) && size != 1)
                    {
                        problems.Add($"{name}: {where} states a Value, but the derived domain of " + $"'{claim.Property}' has {size} members. Replacing a multi-member domain " + "with one value drops the others silently — and a domain whose second and " + "third members were never probed reads exactly like a covered one.");
                    }
                }
                else
                {
                    ...
                    {
                        problems.Add($"{name}: {where} names a constructor arity the element does not declare " + $"(it has {string.Join(", ", arities.OrderBy(a => a))}), so it governs no " + "case. This is how a refinement quietly stops applying after an overload " + "is added or removed.");
                    }
                    else if (!seenArities.Add(claim.ConstructorArity))
                    {
                        problems.Add($"{name}: two [DwarfSurfaceProbe] claims both refine the " + $"{claim.ConstructorArity}-argument constructor. One is never consulted.");
                    }
                ...
                if (claim.ProbeKey is null && claim.Value is null && claim.Arguments is null && claim.MapperOptions is null && claim.Unmeasured is null)
                {
                    problems.Add($"{name}: {where} states neither a ProbeKey, a Value, an argument list, mapper " + "options nor an Unmeasured reason. It refines nothing while reading as a " + "reviewed decision.");
                }

                if (claim.ProbeKey is not null && string.Equals(claim.ProbeKey, elementProbeKey, StringComparison.Ordinal))
                {
                    problems.Add($"{name}: {where} restates the element's own ProbeKey ('{elementProbeKey}'). It " + "refines nothing — delete it, or point it at the shape this case needs.");
                }

                problems.AddRange(UnmeasuredProblems(name, where, claim, domainSizes.Count));
                problems.AddRange(ArgumentProblems(name,
                    where,
                    claim,
        ...
        ///     </para>
        /// </summary>
        private static IEnumerable<string> UnmeasuredProblems(
            string name,
            string where,
            SurfaceProbeClaim claim,
            int writablePropertyCount)
        {
            if (claim.Unmeasured is null)
            {
                yield break;
            }
            ...
            {
                yield return $"{name}: {where} declares a case unmeasurable that is not the zero-argument " + "constructor. Only a case that renders with NO arguments configures nothing by " + "construction; anywhere else this would excuse a cell the generator actually had " + "a decision to make, which is a divergence to report rather than a hole to declare.";
            }

            if (writablePropertyCount == 0)
            {
                yield return $"{name}: {where} declares the bare case unmeasurable, but the element has no " + "writable properties — so the bare form IS the element, and excusing it excuses " + "every question this element can pose.";
            }

            if (claim.ProbeKey is not null || claim.Value is not null || claim.Arguments is not null || claim.MapperOptions is not null)
            {
                yield return $"{name}: {where} is both unmeasurable and given a shape to measure it with. One of " + "the two is wrong.";
            }
        }
        ...
        /// </summary>
        private static IEnumerable<string> ArgumentProblems(
            string name,
            string where,
            SurfaceProbeClaim claim,
            (IReadOnlySet<string> Members, IReadOnlySet<string> Types)? fixture)
        {
            if (claim.Arguments is null || fixture is not { } f)
            {
                yield break;
            }
            ...
            foreach (var type in TypeReferences(claim.Arguments).Where(t => !f.Types.Contains(t)))
                yield return $"{name}: {where} names type '{type}', which the fixture it is measured against does " + $"not declare (it has: {string.Join(", ", f.Types.OrderBy(x => x, StringComparer.Ordinal))}).";
        }
        ...
        internal static (IReadOnlySet<string> Members, IReadOnlySet<string> Types)? FixtureNames(string? probeKey)
        {
            if (probeKey is null)
            {
                return SurfaceFixtures.DeclaredNames(EndpointSources.DefaultTypes);
            }
        ...
        {
            return Enum.GetValues<AttributeTargets>()
        ...
        private static List<SurfaceCase> BuildCases(SurfaceElement element)
        {
            ...
            foreach (var site in sites)
            {
                ...
                foreach (var ctor in ctors)
                {
                    ...
                    cases.Add(new SurfaceCase(element,
                        site,
                        Render(element, ArgumentsFor(element, ctor, claim, 1), ""),
                        $"ctor({arity})",
                        claim?.ProbeKey ?? element.ProbeKey,
                        claim?.Unmeasured,
                        claim?.MapperOptions));
                }
                ...
                foreach (var p in WritablePropertiesOf(element.Type))
                {
                    ...
                    var baseArgs = shortest is null
                        ? ""
                        : ArgumentsFor(element,
                            shortest,
                            ProbeFor(element, shortest.GetParameters().Length),
                            1);

                    foreach (var value in ValueDomainFor(element, p, claim))
                        cases.Add(new SurfaceCase(element,
                            site,
                            Render(element, baseArgs, $"{p.Name} = {value}"),
                            $"{p.Name}={value}",
                            claim?.ProbeKey ?? element.ProbeKey,
                            claim?.Unmeasured,
                            claim?.MapperOptions));
                ...
                if (element.AllowMultiple && ctors.Length > 0)
                {
                    ...
                    var one = Render(element, ArgumentsFor(element, ctors[0], claim, 1), "");
                    var two = Render(element, ArgumentsFor(element, ctors[0], claim, 2), "");
                    cases.Add(new SurfaceCase(element,
                        site,
                        one + "\n" + two,
                        "×2",
        ...
        private static string ArgumentsFor(
            ...
            int variant)
        {
            return claim?.Arguments is { } declared
        ...
        internal static string ExpandArguments(string declared)
        {
            ...
            return Regex.Replace(declared, @"\{(\w+)\}", m => $"\"{m.Groups[1].Value}\"");
        }
        ...
        /// </summary>
        private static IEnumerable<string> ValueDomainFor(
            SurfaceElement element,
            PropertyInfo p,
            SurfaceProbeClaim? claim)
        {
            return claim?.Value is { } stated ? [stated] : ValueDomain(p, element.Type);
        }
        ...
        private static IEnumerable<string> ValueDomain(PropertyInfo p, Type declaring)
        {
            ...
            if (t == typeof(bool))
            {
                ...
                {
                    "true", "false"
                }.Where(v => defRendered is null || v != defRendered);
            }
            ...
            if (t == typeof(int))
            {
                return [def is int i ? (i + 1).ToString(CultureInfo.InvariantCulture) : "1"];
            }
            ...
            throw new InvalidOperationException(
                $"No value domain for {declaring.Name}.{p.Name} of type {t.Name}. Add one rather than letting the " + "property silently fall out of the matrix — an unprobed property is exactly the hole this " + "catalogue exists to close.");
        }
        ...
        private static string SampleArgument(ParameterInfo p, int variant, int position)
        {
            ...
            if (t.IsEnum)
            {
                return $"{t.Name}.{Enum.GetNames(t)[0]}";
            }
            ...
            {
                return variant == 1 ? "typeof(Dst)" : "typeof(Src)";
            }

            throw new InvalidOperationException(
                $"No sample argument for parameter '{p.Name}' of type {t.Name}. Add one; skipping it would drop " + "the constructor overload from the matrix without saying so.");
        }
