...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    public class EndpointContractTests
    {
        ...
        {
            return from usage in UsageNames
        ...
        public void Every_cell_has_a_declared_expectation(string usage, Endpoint endpoint)
        {
            ...
            Assert.True(cell is not null,
                $"'{usage}' has no declared expectation at {endpoint}. A new public attribute must be classified " + "in EndpointContractMatrix (an explicit cell, or an entry in Uniform) before it can ship — " + "otherwise it could reach one endpoint and not another with nothing to notice.");
            ...
            switch (cell.Status)
            {
                case CellStatus.Refused:
                    Assert.False(string.IsNullOrWhiteSpace(cell.DiagnosticId),
                        $"{usage} x {endpoint} is Refused but names no diagnostic — 'refused' without an error " + "shape is indistinguishable from silently ignored, which is the defect class this " + "matrix exists to make unrepresentable.");
                    break;

                case CellStatus.NotApplicable:
                    Assert.False(string.IsNullOrWhiteSpace(cell.Reason),
                        $"{usage} x {endpoint} is NotApplicable with no reason. An unexplained exemption is how a " + "real gap hides.");
                    break;
            }
        }
        ...
        {
            return EndpointSources.All.Select(e => new object[]
            {
                e
            });
        }
        ...
        {
            return EndpointContractMatrix.ModifierCells.Select(c =>
                new object[]
                {
                    c.Modifier, c.Syntax, c.Endpoint, c.Status, c.Id!
                });
        }
    }
}
