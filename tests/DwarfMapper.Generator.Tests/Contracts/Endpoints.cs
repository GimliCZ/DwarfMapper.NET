...
namespace DwarfMapper.Generator.Tests.Contracts
{
    ...
    public static class EndpointSources
    {
        ...
        {
            return Slots.TryGetValue(site, out var slot)
                ? slot.Marker
        ...
        private static bool IsDwarfMapperAttribute(string rendered)
        {
            ...
            return string.Equals(text, "[DwarfMapper]", StringComparison.Ordinal) || text.StartsWith("[DwarfMapper(", StringComparison.Ordinal);
        }
        ...
        public static string? SiteAbsenceReason(Endpoint endpoint, AttributeTargets site, string? types = null)
        {
            if (Slots.TryGetValue(site, out var slot))
            {
                return Build(endpoint, types: types).Contains(slot.Marker, StringComparison.Ordinal)
                    ? null
                    : $"{slot.Cause}: neither the fixture in play nor the endpoint template carries a " + slot.Marker;
            }
        ...
        {
            return endpoint is not (Endpoint.Registry or Endpoint.CoLocatedHost);
        }
            ...
            string? options = null)
        {
            ...
            if (SiteAbsenceReason(endpoint, site, types) is not null)
            {
                return null;
            }
            ...
            if (Slots.ContainsKey(site))
            {
                return SpliceAtSlot(endpoint, site, rendered, types, options);
            }
            ...
            return site switch
            {
                ...
                AttributeTargets.Method
                    => Build(endpoint, rendered, types: types, options: options ?? ""),
                AttributeTargets.Assembly
                    => InsertAssemblyAttribute(Build(endpoint, types: types, options: options ?? ""), rendered),
                _ => throw new InvalidOperationException(
                    $"{site} at {endpoint} has no absence reason and no template. SiteAbsenceReason and BuildAt " + "must agree on which sites exist; one of them gained a case the other did not.")
            };
        }
        ...
        ///     </para>
        /// </summary>
        private static string SpliceAtSlot(
            Endpoint endpoint,
            AttributeTargets site,
            string rendered,
            string? types,
            string? options)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"SpliceAtSlot reached a {endpoint} source with no {marker} for the {site} site; " + "SiteAbsenceReason should have reported the missing slot and BuildAt should have returned " + "null.");
            }
