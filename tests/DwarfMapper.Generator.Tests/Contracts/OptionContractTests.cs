    ...
    public class OptionContractTests
    {
        ...
        [
            ...
            new("RequiredMapping",
                CellStatus.Refused,
                "DWARF039",
                "source-side completeness: an unconsumed source member is reported here exactly as it is at the " + "create and update endpoints"),
            ...
            new("GenerateExtensions",
                CellStatus.NotApplicable,
                null,
                "projection emits no convenience extension in the first place (verified: no static class appears " + "in the output with or without the option), so there is nothing for it to suppress"),
            ...
            new("EnumStringSource",
                CellStatus.Refused,
                "DWARF028",
                "measured, not assumed: enum<->string mapping is a generated switch, which projection refuses " + "outright with DWARF028 before any string-source policy is consulted. The member is rejected, " + "so the option cannot be silently dropped here — the failure mode this matrix exists for"),
        ...
        {
            var scanned = OptionCatalog.Options.Single(c => string.Equals(c.Name, option, StringComparison.Ordinal) && string.Equals(c.ValueLabel, value, StringComparison.Ordinal));
            var cell = ProjectionCells.SingleOrDefault(c => string.Equals(c.Option, option, StringComparison.Ordinal));

            Assert.True(cell is not null,
                $"[DwarfMapper] option '{option}' exists on the attribute but has no declared projection " + "contract. Add a ProjectionCells row saying whether projection honours it, refuses it, or " + "cannot observe it.");
            ...
            switch (cell.Status)
            {
                case CellStatus.Refused:
                    Assert.True(
                        ...
                        $"[DwarfMapper({scanned.NonDefault})] on a Project method must report {cell.DiagnosticId} " + $"({cell.Reason}), but reported: {Describe(diagnostics)}. A projection that accepts the " + "option and quietly ignores it produces silently wrong data — this is the exact shape of " + "the four divergences this matrix was built for.");
                    break;

                case CellStatus.Honoured:
                    Assert.True(
                        !diagnostics.Any(d => d.Severity == DiagnosticSeverity.Error),
                        $"[DwarfMapper({scanned.NonDefault})] is declared Honoured at projection but errored: " + Describe(diagnostics));
                    ...
                    Assert.True(
                        ...
                        $"[DwarfMapper({scanned.NonDefault})] is declared Honoured at projection, but the generated " + "output is byte-identical to the same source without it. Either the option is silently " + $"dropped ({cell.Reason} says it should not be), or the fixture does not trigger it — " + "and those must not be indistinguishable.");
                    break;
        ...
        public void The_NotApplicable_excuse_class_is_a_counted_population()
        {
            ...
            Assert.True(excused.Count >= NotApplicablePin,
                $"Only {excused.Count} ProjectionCells rows are excused as NotApplicable, under the pinned " + $"{NotApplicablePin}. A row was measured into Honoured or Refused — good — but the pin must " + "move in the same commit, or the room it freed silently funds a replacement excuse.");
        }
        ...
        public void Every_class_level_option_has_a_declared_projection_contract()
        {
            ...
            Assert.True(missing.Count == 0,
                "Class-level [DwarfMapper] option(s) with no declared projection contract:\n" + string.Join("\n", missing) + "\n\nAdd a row to ProjectionCells saying whether projection honours it, refuses it (with the " + "diagnostic id), or genuinely cannot observe it (with the reason).");
        ...
        public void Every_non_default_member_of_every_option_enum_is_probed_by_the_matrix()
        {
                         ...
                         .Where(p => p.CanWrite && p.CanRead && p.GetIndexParameters().Length == 0))
            {
                ...
                {
                    if (string.Equals(member, def?.ToString(), StringComparison.Ordinal))
                    {
                        continue;
                    }
            ...
            Assert.True(missing.Count == 0,
                "Enum option member(s) never probed by any matrix cell: " + string.Join(", ", missing) + ". Every member is a distinct behaviour; probing one alternative and calling the option covered " + "is how a member ships with no test at all.");
        }
