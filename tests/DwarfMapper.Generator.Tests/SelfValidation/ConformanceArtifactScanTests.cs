    ...
    public class ConformanceArtifactScanTests
    {
        ...
        public void The_README_assertion_figure_matches_the_sample()
        {
            ...
            Assert.True(quoted.Count > 0,
                "README no longer states a runtime-assertion figure. If the claim was removed deliberately, " + "remove this test with it; otherwise the sample's size is now unadvertised.");

            Assert.All(quoted,
                n => Assert.True(n == actual,
                    $"README advertises {n} runtime assertions but the conformance sample contains {actual}. " + "The figure must be derived from the sample, not maintained by hand — it was wrong by one " + "until this test existed."));
        }
        ...
        public void Every_conformance_artifact_is_well_formed()
        {
            ...
            {
                return; // nothing published yet — the gate creates it on first run
            }
            ...
            foreach (var artifact in artifacts)
            {
                ...
                Assert.True(Regex.IsMatch(name, @"^\d{4}-\d{2}-\d{2}-[a-z0-9-]+\.md$"),
                    $"Artifact '{name}' is not named <date>-<rid>.md — the gate sorts by name to find the " + "previous run, so an off-pattern name breaks the ratchet.");

                Assert.True(Regex.IsMatch(text, @"^- \*\*Assertions:\*\* \d+\s*$", RegexOptions.Multiline),
                    $"Artifact '{name}' has no parseable assertion count; conformance-gate.sh reads exactly this " + "line to enforce that coverage only grows.");

                Assert.True(text.Contains("- **Commit:**", StringComparison.Ordinal),
                    $"Artifact '{name}' records no commit — the gate compares it against the build commit so a " + "stale artifact cannot stand in for a run that did not happen.");
            }
        }
        ...
        public void The_latest_artifact_agrees_with_the_sample()
        {
            ...
            Assert.Equal(
                ...
                int.Parse(recorded.Groups["n"].Value, CultureInfo.InvariantCulture));
        }
