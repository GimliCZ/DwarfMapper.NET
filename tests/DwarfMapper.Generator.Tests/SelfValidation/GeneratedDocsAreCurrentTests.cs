...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class GeneratedDocsAreCurrentTests
    {
        ...
        public void The_api_reference_actually_carries_summaries()
        {
            ...
            var documented = rows.Count(l => l.TrimEnd().EndsWith('|') && l.Split('|').Length > 3 && !string.IsNullOrWhiteSpace(l.Split('|')[^2]));

            Assert.True(rows.Count >= 20, $"API reference rendered only {rows.Count} member rows.");
            Assert.True(documented >= rows.Count / 2,
                $"Only {documented} of {rows.Count} rendered members carry a summary — the XML lookup is likely " + "broken rather than the code being undocumented.");
        }
        ...
        private static void AssertCurrent(string relativePath, string expected)
        {
            ...
            Assert.Fail(written
                ? $"{relativePath} was out of date and has been regenerated in your working tree.\n" + "Review the diff and commit it. This test fails rather than healing quietly on purpose: a " + "healing doc test goes green in CI while the file people actually read stays stale."
        ...
        private static string Describe(OptionEffect effect, string detail)
        {
        ...
        private static List<(string Option, Endpoint Endpoint)> SilentCells()
        {
            ...
            foreach (var line in RenderOptionMatrix().Split('\n'))
            {
                ...
                if (cells.Length != MatrixEndpoints.Length + 3)
                {
                    continue;
                }
        ...
        public void Every_known_gap_is_still_a_real_gap()
        {
            ...
            Assert.True(fixedOnes.Count == 0,
                "DeclaredDivergences.Reasons names option cell(s) that are no longer silent — remove them so " + "the ratchet tightens: " + string.Join(", ", fixedOnes));
        }
