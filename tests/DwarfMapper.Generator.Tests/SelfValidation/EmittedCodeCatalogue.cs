...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    internal static class EmittedCodeCatalogue
    {
            ...
            foreach (var (ordinal, id) in Published)
            {
                if (!byOrdinal.TryGetValue(ordinal, out var example))
                {
                    throw new DocToolingException(
                        $"Emitted-code catalogue publishes example {ordinal}, which no longer exists. Update " + $"EmittedCodeCatalogue.Published or restore the example.");
                }
        ...
        private static string Emit(string relativeFile)
        {
            ...
            {
                throw new DocToolingException(
                    $"{relativeFile}: the generator reported errors while producing published output, so the " + "document would show code from a failed compilation:\n  " + string.Join("\n  ", errors));
            }

            if (string.IsNullOrWhiteSpace(generated))
            {
                throw new DocToolingException(
                    $"{relativeFile}: the generator produced no output. Publishing an empty block would read as " + "\"this mapping needs no code\".");
            }
