    ...
    public class BenchmarkCoverageSelfValidationTests
    {
        ...
        private static List<string> DepthOneShapes()
        {
            ...
            Assert.True(shapes.Count >= 20,
                $"Parsed only {shapes.Count} shapes from CombinatorialSchema.cs; the extraction has broken and this " + "ratchet would pass while checking nothing.");
            return shapes;
        }
        ...
        public void Every_combinatorial_shape_is_benchmarked_or_explicitly_exempted()
        {
            ...
            Assert.True(undeclared.Count == 0,
                "These shapes are crossed by the combinatorial suite but are neither benchmarked nor exempted:\n  " + string.Join("\n  ", undeclared) + "\nAdd each to ShapeToBenchmarkCategory: a category name if a benchmark measures it, or null with " + "a reason. Silence here would mean nobody ever decided.");
        }
        ...
        public void Benchmark_shape_coverage_does_not_silently_regress()
        {
            ...
            Assert.True(covered >= 9,
                $"Benchmark shape coverage dropped to {covered} shapes; it was 9. A benchmark was deleted or a " + "category renamed — restore it rather than lowering this floor.");
        }
    }
}
