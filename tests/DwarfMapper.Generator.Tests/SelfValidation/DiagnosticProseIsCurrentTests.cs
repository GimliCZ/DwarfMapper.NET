...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class DiagnosticProseIsCurrentTests
    {
        ...
        {
            ["DWARF038"] = "severity is computed per conversion (lossy -> Warning, otherwise Info, escalating to " + "Error under ImplicitConversions = false), so a flat DefaultSeverity understates it"
        };
        ...
        {
            return DocSet.Read("docs/diagnostics.md").Replace("\r\n", "\n", StringComparison.Ordinal).Split('\n');
        }
        ...
        public void Every_section_restates_its_diagnostics_real_title_and_severity()
        {
            ...
            Assert.True(checkedSections >= 60,
                $"Only {checkedSections} diagnostic sections were checked — the section-header pattern has " + "probably changed, which would make this test vacuous rather than passing.");

            Assert.True(wrong.Count == 0,
                "docs/diagnostics.md restates a title or severity the descriptors no longer agree with. The code " + "is the truth; update the prose:\n  " + string.Join("\n  ", wrong));
        }
        ...
        public void Every_richer_severity_entry_is_still_needed()
        {
            ...
            foreach (var id in RicherSeverityProse.Keys)
            {
                ...
                if (byId.TryGetValue(id, out var d) && string.Equals(proseSeverity, d.DefaultSeverity.ToString(), StringComparison.Ordinal))
                {
                    unnecessary.Add(id);
                }
            }
            ...
            Assert.True(unnecessary.Count == 0,
                "RicherSeverityProse names id(s) whose prose now states the plain severity — remove them so the " + "check tightens: " + string.Join(", ", unnecessary));
        }
        ...
        public void Every_diagnostic_the_generator_can_report_has_a_prose_section()
        {
            ...
            Assert.True(undocumented.Count == 0,
                "Diagnostic(s) the generator can report with no '## dwarfNNN' section in docs/diagnostics.md. A " + "reader who hits one has nothing to read:\n  " + string.Join("\n  ", undocumented));
        }
    }
}
