...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public sealed class SurfaceObligationTests
    {
        ...
        public void Every_category_carries_an_obligation()
        {
            ...
            {
                SurfaceCategory.ConsumerDirective, SurfaceCategory.GeneratorEmitted, SurfaceCategory.BuildFailureOnly, SurfaceCategory.EmissionShape, SurfaceCategory.CrossAssembly, SurfaceCategory.TestingOnly
            };

            var unclaimed = Enum.GetValues<SurfaceCategory>().Except(withObligations).ToList();

            Assert.True(unclaimed.Count == 0,
                "SurfaceCategory member(s) with no obligation in this file:\n  " + string.Join("\n  ", unclaimed) + "\n\nA category with no obligation is an allowlist with an enum member's name on it: an element " + "assigned to it is asked for nothing, and nothing says so. Add the theory, then add the member " + "to the list above.");
        ...
        public static TheoryData<string> ConsumerDirectives()
        {
            return Of(SurfaceCategory.ConsumerDirective);
        }

        public static TheoryData<string> GeneratorEmitted()
        {
            return Of(SurfaceCategory.GeneratorEmitted);
        }

        public static TheoryData<string> BuildFailureOnly()
        {
            return Of(SurfaceCategory.BuildFailureOnly);
        }

        public static TheoryData<string> EmissionShapes()
        {
            return Of(SurfaceCategory.EmissionShape);
        }

        public static TheoryData<string> CrossAssembly()
        {
            return Of(SurfaceCategory.CrossAssembly);
        }

        public static TheoryData<string> TestingOnly()
        {
            return Of(SurfaceCategory.TestingOnly);
        }
        ...
        {
            Assert.True(IsWritten(ConsumerCorpus, usageName),
                $"[{usageName}] is TestingOnly — part of the surface consumers use to write THEIR tests — but no " + "consumer-shaped assembly writes it. A testing helper nobody outside the package has used is a " + "helper whose ergonomics have never been measured.");
            ...
            Assert.True(IsWritten(TestingContractCorpus, usageName),
                $"[{usageName}] is TestingOnly but tests/DwarfMapper.Testing.Tests never writes it. The " + "testing surface needs its own contract row: it ships to consumers, so a change to it is a " + "breaking change to them.");
        }
        ...
        public void A_class_level_option_satisfies_the_obligation_of_its_declared_category(string option)
        {
            ...
            Assert.True(IsAssigned(corpus, option),
                ...
                "declaration with [DwarfSurfaceOption(nameof(DwarfMapperAttribute." +
                option +
                "), " +
        ...
        public void The_option_scan_is_not_vacuous()
        {
            Assert.True(OptionNames.Count >= 15,
                $"Only {OptionNames.Count} settable options found on [DwarfMapper]. If the surface really shrank, " + "lower this floor deliberately; otherwise the reflection stopped seeing the options and this " + "gate has gone vacuous.");
        ...
        private static (string Corpus, string Name, string Remedy) CorpusFor(SurfaceCategory category)
        {
            return category switch
            {
                SurfaceCategory.ConsumerDirective => (SampleCorpus, "a runnable sample under samples/",
                    "Add a Conformance feature asserting the option's observable runtime difference, or a " + "Gallery example if it deserves prose."),
                ...
                _ => throw new InvalidOperationException(
                    $"SurfaceCategory.{category} has no option-level obligation. Add an arm naming the corpus an " + "option redirected to this category must be written in. Defaulting here would scan the " + "option against a corpus nobody chose for it, which is the allowlist this file deleted, " + "wearing an enum member's name.")
            };
        }
        ...
        {
            return Regex.IsMatch(corpus,
                @"\[\s*(assembly\s*:\s*)?(global\s*::\s*)?(DwarfMapper\s*\.\s*)?" + Regex.Escape(usageName) + @"\s*[\]\(<]",
                RegexOptions.None,
        ...
        {
            return Regex.IsMatch(StripComments(corpus),
