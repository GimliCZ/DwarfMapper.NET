...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class GeneratorTestingScanTests
    {
        ...
        //
        // Baselines: 36 public attribute types (35 -> 36 in round 29 T3.1, [MapShare]; the golden feature case
        // "MapShare" was added in the same commit, which is what this ratchet asks for), 16 public enum
        ...
        {
            "ParsableConverter.cs", "NumericConverter.cs", "UserConversionConverter.cs"
        };
        ...
        public void Every_generator_in_src_is_registered_for_testing()
        {
            var declared = typeof(DwarfGenerator).Assembly.GetTypes()
                .Where(t => t is { IsAbstract: false, IsInterface: false } && typeof(IIncrementalGenerator).IsAssignableFrom(t))
                .Select(t => t.Name)
        ...
        public void Every_tracking_name_constant_is_registered_in_the_battery()
        {
            ...
            foreach (var assemblyType in generatorTypes)
            {
                ...
                Assert.True(registered is not null,
                    $"'{assemblyType.Name}' has no entry in GeneratorRegistry.All — add it there, or the " + "battery never asserts any of its step names are cacheable.");

                var declared = assemblyType
                    .GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)
                    .Where(f => f.IsLiteral && f.FieldType == typeof(string) && f.Name.EndsWith("StepName", StringComparison.Ordinal))
                    .Select(f => (string)f.GetRawConstantValue()!)
                    .ToList();

                foreach (var name in declared)
                    Assert.True(registered.TrackingNames.Contains(name),
                        $"{assemblyType.Name} declares step name '{name}' but AllStepNames does not include it, so " + "the battery never asserts that step is cacheable.");
            }
        }
        ...
        public void The_golden_corpus_covers_every_registered_generator()
        {
            ...
            foreach (var g in GeneratorRegistry.All)
                Assert.True(covered.Contains(g.Name, StringComparer.Ordinal),
                    $"Generator '{g.Name}' contributes no golden cases, so a refactor could change its output " + "undetected. Add feature cases for it to GoldenCorpus.FeatureCases().");
        }
        ...
        public void The_feature_taxonomy_has_not_grown_past_its_recorded_baseline()
        {
            ...
            Assert.True(attributeTypeCount == BaselineAttributeTypeCount,
                $"{runtimeAssembly.GetName().Name} now has {attributeTypeCount} public attribute types; the " + $"recorded baseline is {BaselineAttributeTypeCount}. A taxonomy grew, so consider adding a " + "golden feature case, then update the baseline deliberately.");
            ...
            Assert.True(enumValueCount == BaselineEnumValueCount,
                $"{runtimeAssembly.GetName().Name} now has {enumValueCount} public enum values across its public " + $"enums; the recorded baseline is {BaselineEnumValueCount}. A taxonomy grew, so consider adding " + "a golden feature case, then update the baseline deliberately.");
        }
        ...
        {
            return Directory.EnumerateFiles(Path.Combine(RepoRoot(), GeneratorSrcRoot, "Pipeline"),
                "MapperExtractor*.cs",
        ...
        {
            return Directory.EnumerateFiles(Path.Combine(RepoRoot(), GeneratorSrcRoot, "Registry"),
                "*.cs",
                SearchOption.TopDirectoryOnly);
        }
        ...
        {
            return Directory.EnumerateFiles(Path.Combine(RepoRoot(), GeneratorSrcRoot), "*.cs", SearchOption.AllDirectories)
        ...
        public void MapToGenerator_does_not_call_GetMembers_directly()
        {
            ...
            foreach (var path in RegistryFiles())
            {
                ...
                Assert.False(text.Contains("GetMembers()", StringComparison.Ordinal),
                    $"{Path.GetFileName(path)} calls GetMembers() directly again. Member enumeration must go " + "through Core.MemberFacts, or the registry silently loses inherited members as it did before.");
            }
        }
        ...
        public void Neither_engine_declares_its_own_FNV_constant()
        {
            ...
            foreach (var path in AllGeneratorFiles())
            {
                ...
                Assert.False(text.Contains("2166136261", StringComparison.Ordinal),
                    $"{Path.GetFileName(path)} contains the FNV-1a offset basis constant. Hashing belongs solely to " + "Core.StableHash — ten copies of it is what ISSUE-015 was.");
            }
        }
        ...
        public void Both_engines_call_MemberFacts_Readable_and_Writable()
        {
            ...
            foreach (var (engineName, files) in new[]
                     {
                         ("[DwarfMapper] class model", ClassModelFiles()), ("[MapTo] registry", RegistryFiles())
                     })
            {
                var combined = string.Join("\n", files.Select(File.ReadAllText));

                Assert.True(combined.Contains("MemberFacts.Readable(", StringComparison.Ordinal),
                    $"The {engineName} no longer calls MemberFacts.Readable(...) — member enumeration may have been " + "re-implemented locally instead of routed through the shared core.");
                Assert.True(combined.Contains("MemberFacts.Writable(", StringComparison.Ordinal),
                    $"The {engineName} no longer calls MemberFacts.Writable(...) — member enumeration may have been " + "re-implemented locally instead of routed through the shared core.");
            }
        }
        ...
        private static IEnumerable<string> MigratedEmitterFiles()
        {
            var root = RepoRoot();
            return Directory
                .EnumerateFiles(Path.Combine(root, GeneratorSrcRoot, "Pipeline"),
                    "*Converter.cs",
                    SearchOption.TopDirectoryOnly)
                .Concat(Directory.EnumerateFiles(Path.Combine(root, GeneratorSrcRoot, "Registry"),
                    "*.cs",
                    SearchOption.TopDirectoryOnly))
        ...
        public void Migrated_emitters_do_not_reintroduce_hand_rolled_emission()
        {
            ...
            foreach (var path in MigratedEmitterFiles())
            {
                ...
                var newlineEscapes = Regex.Count(text, NewlineEscapePattern);
                if (newlineEscapes > 0)
                {
                    offenders.Add($"{name}: {newlineEscapes} literal \\n escape(s)");
                }

                var indentRuns = Regex.Count(text, IndentRunPattern);
                if (indentRuns > 0)
                {
                    offenders.Add($"{name}: {indentRuns} hard-coded indent run(s)");
                }
            }
            ...
            Assert.True(scanned >= 5,
                $"The migrated-emitter glob matched only {scanned} file(s) — it is broken or the files moved, and a " + "ratchet that scans nothing is worse than none. Fix the glob, do not lower this floor.");

            Assert.True(offenders.Count == 0,
                "Migrated emitter(s) reintroduced hand-rolled emission:\n  " + string.Join("\n  ", offenders) + "\nUse CodeWriter (Line/Block/Indent) — hand-counted indentation and \\n escapes are what this " + "migration removed.");
        }
    }
}
