...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class FixtureAdoptionScanTests
    {
        ...
        // Raised from 52 to 53 for WrapperMapExpansionReachTests.The_message_names_the_DWARF094_edge_and_that_edge_is_real
        // (DWARF093): the message tells a caller that adding [GenerateMap<A, B>] beside a `partial B Map(A)` over
        // the same pair collides, and that sentence has to be measured rather than believed. The assertion is on
        // ONE specific id (originally that CS0111 is PRESENT; since B27 closed, that it is ABSENT behind the
        // DWARF094 refusal) — a specific-id assertion the fixture deliberately does not model.
        // Raised from 53 to 55 for DuplicateGenerateMapSignatureTests (DWARF094, B27): the two refusal tests
        // assert that CS0111 SPECIFICALLY never reaches the consumer out of generated code — the same shape as
        // DWARF060's test — once for the mapper class and once for the mode-2 co-located host. A refused class
        // emits nothing, so EmitsCompilableCode could never express either.
        private const int DirectCompileErrorCallBaseline = 55;
        ...
        {
            "GeneratorTestHarness.cs", "GeneratorAssert.cs", "GeneratorAssertSelfTests.cs", "FixtureAdoptionScanTests.cs"
        };
        ...
        private static IEnumerable<(string File, string Text)> TestSources()
        {
            ...
            {
                if (path.Contains($"{Path.DirectorySeparatorChar}obj{Path.DirectorySeparatorChar}", StringComparison.Ordinal) || path.Contains($"{Path.DirectorySeparatorChar}bin{Path.DirectorySeparatorChar}", StringComparison.Ordinal))
                {
                    continue;
                }

                if (ExemptFiles.Contains(Path.GetFileName(path), StringComparer.Ordinal))
                {
                    continue;
                }
        ...
        public void The_hand_rolled_compiles_clean_idiom_has_not_come_back()
        {
            ...
            foreach (var (file, text) in TestSources())
            {
                ...
                var matches = Regex.Matches(text,
                    @"Assert\.DoesNotContain\(\s*\w+\s*,\s*\w+ => \w+\.Severity == DiagnosticSeverity\.Error\s*\);\s*" + @"Assert\.Empty\(\s*GeneratorTestHarness\.RunAndGetCompilationErrors");
                if (matches.Count > 0)
                {
                    offenders.Add($"{file} ({matches.Count})");
                }
            }

            Assert.True(offenders.Count == 0,
                "The hand-rolled 'compiles clean' idiom is back in: " + string.Join(", ", offenders) + ".\nUse GeneratorAssert.CompilesClean(source) instead — it asserts both halves and, when it fails, " + "prints the diagnostics AND the generated code rather than a bare 'collection was not empty'.");
        }
        ...
        public void The_fixture_is_actually_used()
        {
            ...
            Assert.True(uses >= 200,
                $"Only {uses} GeneratorAssert call sites found (expected 200+). The shared fixture was bypassed or " + "the migration was reverted — the raw-form ratchets above would still pass, so this is the check " + "that notices.");
        }
