...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class GeneratorFrameworkSelfTests
    {
        ...
        public void FullyCachedOnRerun_FIRES_for_a_non_equatable_model()
        {
            var ex = Assert.ThrowsAny<Exception>(() =>
                GeneratorCacheAssert.FullyCachedOnRerun(
                    new NonEquatableGenerator(),
                    Valid,
                    new[]
                    {
                        NonEquatableGenerator.StepName
                    }));
        ...
        public void NoSymbolsInPipeline_FIRES_for_a_symbol_returned_directly()
        {
            var ex = Assert.ThrowsAny<Exception>(() =>
                GeneratorCacheAssert.NoSymbolsInPipeline(
                    new LeakedSymbolGenerator(),
                    Valid,
                    new[]
                    {
                        LeakedSymbolGenerator.StepName
                    }));
        ...
        public void NoSymbolsInPipeline_FIRES_for_a_symbol_held_in_a_model_property()
        {
            var ex = Assert.ThrowsAny<Exception>(() =>
                GeneratorCacheAssert.NoSymbolsInPipeline(
                    new LeakedSymbolPropertyGenerator(),
                    Valid,
                    new[]
                    {
                        LeakedSymbolPropertyGenerator.StepName
                    }));
        ...
        public void Symbol_leak_walk_reports_truncation_rather_than_passing_silently()
        {
            var ex = Assert.ThrowsAny<Exception>(() =>
                GeneratorCacheAssert.NoSymbolsInPipeline(
                    new DeepNoLeakGenerator(),
                    Valid,
                    new[]
                    {
                        DeepNoLeakGenerator.StepName
                    }));
        ...
        public void Symbol_leak_walk_terminates_on_a_cyclic_model()
        {
            ...
            GeneratorCacheAssert.NoSymbolsInPipeline(
                new CyclicModelGenerator(),
                Valid,
                new[]
                {
                    CyclicModelGenerator.StepName
                });
        }
        ...
        public void Fingerprint_is_sensitive_to_a_single_character_of_output()
        {
            ...
            var b = a with
            {
                Source = Valid.Replace("public int X", "public int Xx", StringComparison.Ordinal)
            };
        ...
        public void Runner_returns_every_emitted_file_not_just_the_first()
        {
            ...
            Assert.True(run.OutputsByHintName.Count > 1,
                "Expected the per-mapper file AND the assembly-wide aggregates. If only one file comes back the " + "runner has inherited GeneratorTestHarness.Run's aggregate filter and the golden corpus would " + "never cover the facade/DI/manifest emitters.");
        }
            ...
            public void Initialize(IncrementalGeneratorInitializationContext context)
            {
                var models = context.SyntaxProvider
                    .CreateSyntaxProvider(
                        ...
                        {
                            Name = ctx.Node.ToString()
                        })
                    .WithTrackingName(StepName);

                context.RegisterSourceOutput(models,
                    static (spc, m) => spc.AddSource(
                        "NonEquatable_" + m.Name.GetHashCode(StringComparison.Ordinal) + ".g.cs",
                        "// x"));
            }
        ...
        private sealed class LeakedSymbolGenerator : IIncrementalGenerator
        {
            ...
            public void Initialize(IncrementalGeneratorInitializationContext context)
            {
                ...
                context.RegisterSourceOutput(symbols,
                    static (spc, symbol) => spc.AddSource(
                        (symbol?.Name ?? "Unknown") + "_Leaked.g.cs",
                        "// x"));
            }
        }
        ...
        private sealed class LeakedSymbolPropertyGenerator : IIncrementalGenerator
        {
            ...
            public void Initialize(IncrementalGeneratorInitializationContext context)
            {
                ...
                context.RegisterSourceOutput(models,
                    static (spc, m) => spc.AddSource(
                        "LeakedProperty_" + m.Name.GetHashCode(StringComparison.Ordinal) + ".g.cs",
                        "// x"));
            }
        ...
        private sealed class DeepNoLeakGenerator : IIncrementalGenerator
        {
            ...
            public void Initialize(IncrementalGeneratorInitializationContext context)
            {
                ...
                context.RegisterSourceOutput(models,
                    static (spc, m) => spc.AddSource(
                        "DeepNoLeak_" + m.Name.GetHashCode(StringComparison.Ordinal) + ".g.cs",
                        "// x"));
            }

            private static Node BuildChain(string name, int depth)
            {
                return depth == 0 ? new Node(name, null) : new Node(name, BuildChain(name, depth - 1));
            }
        ...
        private sealed class CyclicModelGenerator : IIncrementalGenerator
        {
            ...
            public void Initialize(IncrementalGeneratorInitializationContext context)
            {
                ...
                context.RegisterSourceOutput(models,
                    static (spc, m) => spc.AddSource(
                        "Cyclic_" + m.Name.GetHashCode(StringComparison.Ordinal) + ".g.cs",
                        "// x"));
            }
