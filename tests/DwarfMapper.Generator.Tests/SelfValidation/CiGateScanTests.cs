...
public class CiGateScanTests
{
    ...
    public static IEnumerable<object[]> DeclaredJobs()
    {
        ...
        yield return ["surface-matrix", "the 866-cell surface census would stop being measured, and its "
                                        + "red cells are the only place a silently-ignored option is visible"];
        ...
        yield return ["roslyn-forward-compat", "the generator would stop being built and tested against "
                                               + "an unpinned newer SDK (declaration only — the leg is "
                                               + "continue-on-error and cannot fail the workflow)"];
        ...
        yield return ["reproducible-build", "the shipped packages would stop being byte-compared across "
                                            + "two clean builds of the same commit"];
        yield return ["package-size", "the package-size ceiling would stop being measured, and the class "
                                      + "of regression it alone catches — something unintended starting "
                                      + "to ship — would go back to being invisible"];
        yield return ["cross-platform", "Windows and macOS would stop being tested at all, leaving ubuntu "
                                        + "as the only platform any suite has ever run on in CI"];
        // continue-on-error: true by design (a preview SDK's own breakage must not red the nightly).
        yield return ["preview-sdk-canary", "the next major SDK would stop being tried against this repo "
                                            + "before it ships (declaration only — the leg is "
                                            + "continue-on-error and cannot fail the workflow)"];
        ...
        yield return ["bench-wall-time-alert", "benchmark wall-time would stop being tracked night to "
                                               + "night (declaration only — the alert step is "
                                               + "continue-on-error by deliberate design)"];
    }
    ...
    public void Every_job_in_ci_yml_is_covered_by_a_row_of_this_scan()
    {
        ...
        Assert.True(actual.Count > 5,
            $"only {actual.Count} job(s) parsed out of ci.yml — the `jobs:` section or the two-space job "
            + "indentation this scan (and the substring check above) both rely on has changed shape, and "
            + "every assertion in this class is now measuring the wrong thing.");
        ...
        Assert.True(missingRows.Count == 0,
            "ci.yml declares job(s) that no row of this scan names: [" + string.Join(", ", missingRows)
            + "]. Deleting one of them would be a SILENT deletion — the exact hole filed as I16. Add a row "
            + "to DeclaredJobs() with the sentence naming what its absence would stop proving.");

        Assert.True(staleRows.Count == 0,
            "this scan has row(s) for job(s) ci.yml no longer declares: [" + string.Join(", ", staleRows)
            + "]. Either the job was deleted (and the row is the alarm — read it before deleting it) or it "
            + "was renamed (and the row must be renamed with it, or it proves nothing while looking like "
            + "it proves something).");
    }
