    ...
    public class DocFenceScanTests
    {
        ...
        public void No_hand_written_csharp_fence_outside_a_snippet_or_an_exemption()
        {
            ...
            Assert.True(offenders.Count == 0,
                "Hand-written C# fence(s) found. Back each with a '<!-- snippet: id -->' pair whose region lives " + "in a compiled sample, or mark it '<!-- fence-exempt: reason -->' immediately above:\n  " + string.Join("\n  ", offenders));
        }
        ...
        public void Every_unconverted_entry_still_has_an_unbacked_fence()
        {
            ...
            Assert.True(fixedUp.Count == 0,
                "These documents have no unbacked csharp fences left — remove them from Unconverted so the " + "ratchet tightens: " + string.Join(", ", fixedUp));
        }
        ...
        private static List<string> UnbackedFences(string relative, string markdown)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                if (!trimmed.StartsWith("```csharp", StringComparison.Ordinal) || insideSnippet)
                {
                    continue;
                }
