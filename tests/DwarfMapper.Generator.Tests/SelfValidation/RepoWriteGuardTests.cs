    ...
    public class RepoWriteGuardTests
    {
        ...
        public void Under_mutation_the_write_back_refuses_and_the_target_is_untouched()
        {
            InTempDir(dir =>
            {
                ...
                Assert.False(RepoWriteGuard.WriteBack(target, "mutant garbage", true));
                Assert.Equal("original", File.ReadAllText(target));

                Assert.False(RepoWriteGuard.WriteBackLines(target, ["mutant", "garbage"], true));
                Assert.Equal("original", File.ReadAllText(target));
            });
        }
        ...
        public void Under_mutation_the_write_back_does_not_even_create_the_directory()
        {
            ...
            InTempDir(dir =>
            {
                ...
                Assert.False(RepoWriteGuard.WriteBack(target, "content", true));
        ...
        public void Outside_mutation_the_write_back_writes_and_creates_the_directory()
        {
            ...
            {
                var target = Path.Combine(dir, "docs", "generated", "index.md");
                Assert.True(RepoWriteGuard.WriteBack(target, "content", false));
        ...
        public void Every_raw_write_api_use_in_the_test_tree_is_a_registered_pattern()
        {
            ...
            var allowed = new Dictionary<string, (int Count, string Reason)>(StringComparer.Ordinal)
            {
                ...
                ["DwarfMapper.Generator.Tests/SelfValidation/ApiReferenceRendererTests.cs"] =
                    (1, "InTempFile helper: the B23 doc-XML fixture, temp directory only"),
            ...
            Assert.True(offenders.Count == 0,
                "Raw file-writing API use outside the registered ARCH-06 allowlist. Route repo writes through " + "RepoWriteGuard.WriteBack (which refuses them under Stryker — T3-H1), or register a genuinely " + "temp-only write here with its reason:\n  " + string.Join("\n  ", offenders));
        }
