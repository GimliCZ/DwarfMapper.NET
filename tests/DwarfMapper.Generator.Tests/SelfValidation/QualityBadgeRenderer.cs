...
internal static partial class QualityBadgeRenderer
{
    ...
    internal static string BandColour(double measured, double gate, double step)
    {
        ...
        if (truncated < gate) return "red";
        if (truncated >= gate + BandWidth) return "yellow";
        return "brightgreen";
    }
    ...
    public static IReadOnlyList<string> RenderRows()
    {
        ...
        foreach (var (assembly, floor) in floors)
        {
            ...
            var colour = BandColour(floor, floor, step: 0.1);
            rows.Add(Badge($"coverage {assembly}", Percent(floor, decimals: 1), colour, HousekeepingPath));
        }
        ...
        {
            var colour = BandColour(leg.RawScore, leg.Break, step: 1.0);
            rows.Add(Badge($"mutation {leg.Name}", Percent(leg.RawScore, decimals: 2), colour, leg.ConfigFile));
        }
    ...
    internal static string Badge(string label, string value, string colour, string linkTarget)
    {
        if (label.Contains('-', StringComparison.Ordinal))
            throw new InvalidOperationException(
                $"Badge label '{label}' contains a dash, which shields.io reads as the label/value separator. "
                + "Double it ('--') or rename the label; rendering it raw would silently mangle the badge.");
    ...
    private static IReadOnlyList<CoverageFloor> CoverageFloors() =>
        ParseCoverageFloors(File.ReadAllText(Path.Combine(RepoLayout.Root, HousekeepingPath)));
    ...
    internal static IReadOnlyList<CoverageFloor> ParseCoverageFloors(string housekeeping)
    {
        ...
        if (open < 0)
            throw new InvalidOperationException(
                $"{HousekeepingPath}: no '{opener}' block. The README's coverage badges are rendered from it; "
                + "if the floors moved somewhere else, point this reader at the new home rather than letting "
                + "the badges render from nothing.");
        ...
        foreach (var floor in floors)
            if (floor.Floor is <= 0 or > 100)
                throw new InvalidOperationException(
                    $"{HousekeepingPath}: coverage floor for {floor.Assembly} parsed as {floor.Floor}, which is "
                    + "not a percentage. The parse is wrong, not the gate.");
    ...
    private static List<MutationLeg> MutationLegs()
    {
        ...
        return rows.Select(row =>
        {
            ...
            if (!File.Exists(configPath))
                throw new InvalidOperationException(
                    $"{LedgerPath}: leg '{row.Name}' names config '{row.ConfigFile}', which does not exist. "
                    + "The ledger row and the gate it describes have come apart.");

            return row with { Break = ReadBreak(row.ConfigFile, File.ReadAllText(configPath)) };
        }).ToList();
    }
    ...
    internal static IReadOnlyList<MutationLeg> ParseLedgerRows(string ledger)
    {
        ...
        if (legs.Count != ExpectedMutationLegs)
            throw new InvalidOperationException(
                $"{LedgerPath}: parsed {legs.Count} mutation leg row(s), expected {ExpectedMutationLegs}. A leg "
                + "was added or removed (update ExpectedMutationLegs with its badge), or the per-leg summary "
                + "table's shape changed and this reader is now reading fewer legs than are gated.");

        foreach (var leg in legs)
            if (leg.RawScore is <= 0 or > 100)
                throw new InvalidOperationException(
                    $"{LedgerPath}: raw score for leg {leg.Name} parsed as {leg.RawScore}, which is not a "
                    + "percentage. The parse is wrong, not the ledger.");
    ...
    internal static int ReadBreak(string configFile, string json)
    {
        ...
        if (!document.RootElement.TryGetProperty("stryker-config", out var config)
            || !config.TryGetProperty("thresholds", out var thresholds)
            || !thresholds.TryGetProperty("break", out var breakValue)
            || !breakValue.TryGetInt32(out var result))
            throw new InvalidOperationException(
                $"{configFile}: no integer 'stryker-config.thresholds.break'. The badge grades the ledger's raw "
                + "score against that number; without it there is nothing to grade against.");
    ...
    [GeneratedRegex(@"^\|\s*(?<leg>[a-z][a-z0-9.-]*)\s*\|\s*`(?<config>stryker-config[^`]*\.json)`\s*\|"
                    + @"[^|]*\|\s*(?<raw>\d+(?:\.\d+)?)\s*%", RegexOptions.Multiline)]
