...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    internal static class PwshBattery
    {
        ...
        internal static Dictionary<string, string> Run(string workDir, int expectedCases, string subject)
        {
            ...
            {
                Assert.Fail($"pwsh (PowerShell 7) is not on PATH — {subject} cannot be verified on this " + "machine, and housekeeping.ps1 itself needs the same prerequisite (H8). " + $"Underlying error: {e.Message}");
            }
            ...
            {
                process.Kill(true);
                Assert.Fail($"The battery for {subject} did not finish within 120 s — killed. These checks " + "must be milliseconds; a hang here means gate-checks.ps1 gained blocking " + "behaviour.");
            }
            ...
            Assert.True(process.ExitCode == 0,
                $"The battery for {subject} exited {process.ExitCode} — the harness itself failed rather than " + $"a scenario (every scenario catches its own throw).\nstdout:\n{stdout}\nstderr:\n{stderr}");
            ...
            Assert.True(cases.Count == expectedCases,
                $"Expected {expectedCases} CASE lines from the {subject} battery, got {cases.Count} — a " + "scenario vanished, which is exactly the vacuity this test exists to prevent.\nstdout:\n" + $"{stdout}\nstderr:\n{stderr}");
            return cases;
        }
    }
}
