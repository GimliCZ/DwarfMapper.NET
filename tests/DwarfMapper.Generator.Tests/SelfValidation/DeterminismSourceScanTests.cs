    ...
    public class DeterminismSourceScanTests
    {
        ...
        [
            ...
            ("D1", new Regex(@"new\s+Sorted(?:Set|Dictionary)\s*<[^>]*>\s*\(\s*\)", RegexOptions.Compiled),
                "a comparer-less SortedSet/SortedDictionary orders via culture-sensitive string.CompareTo; " + "pass an explicit ordinal comparer (see AmbientValidator.OrdinalPair)"),

            ("D2", new Regex(@"(?:CultureInfo|StringComparison|StringComparer)\.CurrentCulture", RegexOptions.Compiled),
                "current-culture comparison makes generated output depend on the build machine's locale; " + "use the Ordinal/Invariant form"),
        ...
        public void Generator_source_contains_no_nondeterministic_constructs()
        {
            ...
            foreach (var file in GeneratorSources())
            {
                ...
                for (var i = 0; i < lines.Length; i++)
                {
                    ...
                    foreach (var (id, pattern, why) in Banned)
                    {
                        ...
                        if (Allowlist.ContainsKey(relative + ":" + id))
                        {
                            continue;
                        }
            ...
            Assert.True(violations.Count == 0,
                "Nondeterministic construct(s) in generator source — generated output would depend on the machine " + "that built it:\n" + string.Join("\n", violations) + "\n\nFix the construct. Only add an entry to DeterminismSourceScanTests.Allowlist if the " + "construct provably cannot reach emitted text, and say why in the value.");
        }
        ...
        private static IEnumerable<string> GeneratorSources()
        {
            return Directory.EnumerateFiles(
                    Path.Combine(RepoRoot, "src", "DwarfMapper.Generator"),
                    "*.cs",
