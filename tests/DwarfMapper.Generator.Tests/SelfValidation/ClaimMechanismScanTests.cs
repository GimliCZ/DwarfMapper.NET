...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class ClaimMechanismScanTests
    {
        ...
        private static HashSet<string> BuildTestTypeNames()
        {
            ...
            foreach (var file in Directory.EnumerateFiles(Path.Combine(RepoRoot, "tests"),
                         "*.cs",
        ...
        public void Every_published_claim_names_a_mechanism_that_exists()
        {
            ...
            Assert.True(unbound.Count == 0,
                "Published claim(s) whose mechanism is missing — the documentation asserts a guarantee nothing " + "enforces:\n" + string.Join("\n", unbound) + "\n\nEither restore the mechanism, or delete the claim. A claim without a mechanism is the " + "drift this scan exists to end.");
        }
        ...
        public void The_register_is_not_vacuous()
        {
            ...
            Assert.True(rows.Count >= 13,
                $"Only {rows.Count} claim rows parsed (expected >= 13: SEC-01..07 + COR-01..06). The register " + "regex has probably stopped matching after a table edit, which would make this scan vacuous.");
        ...
        public void Claim_ids_are_unique_and_contiguous()
        {
                     ...
                     })
            {
                var numbers = ids.Where(i => i.StartsWith(prefix, StringComparison.Ordinal))
                    .Select(i => int.Parse(i[4..], CultureInfo.InvariantCulture))
                    .OrderBy(n => n).ToList();
