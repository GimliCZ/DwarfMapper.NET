...
public class ReflectionFreeMetaTests
{
    ...
    public static IEnumerable<object[]> Sources()
    {
        for (var seed = 0; seed < 40; seed++)
            yield return new object[] { SyntheticSchema.GenerateBehavioral(seed), $"behavioral#{seed}" };
        for (var seed = 0; seed < 24; seed++)
            yield return new object[] { SyntheticSchema.GenerateWithAdvancedFeatures(seed), $"advanced#{seed}" };
    }
