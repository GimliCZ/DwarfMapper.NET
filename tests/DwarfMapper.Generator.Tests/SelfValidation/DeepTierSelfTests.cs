...
public class DeepTierSelfTests
{
    ...
    public void The_fast_tier_is_byte_for_byte_the_historical_counts()
    {
        foreach (var (population, pinned) in PinnedFastCounts)
            Assert.True(DeepTier.CountFor(population, deep: false) == pinned,
                $"{population}: fast count {DeepTier.CountFor(population, deep: false)} != pinned {pinned}. "
                + "The fast tier is time-capped; changing it is a deliberate decision made HERE, not a side "
                + "effect of a catalog edit.");
    }
    ...
    public void No_test_reads_the_environment_variable_outside_the_helper()
    {
        ...
        {
            if (Path.GetFileName(file) == "DeepTier.cs") continue;
            if (read.IsMatch(File.ReadAllText(file)))
                offenders.Add(Path.GetRelativePath(RepoPaths.Tests, file));
        }

        Assert.True(offenders.Count == 0,
            "These test files read the deep-tier environment variable directly instead of registering a "
            + "DeepPopulation and calling DeepTier.Count — the self-test cannot see an unregistered "
            + "population: " + string.Join(", ", offenders));
    }
    ...
    public void Every_registered_population_has_a_call_site()
    {
        ...
        var unused = Enum.GetValues<DeepPopulation>()
            .Where(population => !sources.Any(s => s.Contains(
                "DeepPopulation." + population, StringComparison.Ordinal)))
            .ToList();

        Assert.True(unused.Count == 0,
            "These DeepPopulation entries have no call site (no test passes them to DeepTier.Count): "
            + string.Join(", ", unused));
    }
}
