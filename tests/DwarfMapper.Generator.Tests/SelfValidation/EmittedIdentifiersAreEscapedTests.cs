    ...
    public void No_emitter_writes_a_raw_member_name()
    {
        ...
        foreach (var relative in EmitterFiles)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                if (line.Contains("EmitTargetName", StringComparison.Ordinal)
                    || line.Contains("EmitSourceName", StringComparison.Ordinal)) continue;
                if (Array.Exists(NotEmission, e => line.Contains(e, StringComparison.Ordinal))) continue;
