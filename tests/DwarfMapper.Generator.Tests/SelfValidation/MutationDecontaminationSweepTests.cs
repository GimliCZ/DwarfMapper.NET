...
public class MutationDecontaminationSweepTests
{
    ...
    public void The_sweep_reds_on_a_planted_mutant_passes_when_clean_and_refuses_to_pass_vacuously()
    {
        ...
        try
        {
            ...
            MakeTree(Path.Combine(dir, "clean"), plantedMarker: false, withOriginals: true, withTestCopies: true);
            MakeTree(Path.Combine(dir, "planted"), plantedMarker: true, withOriginals: true, withTestCopies: true);
            MakeTree(Path.Combine(dir, "no-originals"), plantedMarker: false, withOriginals: false, withTestCopies: true);
            MakeTree(Path.Combine(dir, "no-copies"), plantedMarker: false, withOriginals: true, withTestCopies: false);
            ...
            MakeTree(dirtyOriginals, plantedMarker: false, withOriginals: true, withTestCopies: true);
            WriteTemp(Path.Combine(dirtyOriginals, "src", "DwarfMapper.Generator", "bin", "Release",
                "DwarfMapper.Generator.dll"), "clean bytes " + Marker + " oops");
            ...
            var cases = PwshBattery.Run(dir, expectedCases: 5, subject: "the I4 decontamination sweep");
    ...
    public void Every_mutation_leg_calls_the_sweep_in_housekeeping_and_in_ci()
    {
        ...
        Assert.True(legCalls == 3,
            $"scripts/housekeeping.ps1 calls Assert-NoMutatedProductBinaries {legCalls} time(s), expected "
            + "exactly 3 (one per mutation leg) — a leg that is not swept can leave a mutated product "
            + "assembly in a test bin, and the next incremental build keeps it (I4).");
        ...
        Assert.True(ci.Contains("Assert-NoMutatedProductBinaries", StringComparison.Ordinal),
            ".github/workflows/ci.yml: the mutation matrix no longer sweeps for mutated product binaries. "
            + "`git diff --exit-code` cannot see them — bin/ is git-ignored (I4).");
    ...
    private static void MakeTree(string root, bool plantedMarker, bool withOriginals, bool withTestCopies)
    {
        if (withOriginals)
        {
            WriteTemp(Path.Combine(root, "src", "DwarfMapper", "bin", "Release", "DwarfMapper.dll"),
                "clean runtime bytes");
            WriteTemp(Path.Combine(root, "src", "DwarfMapper.Generator", "bin", "Release",
                "DwarfMapper.Generator.dll"), "clean generator bytes");
        }

        if (!withTestCopies) return;
        ...
        WriteTemp(Path.Combine(root, "tests", "DwarfMapper.Generator.Tests", "bin", "Debug",
            "DwarfMapper.Generator.Tests.dll"), "test bytes mentioning " + Marker + " in a scan pin");

        WriteTemp(Path.Combine(root, "tests", "DwarfMapper.CorpusTests", "bin", "Debug", "DwarfMapper.dll"),
            "clean runtime bytes");
        WriteTemp(Path.Combine(root, "tests", "DwarfMapper.CorpusTests", "bin", "Debug",
