    ...
    public class SelfAuditNonVacuityTests
    {
        ...
        public void Descriptor_reflection_finds_the_full_diagnostic_family()
        {
            ...
            Assert.True(descriptors.Count >= 60,
                $"Only {descriptors.Count} DiagnosticDescriptor fields reflected — the descriptor-based scans would " + "be near-vacuous. The type moved or its field shape changed.");
        }
        ...
        public void Public_attribute_and_enum_reflection_is_non_empty()
        {
            ...
            Assert.True(attributeCount >= 10,
                $"Only {attributeCount} public attribute types reflected — the attribute-coverage scans would be " + "near-vacuous.");
        ...
        public void The_internal_taxonomy_enums_reflect_with_values()
        {
            ...
            foreach (var metadataName in new[]
                     {
                         "DwarfMapper.Generator.Pipeline.CollectionConverter+TargetKind", "DwarfMapper.Generator.Pipeline.CollectionConverter+CountKind", "DwarfMapper.Generator.Pipeline.DictionaryConverter+DictTargetKind", "DwarfMapper.Generator.Model.NullHandling"
                     })
            {
        ...
        public void The_generator_and_test_source_trees_are_found_and_non_empty()
        {
            ...
            var generatorCs = Directory.EnumerateFiles(
                    Path.Combine(root, "src", "DwarfMapper.Generator"),
                    "*.cs",
        ...
        {
            return !f.Contains($"{Path.DirectorySeparatorChar}obj{Path.DirectorySeparatorChar}", StringComparison.Ordinal) && !f.Contains($"{Path.DirectorySeparatorChar}bin{Path.DirectorySeparatorChar}", StringComparison.Ordinal);
        }
