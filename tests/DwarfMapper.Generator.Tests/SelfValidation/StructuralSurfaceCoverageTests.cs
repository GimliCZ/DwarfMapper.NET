...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             the signature-triggered map MODES on <c>MapMethodModel</c> (projection / span / async-stream),
    ///             recognised by method-signature shape rather than any attribute; and
    ///         </item>
    ...
    public sealed class StructuralSurfaceCoverageTests
    {
        ...
        private static readonly (string Flag, string[] AnchorFiles)[] SignatureTriggeredModes =
        {
            ("IsProjection", new[]
            {
                "ProjectionTests.cs", "ProjectionRuntimeTests.cs"
            }),
            ("IsSpanMap", new[]
            {
                "SpanMapGeneratorTests.cs", "SpanMapRuntimeTests.cs"
            }),
            ("IsAsyncStreamMap", new[]
            {
                "AsyncStreamMapGeneratorTests.cs", "AsyncStreamMapRuntimeTests.cs"
            })
        };
        ...
        public void Every_signature_triggered_mode_flag_exists_and_is_anchored_by_real_tests()
        {
            ...
            var testRoots = new[]
            {
                Path.Combine(RepoRoot(), "tests", "DwarfMapper.Generator.Tests"), Path.Combine(RepoRoot(), "tests", "DwarfMapper.IntegrationTests")
            };

            foreach (var (flag, anchorFiles) in SignatureTriggeredModes)
            {
                Assert.True(flagNames.Contains(flag),
                    $"Signature-triggered mode flag '{flag}' no longer exists on MapMethodModel — it was renamed or " + "removed. Update SignatureTriggeredModes.");
                ...
                foreach (var file in anchorFiles)
                {
                    ...
                    Assert.True(path is not null,
                        $"Coverage anchor '{file}' for mode '{flag}' not found — the mode's dedicated tests moved or " + "were deleted, leaving the mode unexercised. Restore them or update the anchor.");
                    var text = File.ReadAllText(path);
                    Assert.True(text.Contains("[Fact]", StringComparison.Ordinal) || text.Contains("[Theory]", StringComparison.Ordinal),
        ...
        public void Every_public_MapConfig_method_is_exercised_by_a_MapConfig_test()
        {
            ...
            Assert.True(methodNames.Count >= 7,
                $"Only {methodNames.Count} public MapConfig methods reflected — this gate would be near-vacuous. " + "MapConfig moved or its shape changed.");
            ...
            var missing = methodNames
                .Where(n => !corpus.Contains($".{n}(", StringComparison.Ordinal) && !corpus.Contains($".{n}<", StringComparison.Ordinal))
                .ToList();

            Assert.True(missing.Count == 0,
                "Public MapConfig method(s) with no call in any *MapConfig*.cs test: " + string.Join(", ", missing) + ". Add a test that configures a mapping using the method.");
        }
