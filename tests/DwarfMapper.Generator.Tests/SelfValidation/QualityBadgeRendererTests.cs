...
public class QualityBadgeRendererTests
{
    ...
    public void The_coverage_floors_are_read_from_the_gate_script_itself()
    {
        var floors = QualityBadgeRenderer.ParseCoverageFloors(
            File.ReadAllText(Path.Combine(DocTooling.RepoLayout.Root, "scripts", "housekeeping.ps1")));
    ...
    public void A_renamed_floors_block_is_a_loud_failure_not_an_empty_badge_row()
    {
        var ex = Assert.Throws<InvalidOperationException>(
            () => QualityBadgeRenderer.ParseCoverageFloors("$somethingElse = [ordered]@{\n  'A' = 1.0\n}\n"));
    ...
    public void The_mutation_legs_are_read_from_the_equivalents_ledger()
    {
        var legs = QualityBadgeRenderer.ParseLedgerRows(File.ReadAllText(
            Path.Combine(DocTooling.RepoLayout.Root, "Issues", "ledgers", "equivalent-mutants.md")));
    ...
    {
        var ex = Assert.Throws<InvalidOperationException>(
            () => QualityBadgeRenderer.ParseLedgerRows("| leg | scoreable |\n|---|---|\n| generator | 201 |\n"));
    ...
    public void The_colour_is_the_gates_own_verdict_on_the_number(
        double measured, double gate, double step, string expected)
    {
    ...
    {
        var ex = Assert.Throws<InvalidOperationException>(
            () => QualityBadgeRenderer.Badge("code-coverage", "91.2%25", "brightgreen", "x"));
    ...
    public void The_note_carries_every_legs_break_value()
    {
        ...
        foreach (var leg in legs)
        {
            var expected = QualityBadgeRenderer.ReadBreak(leg.ConfigFile,
                File.ReadAllText(Path.Combine(DocTooling.RepoLayout.Root, leg.ConfigFile)));
            Assert.Contains(
                string.Create(CultureInfo.InvariantCulture, $"break {expected} ({leg.Name})"), note,
    ...
    public void The_committed_readme_carries_the_rendered_region_verbatim()
    {
        ...
        var readme = DocTooling.DocSet.Read("README.md").Replace("\r\n", "\n", StringComparison.Ordinal);
