...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class DocPipelinePropertyTests
    {
        ...
        {
            return new Dictionary<string, SnippetRegion>(StringComparer.Ordinal)
            {
                ["demo"] = new SnippetRegion("demo", body, "F.cs", 1)
            };
        }
        ...
        public void A_region_survives_extraction_after_being_written_into_source()
        {
            ...
            Body.Select(Gen.Int[0, 3]).Sample(t =>
                {
                    ...
                    {
                        return; // an all-blank region is a loud error, tested elsewhere
                    }
        ...
        public void A_malformed_marker_always_throws_and_never_returns_partial_text()
        {
            ...
            Gen.OneOf(
                    ...
                    Gen.Const("// <snippet: >\nx\n// </snippet>\n"), // empty id
                    Gen.Const("// <snippet: a>\n// </snippet>\n")) // empty body
                .Sample(source => Assert.Throws<DocToolingException>(() => SnippetScanner.ScanFile("F.cs", source)), iter: ScanIters);
        }
