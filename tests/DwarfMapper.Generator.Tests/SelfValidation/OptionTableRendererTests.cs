...
namespace DwarfMapper.Generator.Tests.SelfValidation
{
    ...
    public class OptionTableRendererTests
    {
        ...
        [Fact]
        public void A_null_committed_document_still_renders_the_mechanical_columns()
        {
            // The renderer is asked for a table before any document exists — a brand-new options page, or a
            // caller that read a missing file into null. "No document" means "no carried prose", not a
            // dereference: the mechanical columns come from reflection and owe the committed file nothing.
            var rows = OptionTableRenderer.RenderRows(typeof(OptionNamedOption), null!, "options");
        ...
        [Fact]
        public void Prose_is_read_only_from_inside_the_named_table()
        {
            // Option-shaped rows, but no '<!-- table: options -->' marker anywhere: a hand-written table nobody
            // has wired up yet, or the same page's table under a different name. The reader must find NOTHING.
            // A reader that fell back to "start at the top of the file" whenever the marker was missing would
            // caption real options from whatever pipe rows the page happens to contain — and this table is
            // generated into a tracked document, so the wrong caption would be committed.
            const string committed = "Intro.\n\n" + "| Option | Type | Default | What it does |\n" + "|---|---|---|---|\n" + "| `Named` | `string` | `\"abc\"` | HIJACKED |\n";

            var rows = OptionTableRenderer.RenderRows(typeof(SyntheticOptions), committed, "options");

            Assert.Contains("| `Named` | `string` | `\"abc\"` |  |", rows);
            Assert.DoesNotContain(rows, r => r.Contains("HIJACKED", StringComparison.Ordinal));
        }

        [Fact]
        public void Both_bool_defaults_render_as_their_own_literal()
        {
            // Only a TRUE default had ever been rendered, which left the false arm free: a formatter answering
            // "`true`" for every bool — or answering nothing at all for false — produced a table that read fine
            // and documented the opposite of what the attribute does. Both arms are pinned here, in one type,
            // so neither can be reached only by the other's value.
            var rows = OptionTableRenderer.RenderRows(typeof(BoolDefaults), "", "options");
        ...
        {
            var exType = Assert.Throws<ArgumentNullException>(() => OptionTableRenderer.RenderRows(null!, "", "options"));
            var exRows = Assert.Throws<ArgumentNullException>(() => OptionTableRenderer.UndocumentedOptions(null!));
...
// reflection fixtures for OptionTableRendererTests only.
    ...
    /// <summary>
    ///     Both bool defaults at once. <c>SyntheticOptions</c> carries only a true-valued flag, so the
    ///     formatter's false arm had no case; a separate type adds one without disturbing that type's
    ///     exact-row-list test.
    /// </summary>
    public sealed class BoolDefaults
    {
    ...
    public sealed class ThrowingCtor
    {
        public ThrowingCtor()
        {
            throw new InvalidOperationException("defaults are not constructible");
        }
