...
{
    private static string InGallery(string fileName) => Path.Combine(RepoLayout.GalleryRoot, fileName);

    [Fact]
    public void A_wellformed_example_binds_to_its_single_file()
    {
        var attr = new DocExampleAttribute(5, Tier.Basics, "Synthetic title") { Shows = "the file-list seam" };
    ...
    public void A_type_without_a_public_static_run_is_refused()
    {
        ...
        var ex = Assert.Throws<DocToolingException>(
            () => ExampleCatalogue.Build(typeof(ExampleWithoutRun), attr, []));
    ...
    public void An_example_whose_file_was_renamed_is_refused()
    {
        ...
        var ex = Assert.Throws<DocToolingException>(
            () => ExampleCatalogue.Build(typeof(SyntheticExample), attr, [InGallery("06_Other.cs")]));
    ...
    public void An_example_matching_two_files_is_refused_naming_both()
    {
        ...
        var ex = Assert.Throws<DocToolingException>(() => ExampleCatalogue.Build(
            typeof(SyntheticExample), attr, [InGallery("07_A.cs"), InGallery("07_B.cs")]));
