...
namespace DwarfMapper.Generator.Tests
{
    public class DiagnosticTests
    {
        ...
        public void ReadOnly_destination_with_matching_source_reports_DWARF007()
        {
            ...
            Assert.Contains(diagnostics,
                d => d.Id == "DWARF007" &&
                     d.GetMessage(CultureInfo.InvariantCulture).Contains("Age", StringComparison.Ordinal));
        }
        ...
        public void UpdateInto_initOnly_target_reports_DWARF007_not_CS8852()
        {
            ...
            const string src = """
                               ...
                               """;
            var (diagnostics, _) = GeneratorTestHarness.Run(src);
            Assert.Contains(diagnostics, d => d.Id == "DWARF007" && d.GetMessage(CultureInfo.InvariantCulture).Contains("Id", StringComparison.Ordinal));
        ...
        public void MapValue_shadowing_source_member_reports_DWARF064()
        {
            const string src = """
                               ...
                               """;
            var (diagnostics, _) = GeneratorTestHarness.Run(src);
            Assert.Contains(diagnostics, d => d.Id == "DWARF064" && d.GetMessage(CultureInfo.InvariantCulture).Contains("Name", StringComparison.Ordinal));
        }
        ...
        public void UpdateInto_nested_member_reports_DWARF065()
        {
            const string src = """
                               ...
                               """;
            var (diagnostics, _) = GeneratorTestHarness.Run(src);
            Assert.Contains(diagnostics, d => d.Id == "DWARF065" && d.GetMessage(CultureInfo.InvariantCulture).Contains("Child", StringComparison.Ordinal));
            // A scalar member is a direct copy, not a replacement → no DWARF065 for it.
            Assert.DoesNotContain(diagnostics, d => d.Id == "DWARF065" && d.GetMessage(CultureInfo.InvariantCulture).Contains("Top", StringComparison.Ordinal));
        }
        ...
        public void When_guard_on_non_nullable_reference_reports_DWARF066()
        {
            ...
            Assert.Contains(diagnostics, d => d.Id == "DWARF066" && d.GetMessage(CultureInfo.InvariantCulture).Contains("Note", StringComparison.Ordinal));
        }
        ...
        public void GenerateWrapperMap_single_payload_synthesizes_closed_map_without_DWARF067()
        {
            ...
            Assert.Contains("Env<global::A>", generated, StringComparison.Ordinal);
            Assert.Contains("Env<global::B>", generated, StringComparison.Ordinal);
            GeneratorAssert.EmitsCompilableCode(src);
        }
    }
}
