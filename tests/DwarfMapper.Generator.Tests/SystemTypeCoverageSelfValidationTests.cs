    ...
    public class SystemTypeCoverageSelfValidationTests
    {
        ...
        public void Every_system_type_is_either_supported_or_loudly_rejected(string memberType)
        {
            ...
            if (dwarfErrors.Count > 0)
            {
                ...
                Assert.All(dwarfErrors,
                    d =>
                        Assert.True(d.Id.StartsWith("DWARF", StringComparison.Ordinal),
                            $"{memberType} is rejected, but not by a DwarfMapper diagnostic — got " + $"'{d.Id}: {d.GetMessage(CultureInfo.InvariantCulture)}'. An unsupported type must be " + "refused with a DWARF diagnostic, never leak a raw compiler error."));
                return;
            }
