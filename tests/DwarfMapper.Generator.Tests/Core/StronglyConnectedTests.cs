...
namespace DwarfMapper.Generator.Tests.Core
{
    ...
    public class StronglyConnectedTests
    {
        ...
        {
            var result = StronglyConnected.NodesOnACycle(Graph(("a", new[]
            {
                "a"
            })));
            Assert.Equal(new[]
        ...
        public void Two_disjoint_cycles_are_both_found()
        {
            ...
            Assert.Equal(new[]
                ...
                result.OrderBy(v => v, StringComparer.Ordinal));
        }
        ...
        public void Matches_the_per_node_DFS_it_replaced_across_generated_graphs()
        {
            for (var seed = 0; seed < 200; seed++)
            {
                ...
                Assert.True(viaScc.SetEquals(viaDfs),
                    $"seed {seed}: SCC gave [{string.Join(",", viaScc.OrderBy(x => x, StringComparer.Ordinal))}] " + $"but the reference DFS gave [{string.Join(",", viaDfs.OrderBy(x => x, StringComparer.Ordinal))}]");
            }
        }
        ...
        private static bool CanReachSelf(Dictionary<string, HashSet<string>> graph, string start)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                if (StringComparer.Ordinal.Equals(current, start))
                {
                    return true;
                }
