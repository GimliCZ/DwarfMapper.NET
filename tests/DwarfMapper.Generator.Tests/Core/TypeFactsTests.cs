...
public class TypeFactsTests
{
    ...
    private static INamedTypeSymbol Type(string metadataName) =>
        Compiled.GetTypeByMetadataName(metadataName)
        ?? throw new InvalidOperationException($"Test fixture does not declare '{metadataName}'.");

    private static ITypeSymbol TypeParameter(string name) =>
        Type("Demo.Generic`3").TypeParameters.Single(p => p.Name == name);

    private static ITypeSymbol NullableOf(ITypeSymbol underlying) =>
        Type("System.Nullable`1").Construct(underlying);
