...
namespace DwarfMapper.Generator.Tests.Fuzzing
{
    ...
    internal static class SyntheticSchema
    {
        ...
        /// <summary>
        ///     Reference-handling / update-into / round-trip mode for <see cref="GenerateBehavioralForMode" />
        ///     (items 5, 11).
        /// </summary>
        ...
        /// <summary>
        ///     Every fuzzed compilation is nullable-ANNOTATED — the context real consumers ship
        ...
        ///     <para>
        ///         <see cref="GeneratorTestHarness" /> defaults to <c>NullableContextOptions.Disable</c>, so without this
        ///         the whole fuzz corpus ran OBLIVIOUS: every reference member came back as
        ///         <c>NullableAnnotation.None</c> instead of Annotated/NotAnnotated. The generator branches on that
        ///         annotation (<c>SourceMayBeNullRef</c>, the null-forgiving <c>!</c>, DWARF070), so the fuzzers were
        ///         exercising paths production never takes and skipping the ones it does. Nullability has three states and
        ///         we were fuzzing only the one nobody ships.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     Item 5: the same behavioural Src/Dst shape emitted under a chosen reference-handling /
        ///     update-into mode. Member SELECTION is identical for a given seed across modes, so an instance built
        ///     from the same seed is structurally identical and the four modes' outputs can be compared directly.
        /// </summary>
        public static string GenerateBehavioralForMode(int seed, EmitMode mode)
        {
            var rng = new Random(seed);
            var memberCount = rng.Next(1, 11);
            var members = new (string Name, string Type)[memberCount];
        ...
        public static string GenerateWithAdvancedFeatures(int seed)
        {
            ...
            return featureVariant switch
            {
                ...
                2 => BuildSourceWithFlattenGraph(seed),
        ...
        private static string BuildSourceWithFlattenGraph(int seed)
        {
            var sb = new StringBuilder();
        ...
        /// <summary>
        ///     Item 18: the same behavioural Src/Dst shape with an explicit [DwarfMapper(...)] argument
        ///     string and emit mode. Because Src/Dst have identical member types no conversion diagnostic can fire,
        ///     so any combination of config options must compile cleanly and stay value-preserving on acyclic input.
        /// </summary>
        ...
        {
            return BuildSource(members, EmitMode.None);
        }
