...
namespace DwarfMapper.Generator.Tests.Fuzzing
{
    ...
    public class TopologyOracleFuzzTests
    {
        ...
        public void Topology_diamond_no_back_edge_preserved()
        {
            ...
            var dst = mapMethod.Invoke(mapper,
                new[]
                {
                    root
                })!;
        ...
        public static IEnumerable<object[]> GraphSeeds()
        {
            ...
            return Enumerable.Range(0, DeepTier.Count(DeepPopulation.TopologyGraphSeeds)).Select(i => new object[]
            {
                i
            });
        }
        ...
        private static void AddDictEdge(Type nodeType, object node, int key, object target)
        {
            ...
            dict.GetType().GetMethod("Add")!.Invoke(dict,
                new[]
                {
                    key, target
                });
        ...
        /// <summary>
        ///     DFS asserting the reference graph reachable from <paramref name="root" /> via Left/Right/Back
        ///     contains no cycle (back-edge to a node already on the active path).
        /// </summary>
