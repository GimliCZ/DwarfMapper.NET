...
internal static class CombinatorialSchema
    {
        ...
        public static IEnumerable<MatrixCell> DepthTwoMatrix()
        {
            ...
            {
                ("int", "long"), ("string", null), ("global::System.Guid", null), ("global::System.DateTime", null), ("Cmb_IntEnum", null)
            };
            var seed = 10_000;
            ...
            int seed)
        {
            ...
            if (shape == "nullable" && !IsValueType(basicType))
            {
                return null;
            }

            // Filter: DictStringValue requires a value-type or string key (to avoid duplicate key risk)
            if (shape == "DictStringValue" && !IsValidDictKey(basicType))
            {
                return null;
            }
            ...
            int seed)
        {
            ...
            {
                // Src nested object
                sb.AppendLine("public class CmbNested_" +
                              EscapeType(srcElem) +
                              "_Src { public " +
                              srcElem +
                              " Val { get; set; }" +
            ...
            int seed)
        {
            ...
            sb.AppendLine("public class CmbPolyConcrete : CmbPolyBase { public " +
                          srcElem +
                          " Val { get; set; }" +
                          DefaultInit(srcElem) +
                          " }");
            ...
            sb.AppendLine("public class CmbPolyConcreteDto : CmbPolyBaseDto { public " +
                          dstElem +
                          " Val { get; set; }" +
                          DefaultInit(dstElem) +
                          " }");
