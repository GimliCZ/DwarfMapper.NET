...
public class CombinatorialEngineTests
    {
        ...
        public void Identity_cell_compiles_emits_and_maps(MatrixCell cell)
        {
            ...
            {
                Assert.Fail(FormatCellFailure(cell,
                    "generator produced unexpected errors",
                    string.Join(", ",
        ...
        private static void TryMapAndVerify(MatrixCell cell, Assembly asm, string generatedSource)
        {
            ...
            {
                Assert.Fail(FormatCellFailure(cell,
                    "ObjectFactoryV2.Create threw (TargetInvocationException)",
            ...
            {
                Assert.Fail(FormatCellFailure(cell,
                    "ObjectFactoryV2.Create threw (InvalidOperationException)",
                    ex.Message,
                    generatedSource));
                return;
            }
            ...
            try
            {
                dstInstance = mapMethod.Invoke(mapper,
                    new[]
                    {
                        srcInstance
                    });
            ...
            string generatedSource)
        {
            ...
            var diffs = GraphOracleComparer.CrossTypeDiff(sv,
                dv,
            ...
            Assert.True(diffs.Count == 0,
                FormatCellFailure(cell,
                    "value not preserved after mapping",
...
public class CombinatorialDepthTwoTests
    {
        ...
        public void DepthTwo_cell_compiles_and_maps(MatrixCell cell)
        {
            ...
            Assert.True(unexpectedErrors.Count == 0,
                ...
                "\n" +
                "Errors: " +
                string.Join(", ", unexpectedErrors.Select(d => d.Id)) +
                "\n" +
                cell.Source);
            ...
            try
            {
                var dstInstance = mapMethod.Invoke(mapper,
                    new[]
                    {
                        srcInstance
                    });
...
public class ExtendedBehavioralFuzzTests
    {
        ...
        public void Oracle_crosstype_diff_catches_no_false_positives(int seed)
        {
            ...
            Assert.True(asm is not null,
                ...
                " emit failed: " +
                string.Join(", ", errors.Select(e => e.Id)) +
                "\n" +
                src);
            ...
            var diffs = GraphOracleComparer.CrossTypeDiff(srcInstance,
                dstInstance,
                srcType,
                dstType);
