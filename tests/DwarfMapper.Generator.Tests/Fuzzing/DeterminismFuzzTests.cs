...
public class DeterminismFuzzTests
{
    ...
    [InlineData(13)] [InlineData(29)] [InlineData(50)] [InlineData(101)] [InlineData(255)]
    public void Behavioral_schema_generates_byte_identical_output_twice(int seed)
    {
        ...
        Assert.Equal(
            string.Join("|", System.Linq.Enumerable.Select(d1, x => x.Id + "@" + x.Location.GetLineSpan().StartLinePosition)),
            string.Join("|", System.Linq.Enumerable.Select(d2, x => x.Id + "@" + x.Location.GetLineSpan().StartLinePosition)));
    }

    // GenerateWithAdvancedFeatures rotates through MapDerivedType, FlattenGraph, and top-level collections.
    [Theory]
    [InlineData(0)] [InlineData(1)] [InlineData(2)] [InlineData(3)] [InlineData(4)]
    [InlineData(5)] [InlineData(6)] [InlineData(7)] [InlineData(11)] [InlineData(64)]
