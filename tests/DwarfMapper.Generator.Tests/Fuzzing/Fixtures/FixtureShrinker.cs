        ...
        public static Fixture Shrink(Fixture failing, Func<Fixture, bool> stillFails)
        {
            ...
            if (!stillFails(failing))
            {
                throw new InvalidOperationException(
                    "Shrink was given a fixture that does not fail. Shrinking a passing input would 'minimise' to " + "an arbitrary fixture and present it as a repro.");
            }
            ...
            foreach (var simpler in new[]
                     {
                         MemberShape.Scalar, MemberShape.Reference, MemberShape.NullableScalar
                     })
                for (var i = 0; i < current.Members.Count; i++)
                {
                    if (current.Members[i].Shape == simpler)
                    {
                        continue;
                    }
