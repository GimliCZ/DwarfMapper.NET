...
namespace DwarfMapper.Generator.Tests.Fuzzing
{
    ...
    public class AllEmitPathsAgreeFuzzTests
    {
        ...
        {
            return Enumerable.Range(0, DeepTier.Count(DeepPopulation.AllEmitPathsSeeds)).Select(i => new object[]
            {
                i
            });
        }
        ...
        public void All_emit_modes_agree_on_acyclic_input(int seed)
        {
            var modes = new[]
            {
                SyntheticSchema.EmitMode.None, SyntheticSchema.EmitMode.Preserve, SyntheticSchema.EmitMode.SetNull, SyntheticSchema.EmitMode.UpdateInto
            };
            ...
            foreach (var mode in modes)
            {
                ...
                if (mode == SyntheticSchema.EmitMode.UpdateInto)
                {
                    ...
                    mapperType.GetMethod("Update")!.Invoke(mapper,
                        new[]
                        {
                            srcInstance, dstInstance
                        });
                }
                else
                {
                    dstInstance = mapperType.GetMethod("Map")!.Invoke(mapper,
                        new[]
                        {
                            srcInstance
                        })!;
