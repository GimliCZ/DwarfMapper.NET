...
public sealed record GraphSpec(IReadOnlyList<NodeSpec> Nodes, string RootSource, string RootDest)
{
    ...
    public void Validate()
    {
        ...
        Demand(Nodes.Select(n => n.Name).Distinct(StringComparer.Ordinal).Count() == Nodes.Count,
            "V1", "node names must be unique (CS0101: duplicate type declaration)");
        ...
        for (var i = 0; i < Nodes.Count; i++)
        {
            ...
            Demand(node.Members.Select(m => m.Name).Distinct(StringComparer.Ordinal).Count()
                   == node.Members.Count,
                "V4", $"node '{node.Name}' declares a duplicate member name (CS0102)");
            ...
            {
                if (member.NestedRef is not int j) continue;
                Demand(j >= 0 && j < Nodes.Count, "V2",
                    $"node '{node.Name}' member '{member.Name}' references node index {j}, out of range");
                Demand(j > i, "V2",
                    $"node '{node.Name}' (index {i}) member '{member.Name}' references index {j} — "
                    + "NestedRef must reference a STRICTLY LATER node so the containment graph is a DAG");
            }
            ...
            if (node.BaseRef is int b)
            {
                Demand(node.Kind is TypeKind.Class or TypeKind.Record, "V5",
                    $"node '{node.Name}' is a {node.Kind} and cannot have a base type (CS0527)");
                Demand(b >= 0 && b < Nodes.Count, "V5",
                    $"node '{node.Name}' BaseRef {b} is out of range");
                Demand(b > i, "V5",
                    $"node '{node.Name}' (index {i}) BaseRef {b} — the base must be a strictly later node "
                    + "so inheritance is acyclic (CS0146)");
                Demand(Nodes[b].Kind == node.Kind, "V5",
                    $"node '{node.Name}' ({node.Kind}) inherits '{Nodes[b].Name}' ({Nodes[b].Kind}) — "
                    + "classes inherit classes and records inherit records (CS8864/CS8865)");
                ...
                // the grammar" demands.)
                Demand(Nodes[b].Members.All(m => m.Shape != MemberShape.CtorParam), "V5b",
                    $"base node '{Nodes[b].Name}' has a CtorParam member — its explicit ctor would leave "
                    + "no parameterless ctor for derived declarations to chain to (CS1729)");
            }
        }
    ...
    private void DemandNoValueTypeCycle()
    {
        ...
        for (var start = 0; start < Nodes.Count; start++)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                if (colour[n] == 2) continue;
                Demand(colour[n] != 1, "V3",
    ...
    {
        if (Nodes[i].Kind is not (TypeKind.Struct or TypeKind.RecordStruct)) yield break;
        foreach (var member in Nodes[i].Members)
        {
            if (member is { Coll: CollShape.None, NestedRef: int j }
                && j >= 0 && j < Nodes.Count
                && Nodes[j].Kind is TypeKind.Struct or TypeKind.RecordStruct)
            {
                yield return j;
            }
        }
    ...
    public string Describe()
    {
        return $"GraphSpec({RootSource}->{RootDest}; "
               + string.Join("; ", Nodes.Select(n =>
                   $"{n.Name}:{n.Kind}"
                   + (n.BaseRef is int b ? $":base={b}" : "")
                   + (n.SplitAcrossFiles ? ":split" : "")
                   + "["
                   + string.Join(",", n.Members.Select(m =>
                       $"{m.Name}:{m.Shape}:{(m.NestedRef is int j ? "->" + j : m.ScalarType)}"
                       + (m.Nullable ? "?" : "")
                       + (m.Coll == CollShape.None ? "" : ":" + m.Coll)))
                   + "]"))
               + ")";
    }
}
