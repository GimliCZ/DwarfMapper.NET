    ...
    public static class TypeGraphGen
    {
        ...
        private static Gen<NodeRoll> NodeGen(int maxMembers)
        {
            return Gen.Int[1, maxMembers].SelectMany(memberCount =>
                MemberGen.Array[memberCount].Select(Pick(AllKinds),
                    Pick(AllKinds),
                    Gen.Int[0, 99],
                    Gen.Int[0, 999],
        ...
        public static Gen<GraphSpec> MirroredPair(int maxNodes = 4, int maxMembers = 5)
        {
            return Gen.Int[1, maxNodes]
                .SelectMany(n => NodeGen(maxMembers).Array[n].Select(Gen.Int[0, 999].Array[n], // dest-side member-shape re-roll seed, one per node
        ...
        private static GraphSpec Assemble(NodeRoll[] rolls, int[] destRolls)
        {
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                for (var k = 0; k < members.Length; k++)
                {
                    ...
                    if (isBaseTarget[i] && destShape == MemberShape.CtorParam)
                    {
                        destShape = MemberShape.AutoProp;
                    }

                    destMembers[k] = members[k] with
                    {
                        Shape = destShape,
                        NestedRef = nested + n
                    };
                ...
                sourceNodes[i] = new NodeSpec(
                    FormattableString.Invariant($"S{i}"),
                    sourceKinds[i],
                    members,
                    baseRef[i]);
                destNodes[i] = new NodeSpec(
                    FormattableString.Invariant($"D{i}"),
                    destKinds[i],
                    destMembers,
                    baseRef[i] is { } b ? b + n : null);
            }
