...
namespace DwarfMapper.CompilerTests.TypeGraphs
{
    ...
    public static class TypeGraphRenderer
    {
            ...
            string namespaceName = "T")
        {
            ...
            foreach (var node in graph.Nodes)
            {
                ...
                if (node.SplitAcrossFiles)
                {
                    ...
                    RenderNode(main, graph, node, node.Members.Take(half).ToList(), true, true);
                    var rest = new StringBuilder();
                    rest.Append("namespace ").Append(namespaceName).AppendLine(";");
                    rest.AppendLine();
                    RenderNode(rest, graph, node, node.Members.Skip(half).ToList(), true, false);
                    units.Add(rest.ToString());
                }
                else
                {
                    RenderNode(main, graph, node, node.Members, false, true);
                }
            }
            ...
            bool withCtor)
        {
            ...
            var baseClause = node.BaseRef is { } b ? " : " + graph.Nodes[b].Name : "";
        ...
        {
            var element = member.NestedRef is { } j ? graph.Nodes[j].Name : member.ScalarType;
            if (member.Nullable)
            {
                element += "?";
            }
