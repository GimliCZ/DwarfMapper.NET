...
public class EnumCoverageRatchetTests
{
        ...
        foreach (var file in RepoPaths.SourceFiles(contractsDir))
        {
            ...
            foreach (var token in root.DescendantTokens())
            {
                if (token.Kind() is not (SyntaxKind.StringLiteralToken
                    ...
                    or SyntaxKind.MultiLineRawStringLiteralToken)) continue;
                if (!DeclarationHint.IsMatch(token.ValueText)) continue;
                fragments.Add(SyntaxFactory.ParseCompilationUnit(token.ValueText));
            }
        }

        Assert.True(fragments.Count > 0,
            "no C# fragments found in the Contracts folder — the case-space moved and this scan is "
            + "pointed at nothing; fix the path, do not let the ratchet go vacuous.");
        return fragments;
    }
    ...
    private static readonly IReadOnlyDictionary<string, string> DeclaredKindGaps =
        ...
        {
            ["Enum"] = "enum declarations enter mappings as MEMBER types, not graph nodes; the descriptor's "
                       + "scalar vocabulary does not yet roll enum types — a declared blind spot for a "
                       + "later K-task, not a silent one"
        };
    ...
    public void TypeKind_covers_every_declaration_kind_the_case_space_enumerates()
    {
        ...
        Assert.True(unmapped.Count == 0,
            "The surface matrix's case-space declares kinds the descriptor can neither roll nor account "
            + "for. Add them to TypeKind (and to the generators + renderer), or declare the gap with a "
            + "reason in DeclaredKindGaps:\n  " + string.Join("\n  ", unmapped));

        var stale = DeclaredKindGaps.Keys.Where(k => !found.Contains(k)).Order(StringComparer.Ordinal).ToList();
        Assert.True(stale.Count == 0,
            "Declared kind gaps no longer present in the case-space — a stale excuse is an allowlist; "
            + "delete these rows:\n  " + string.Join("\n  ", stale));
    }
    ...
    public void MemberShape_covers_every_member_shape_the_case_space_enumerates()
    {
        ...
        var unmapped = found
            ...
            .ToList();
        Assert.True(unmapped.Count == 0,
            "The case-space declares member shapes the descriptor can neither roll nor account for. Add "
            + "them to MemberShape (and the renderer), or declare the gap with a reason in "
            + "DeclaredShapeGaps:\n  " + string.Join("\n  ", unmapped));
    ...
    private static readonly HashSet<string> DeclaredCollectionGaps = new(StringComparer.Ordinal)
    {
        ...
        "Queue", "Stack",
        "IEnumerable", "ICollection", "IList", "IReadOnlyCollection",
        "ISet", "IReadOnlySet",
        "ImmutableArray", "ImmutableList", "IImmutableList", "ImmutableHashSet", "IImmutableSet",
        // DictTargetKind (DictionaryConverter)
        "IDictionary", "IReadOnlyDictionary", "ImmutableDictionary", "IImmutableDictionary"
    };
    ...
    public void CollShape_covers_the_generator_taxonomy_or_declares_the_exact_blind_spot()
    {
        ...
        Assert.True(silent.Count == 0,
            "The generator supports collection targets the descriptor neither rolls nor declares — a "
            + "SILENT generator-bias blind spot (this is exactly how the IEnumerable<T> aliasing bug hid "
            + "from the fuzzers). Roll them in CollShape or declare them in DeclaredCollectionGaps:\n  "
            + string.Join("\n  ", silent));

        var overlap = DeclaredCollectionGaps.Where(rolled.Contains).Order(StringComparer.Ordinal).ToList();
        Assert.True(overlap.Count == 0,
            "These targets are BOTH rolled and declared-as-gap — remove the stale gap rows (shrink-only, "
            + "same commit as the CollShape addition):\n  " + string.Join("\n  ", overlap));
