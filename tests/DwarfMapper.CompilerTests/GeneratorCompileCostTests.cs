...
namespace DwarfMapper.CompilerTests
{
    ...
    public class GeneratorCompileCostTests
    {
        ...
        public GeneratorCompileCostTests(ITestOutputHelper output)
        {
            _output = output;
        }
        ...
        private static Corpus BuildCorpus()
        {
            ...
            var compilation = CSharpCompilation.Create("CostCorpusAsm",
                trees,
            ...
            return new Corpus(mapperCount,
                compilation,
                graphs,
                TrackedDriver().RunGenerators(compilation),
                parseWatch.Elapsed,
                emitWatch.Elapsed,
                run.GeneratedTrees.Length,
                run.Diagnostics.Length);
        }

        private static List<SyntaxTree> TreesFor(GraphSpec graph, int index)
        {
            return TypeGraphRenderer.Render(graph, false, Namespace(index))
                .Select(u => CSharpSyntaxTree.ParseText(u)).ToList();
        }

        private static string Namespace(int index)
        {
            return FormattableString.Invariant($"G{index}");
        }
        ...
        private static Dictionary<string, List<IncrementalStepRunReason>> OurSteps(GeneratorDriver driver)
        {
            ...
            {
                if (!name.StartsWith(OurStepPrefix, StringComparison.Ordinal) && !string.Equals(name, SourceOutputStep, StringComparison.Ordinal))
                {
                    continue;
                }

                if (!steps.TryGetValue(name, out var reasons))
                {
                    steps[name] = reasons = [];
                }
            ...
            if (steps.Count < MinimumTrackedSteps)
            {
                throw new InvalidOperationException(
                    $"only {steps.Count} tracked step(s) matched '{OurStepPrefix}*' or '{SourceOutputStep}', " + $"expected at least {MinimumTrackedSteps}. The pipeline's tracking names have changed and " + "these caching assertions are now asserting almost nothing — re-point the match rather than " + "lowering the floor.");
            }
        ...
        ///             assumed.</b>
        ///         Counting anything but <c>Cached</c> reports <c>DwarfMapperExtract</c> re-running
        ...
        {
            return reasons.Count(r => r is not (IncrementalStepRunReason.Cached or IncrementalStepRunReason.Unchanged));
        }
        ...
        private static KeyValuePair<string, List<IncrementalStepRunReason>> PerMapperStep(
            Dictionary<string, List<IncrementalStepRunReason>> steps,
            int mapperCount)
        {
            ...
            Assert.True(candidates.Count == 1,
                ...
                " among [" +
                string.Join(", ",
                    steps.Select(s => $"{s.Key}={s.Value.Count}")) +
                "]. Without it there is no evidence the " +
        ...
        public void Time_to_first_emit_is_recorded_never_gated()
        {
            ...
            _output.WriteLine(string.Create(CultureInfo.InvariantCulture,
                $"S3 generator compile cost: mappers={corpus.MapperCount} " + $"parse+compilation={corpus.ParseTime.TotalMilliseconds:F0} ms " + $"time-to-first-emit={corpus.TimeToFirstEmit.TotalMilliseconds:F0} ms " + $"{normalised} " + $"generated={corpus.GeneratedTreeCount} diagnostics={corpus.GeneratorDiagnosticCount}"));
            ...
            Assert.True(corpus.GeneratedTreeCount >= corpus.MapperCount,
                $"{corpus.MapperCount} mappers produced only {corpus.GeneratedTreeCount} generated file(s) — the " + "corpus is not exercising one emission per mapper, so the recorded time measures something " + "smaller than it claims.");
        ...
        public void An_identical_rerun_recomputes_nothing_at_corpus_scale()
        {
            ...
            foreach (var (name, reasons) in steps)
                Assert.True(Changed(reasons) == 0,
                    $"step '{name}' produced {Changed(reasons)} changed output(s) of {reasons.Count} on an " + $"IDENTICAL re-run over {corpus.MapperCount} mappers. A model that stopped being " + "value-equatable disables caching for every consumer, and at this scale that is the " + "whole build.");
        ...
        public void An_unrelated_edit_leaves_every_mapper_in_the_corpus_cached()
        {
            ...
            foreach (var (name, reasons) in steps)
                Assert.True(Changed(reasons) == 0,
                    $"step '{name}' produced {Changed(reasons)} changed output(s) of {reasons.Count} after an " + $"edit that touched no mapper at all, in a {corpus.MapperCount}-mapper compilation.");
            ...
            Assert.True(Changed(perMapper) == 0,
                $"step '{perMapperName}' produced {Changed(perMapper)} changed model(s) of {corpus.MapperCount} " + "after an edit that touched no mapper at all. Every keystroke anywhere in a consumer's " + "solution would re-emit their whole mapper surface.");
        }
        ...
        public void Editing_one_mapper_recomputes_a_constant_amount_of_work()
        {
            ...
            var target = corpus.Compilation.SyntaxTrees.First(t => t.ToString().Contains("namespace " + Namespace(0) + ";", StringComparison.Ordinal));
            ...
            Assert.True(Changed(perMapper) == ExpectedExtractRecomputes,
                $"editing ONE mapper re-extracted {Changed(perMapper)} of {corpus.MapperCount} " + $"(step '{perMapperName}'), expected exactly {ExpectedExtractRecomputes}. This number is " + "measured identical at 40 and at 1,000 mappers; if it now scales with the corpus, a " + "thousand-mapper solution re-analyses every mapper on every keystroke.");

            var sourceOutput = steps[SourceOutputStep];
            Assert.True(Changed(sourceOutput) == ExpectedSourceOutputRecomputes,
                $"editing ONE mapper re-emitted {Changed(sourceOutput)} of {sourceOutput.Count} source file(s), " + $"expected exactly {ExpectedSourceOutputRecomputes} — the edited mapper's own file plus the one " + "aggregate facade that lists them all. A third re-emission means some other mapper's output " + "depends on this one, and the dependency is O(N).");

            _output.WriteLine(string.Create(CultureInfo.InvariantCulture,
                $"S3 one-mapper edit at {corpus.MapperCount} mappers: " + $"{perMapperName} {Changed(perMapper)}/{perMapper.Count} re-ran, " + $"{SourceOutputStep} {Changed(sourceOutput)}/{sourceOutput.Count} re-emitted"));
        }
