...
namespace DwarfMapper.CompilerTests
{
    ...
    public class PinnedCorpusTests
    {
        ...
        {
            return PinnedCorpus.Rows.Select(r => new object[]
            {
                r.Id
            });
        }
        ...
        public void Every_pinned_row_is_valid_and_compiles_clean_or_refuses_loudly(string id)
        {
            ...
            if (row.ExpectedRefusalIds is not null)
            {
                ...
                Assert.True(result.RefusedLoudly,
                    $"corpus row '{row.Id}' no longer refuses — the pinned refusal contract changed; " + "re-measure and move this pin deliberately\n" + row.Graph.Describe());
            ...
            Assert.True(result.CompilationErrors.Length == 0,
                $"corpus row '{row.Id}' miscompiled silently: [" + string.Join(",", result.CompilationErrors.Select(e => e.Id).Distinct()) + "]\n" + row.Graph.Describe());
        }
        ...
        public void Partial_split_row_verdict_and_emission_are_file_order_independent()
        {
            ...
            Assert.False(forward.RefusedLoudly,
                "the split struct pair is expected to be accepted; a refusal here means the corpus row went " + "vacuous for the emission comparison — re-measure and move this pin deliberately");
