    ...
    public class DifferentialOracleTests
    {
        ...
        public void Sampled_graphs_compile_or_refuse_and_the_executed_map_agrees_with_the_naive_oracle()
        {
            ...
            PinnedSampling.Run(DeepPopulation.CompilerOracleSeeds,
                ...
                sample =>
                {
                    ...
                    Assert.True(result.CompilationErrors.Length == 0,
                        "seed-replayable silent miscompilation: the generators were silent but the output has [" + string.Join(",", result.CompilationErrors.Select(e => e.Id).Distinct()) + "]\n--- graph ---\n" + graph.Describe() + "\n--- units ---\n" + string.Join("\n--- next unit ---\n", units) + "\n--- generated ---\n" + result.GeneratedSource);
            ...
            Assert.True(executed > 0,
                $"no sampled graph executed ({refused} refusals, 0 executions) — the sampled space has " + "drifted into the refusal grammar and the differential leg is vacuous.");
        ...
        public void Accepted_pinned_corpus_rows_agree_with_the_naive_oracle(string id)
        {
            ...
            RunOracleLeg(assembly, row.Graph, 12345, result.GeneratedSource);
        }
        ...
        public void Null_source_collection_maps_to_empty_on_both_sides_per_the_documented_default()
        {
            var graph = new GraphSpec(
                [
                    new NodeSpec("S0",
                        TypeKind.Class,
                        [new MemberSpec("M0_0", "int", false, MemberShape.AutoProp, CollShape.List, 1)],
                        null),
                    new NodeSpec("S1",
                        TypeKind.Class,
                    ...
                    new NodeSpec("D0",
                        TypeKind.Class,
                        [new MemberSpec("M0_0", "int", false, MemberShape.AutoProp, CollShape.List, 3)],
                        null),
                    new NodeSpec("D1",
                        TypeKind.Class,
                        [new MemberSpec("M1_0", "int", false, MemberShape.AutoProp, CollShape.None, null)],
            ...
            var expected = ReflectionOracle.NaiveMap(
                source,
                assembly.GetType("T.D0")!,
                ReflectionOracle.OracleOptions.DocumentedDefaults);
        ...
        public void I5_nullable_struct_element_lifts_null_to_null_in_every_wrapper(
            CollShape wrapper,
            TypeKind sourceElement,
            TypeKind destElement)
        {
            var graph = new GraphSpec(
                [
                    new NodeSpec("S0",
                        TypeKind.Class,
                        [new MemberSpec("M0_0", "int", true, MemberShape.AutoProp, wrapper, 1)],
                        null),
                    new NodeSpec("S1",
                        sourceElement,
                        [new MemberSpec("M1_0", "int", false, MemberShape.AutoProp, CollShape.None, null)],
                    ...
                    new NodeSpec("D0",
                        TypeKind.Class,
                        [new MemberSpec("M0_0", "int", true, MemberShape.AutoProp, wrapper, 3)],
                        null),
                    new NodeSpec("D1",
                        destElement,
                        [new MemberSpec("M1_0", "int", false, MemberShape.AutoProp, CollShape.None, null)],
            ...
            Assert.True(result.CompilationErrors.Length == 0,
                "I5 regression — the element loop emitted code that does not compile: [" + string.Join(",", result.CompilationErrors.Select(e => e.Id).Distinct()) + "]");
            Assert.NotNull(assembly);
            Assert.Contains(".HasValue ? (global::T.D1?)__DwarfMap_Obj_",
                result.GeneratedSource,
        ...
        private static object TwoElementPayload(Type collectionType, object nonNull)
        {
            ...
            var add = concrete.GetMethod("Add") ?? throw new InvalidOperationException($"no Add on payload type {concrete}");
        ...
        {
            var sourceType = assembly.GetType("T." + graph.RootSource) ?? throw new InvalidOperationException($"no type T.{graph.RootSource} in emitted assembly");
            var destType = assembly.GetType("T." + graph.RootDest) ?? throw new InvalidOperationException($"no type T.{graph.RootDest} in emitted assembly");
