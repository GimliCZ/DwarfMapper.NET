...
namespace DwarfMapper.CompilerTests
{
    ...
    public class MetamorphicTests
    {
                ...
                sample =>
                {
                    ...
                    var reversed = graph with
                    {
                        Nodes = graph.Nodes
                            .Select(n => n with
                            {
                                Members = n.Members.Reverse().ToList()
                            })
                            .ToList()
                    };
        ...
        private static HashSet<int> ReachableFrom(GraphSpec graph, string rootName)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (var member in node.Members)
                    if (member.NestedRef is { } j && seen.Add(j))
                    {
                        queue.Enqueue(j);
                    }

                if (node.BaseRef is { } b && seen.Add(b))
                {
                    queue.Enqueue(b);
                }
            }
        ...
        private static GraphSpec ListifySetsOfStructuralElements(
            GraphSpec graph,
            Func<NodeSpec, bool> elementIsStructural)
        {
            return graph with
            {
                Nodes = graph.Nodes
                    ...
                    {
                        Members = n.Members
                            .Select(m => m is { Coll: CollShape.HashSet, NestedRef: { } target } && elementIsStructural(graph.Nodes[target])
                                ? m with
                                {
                                    Coll = CollShape.List
                                }
        ...
        private static GraphSpec ClearNestedNullability(GraphSpec graph)
        {
            return graph with
            {
                Nodes = graph.Nodes
                    ...
                    {
                        Members = n.Members
                            .Select(m => m.NestedRef is not null
                                ? m with
                                {
                                    Nullable = false
                                }
        ...
        private static GraphSpec Rekind(GraphSpec graph, TypeKind kind)
        {
            ...
            {
                Nodes = graph.Nodes.Select(n => n with
                {
                    Kind = kind
                }).ToList()
            };
        ...
        ///     <list type="number">
        ///         <item>
        ///             the cardinality really is representation-dependent — 2 under Class, 1 under Record and
        ///             record struct;
        ///         </item>
        ///         <item>
        ///             it is a POPULATION difference, not a mapping one — the SOURCE set already holds 2 vs 1
        ///             before the mapper is invoked, and the mapped set matches its own source every time, so
        ///             the mapper is exonerated by measurement rather than by assertion;
        ///         </item>
        ...
        public void Set_shaped_members_are_representation_dependent_by_design()
        {
            ...
            foreach (var (kind, cardinality) in expected)
            {
                ...
                Assert.True(sourceCount == cardinality,
                    $"the {kind} SOURCE set holds {sourceCount}, expected {cardinality} — the I11 pin rests on " + "the populator's ~25% nullable-scalar bias nulling both elements' long? member; if that " + "bias moved, re-measure the pinned seed rather than relaxing this number.");
                Assert.Contains(
                    "r.M0_1.#=" + cardinality.ToString(CultureInfo.InvariantCulture),
                    outcome,
        ...
        public void Set_shaped_members_are_unmapped_member_dependent_by_design()
        {
            ...
            Assert.True(baselineCount == 1 && fattenedCount == 2,
                $"the I11/MR-2 pin expects source sets of 1 (baseline) and 2 (fattened), measured " + $"{baselineCount} and {fattenedCount} — re-measure before moving the pin.");
        ...
        private static (int SourceCount, string Outcome) RunRootSet(GraphSpec graph, int seed)
        {
            ...
            Assert.True(result.CompilationErrors.Length == 0,
                "the I11 pins compile clean: [" + string.Join(",", result.CompilationErrors.Select(e => e.Id).Distinct()) + "]");
            Assert.NotNull(assembly);

            var sourceType = assembly.GetType("T." + graph.RootSource) ?? throw new InvalidOperationException($"no type T.{graph.RootSource}");
            var source = ReflectionOracle.Populate(sourceType, seed);
            var set = sourceType.GetProperty("M0_1")?.GetValue(source) ?? throw new InvalidOperationException("the I11 pins declare their set member as M0_1");
            var count = ((IEnumerable)set).Cast<object?>().Count();
        ...
        private static string Outcome(GraphSpec graph, int seed)
        {
            ...
            Assert.True(result.CompilationErrors.Length == 0,
                "seed-replayable silent miscompilation inside a metamorphic variant: the generators were " + "silent but the output has [" + string.Join(",", result.CompilationErrors.Select(e => e.Id).Distinct()) + "]\n--- graph ---\n" + graph.Describe() + "\n--- units ---\n" + string.Join("\n--- next unit ---\n", units) + "\n--- generated ---\n" + result.GeneratedSource);
            Assert.NotNull(assembly);
        ...
        private void AssertNotVacuous(int executed, int refused, string relation)
        {
            Assert.True(executed > 0,
                $"{relation}: no sampled graph executed ({refused} refusals, 0 executions) — the sampled " + "space has drifted into the refusal grammar and the relation is vacuous.");
