...
internal static class ReflectionOracle
{
    ...
    private static object? CopyValue(object? source, Type destType, OracleOptions options, int depth)
    {
        ...
        if (IsScalar(destType)) return source; // mirrored scalar vocabulary — no conversions to model
        ...
        return Instantiate(destType, (name, memberType) =>
    ...
    private static object? ReadSourceMember(object source, string name)
    {
        ...
        if (property is not null && property.CanRead) return property.GetValue(source);
        var field = type.GetField(name, BindingFlags.Public | BindingFlags.Instance);
        if (field is not null) return field.GetValue(source);
    ...
    private static bool TryGetCollectionShape(
        Type type, out CollectionShapeKind shape, out Type elementType, out Type? valueType)
    {
        shape = default;
    ...
    private static object MaterializeSequence(
        CollectionShapeKind shape, Type declaredType, Type elementType, object?[] items)
    {
    ...
    private static object EmptyCollection(
        CollectionShapeKind shape, Type declaredType, Type elementType, Type? valueType)
    {
        return shape switch
        {
    ...
    private static object ScalarFor(Type type, int key)
    {
        ...
        if (type == typeof(int)) return h;
        if (type == typeof(long)) return h * 2654435761L;
        if (type == typeof(string)) return "s" + h.ToString(CultureInfo.InvariantCulture);
        if (type == typeof(decimal)) return new decimal(h) / 100m;
        if (type == typeof(bool)) return (h & 1) == 0;
        if (type == typeof(byte)) return (byte)h;
        if (type == typeof(double)) return h * 0.5;
        if (type == typeof(Guid))
        {
    ...
    private static void DemandDepth(int depth, Type at)
    {
        ...
        {
            throw new InvalidOperationException(
                $"oracle recursion exceeded depth {MaxDepth} at {at} — the sampled graph is not the DAG "
                + "GraphSpec rule V2 guarantees; fix the upstream invariant, do not raise this bound");
        }
    }
