...
namespace DwarfMapper.CompilerTests
{
    ...
    public class TypeGraphSmokeTests
    {
        ...
        public void Sampled_graphs_compile_silently_clean_or_refuse_loudly()
        {
            ...
            PinnedSampling.Run(DeepPopulation.CompilerGraphSmokeSeeds,
                TypeGraphGen.MirroredPair(),
            ...
            Assert.True(accepted > 0,
                $"the whole sample was refused ({refused} refusals, 0 accepts) — the sampled space has " + "drifted into the refusal grammar and the compile-clean invariant is vacuous.");
        ...
        public void Sampled_graphs_with_a_projection_compile_silently_clean_or_refuse_loudly()
        {
            ...
            PinnedSampling.Run(DeepPopulation.CompilerProjectionSmokeSeeds,
                TypeGraphGen.MirroredPair(),
                graph => graph.Describe(),
                graph =>
                {
            ...
            Assert.True(accepted > 0,
                $"every sampled projection was refused ({refused} refusals, 0 accepts) — the projection's " + "refusal grammar now covers the whole sampled space and this leg proves nothing.");
            _output.WriteLine(string.Create(CultureInfo.InvariantCulture,
                $"projection sampled {accepted + refused}: {accepted} accepted (compiled clean), " + $"{refused} refused loudly"));
        }
    }
}
