    ...
    public class DiagnosticCoverageRatchetTests
    {
        ...
        public void Every_diagnostic_has_a_case_file_or_predates_this_project()
        {
            var unaccounted = AllDescriptorIds()
                .Where(id => !NegativeCase.CoveredIds.Contains(id, StringComparer.Ordinal) && !PredatesThisProject.Contains(id, StringComparer.Ordinal))
                .ToList();
        ...
        public void The_exemption_list_is_an_exactly_pinned_bounded_population()
        {
            Assert.True(PredatesThisProject.Length <= PredatesThisProjectPin,
                $"PredatesThisProject holds {PredatesThisProject.Length} entries, above the pinned " + $"{PredatesThisProjectPin}. This population may only shrink: a new diagnostic arrives with a " + "case file or it does not arrive, and an id created after 2026-08-12 cannot 'predate this " + "project' whatever the commit message says.");

            Assert.True(PredatesThisProject.Length >= PredatesThisProjectPin,
                $"PredatesThisProject holds {PredatesThisProject.Length} entries, under the pinned " + $"{PredatesThisProjectPin}. An entry was retired — good — but the pin must move in the same " + "commit, or the room it freed silently funds a swap-in the exact pin exists to prevent.");
            ...
            Assert.True(beyondHorizon.Count == 0,
                "PredatesThisProject names id(s) newer than the project's creation date:\n  " + string.Join("\n  ", beyondHorizon) + $"\n\nEvery id after {ExemptionIdHorizon} was minted after 2026-08-12 and therefore does not " + "predate this project. It needs a case file — there is no list to add it to.");
            ...
            Assert.True(duplicated.Count == 0,
                "PredatesThisProject lists id(s) twice: " + string.Join(", ", duplicated) + ". A duplicate makes the pinned count claim coverage of an id the list does not actually " + "hold room for.");
        }
        ...
        public void The_exemption_list_does_not_name_a_diagnostic_that_now_has_a_case()
        {
            ...
            Assert.True(both.Count == 0,
                "These diagnostics have a case file AND an exemption:\n  " + string.Join("\n  ", both) + "\n\nRemove them from PredatesThisProject — that is the ratchet tightening, and it is the only " + "way the list ever gets shorter.");
        }
    }
}
