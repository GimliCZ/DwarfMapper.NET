...
public class NegativeCaseTests
{
    ...
    private static NegativeCase Case(string name) =>
        NegativeCase.All.Single(c => string.Equals(c.Name, name, StringComparison.Ordinal));
    ...
    {
        var testCase = Case(name);
        if (testCase.ExpectedMessages.Length == 0) return;
        ...
        foreach (var (id, substring) in testCase.ExpectedMessages)
        {
            ...
            Assert.True(rendered.Exists(m => m.Contains(substring, StringComparison.Ordinal)),
                ...
                + "\n\nThis is the half of the diagnostic that tells the reader what to write instead. "
                + "Reword freely; keep the way out.");
        }
    }
    ...
    {
        var testCase = Case(name);
        if (testCase.ExpectedCompilerErrors.Length == 0) return;
