    ...
    string Source)
{
    ...
    private static ImmutableArray<NegativeCase> Load()
    {
        ...
        if (cases.Length == 0)
            throw new InvalidOperationException(
                "No case files were embedded. Check that Cases\\**\\*.cs is an EmbeddedResource "
                + "(and removed from Compile) in DwarfMapper.NegativeCases.csproj.");
    ...
    private static NegativeCase Parse(string name, string text)
    {
        ...
        foreach (var raw in text.Split('\n'))
        {
            ...
            if (TryTake(body, "CASE:", out var t)) title = t;
            else if (TryTake(body, "WHY:", out var w)) why = w;
            else if (TryTake(body, "EXPECT-CS:", out var cs)) compilerErrors.AddRange(SplitIds(cs));
            else if (TryTake(body, "EXPECT-MESSAGE ", out var m)) messages.Add(ParseMessage(name, m));
        ...
        if (!sawExpect)
            throw new InvalidOperationException(
                $"Case '{name}' has no `// EXPECT:` line. Every case must declare the exact set of DWARF ids "
                + "it provokes (or `none`) — an undeclared case asserts nothing.");

        if (string.IsNullOrWhiteSpace(why))
            throw new InvalidOperationException(
                $"Case '{name}' has no `// WHY:` line. State why this shape is red in the reader's terms; a "
                + "case nobody can explain is a case nobody can maintain.");
