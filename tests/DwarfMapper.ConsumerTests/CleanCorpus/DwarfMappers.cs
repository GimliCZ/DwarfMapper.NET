...
[DwarfMapper]
    ...
    public partial class LoanMappers
    {
        ...
        private static bool IsClosed(Loan source)
        {
            return source.ReturnedOn.HasValue;
        }

        private static string DescribeFine(decimal accrued)
        {
            return Before.Fines.Describe(accrued);
        }
    }
    ...
    [MapProperty<CatalogueItem, CatalogueItemDto>(
        nameof(CatalogueItem.Publisher) + "." + nameof(Publisher.Name),
        nameof(CatalogueItemDto.PublisherName))]
