...
namespace ConsumerTests.Host
{
    ...
    public sealed class ConsumerSurfaceTests
    {
        ...
        public void The_host_does_not_reference_the_provider_assemblies()
        {
            ...
            Assert.False(ProviderLoader.HostReferencesProviders(),
                "The host has acquired a compile-time reference to a provider assembly. That removes the whole " + "point of this project — the ambient registry must be the only thing that can resolve these " + "maps, because that is the condition under which Round 18's 47-site gap existed.");
        }
        ...
        public void A_method_declared_ICollection_but_returning_List_still_resolves()
        {
            ...
            ICollection<Customer> declaredAsInterface = new List<Customer>
            {
                new()
                {
                    Id = 3
        ...
        public void Two_providers_configuring_the_same_nested_pair_do_not_silently_share_behaviour()
        {
            ...
            var patchTarget = new AddressDto
            {
                City = "kept",
                Postcode = "kept"
            };
            ...
            Assert.True(DwarfMapperRegistry.IsAmbiguous(typeof(Address), typeof(AddressDto)),
                "Two assemblies register Address -> AddressDto under different options. That MUST be reported as " + "ambiguous rather than resolved silently — a consumer picking up whichever module initializer " + "ran first is exactly the non-determinism this library exists to refuse.");
        }
        ...
        public void An_internal_constructor_is_reachable_via_InternalsVisibleTo_and_AllowNonPublic()
        {
            ...
            var entry = (AuditEntry)DwarfMapperRegistry.Map(new AuditRow
                {
                    Id = 5
                },
                typeof(AuditEntry));
        ...
        public void Every_registration_in_the_container_resolves()
        {
            ...
            services.AddSingleton(DwarfMapperFacade.Instance);
            ...
            Assert.Equal(2,
                provider.GetRequiredService<CustomerService>().ToDtos(
                [
                    new Customer
                    {
                        Id = 1
        ...
        public void The_default_writes_the_annotated_text()
        {
            ...
            Assert.Equal("Next-Day",
                Mapper().Map<ShipmentDoc>(new Shipment
                {
                    Channel = DispatchChannel.NextDay
                }).Channel);
        }
        ...
        public void EnumStringSource_Identifier_writes_the_member_name_from_another_assembly()
        {
            ...
            Assert.Equal("NextDay",
                Mapper().Map<ShipmentLog>(new Shipment
                {
                    Channel = DispatchChannel.NextDay
                }).Channel);
        }
        ...
        public void A_member_named_for_a_keyword_maps_across_the_assembly_boundary()
        {
            ...
            var dto = Mapper().Map<KeywordDto>(new KeywordRow
            {
                @class = "wizard",
                @event = 3
            });
    ...
    public sealed class CustomerService(IDwarfMapper mapper)
    {
        ...
        {
            return mapper.Map<ICollection<CustomerDto>>(customers.ToList());
        }
    }
    ...
    public sealed class CommandService(IDwarfMapper mapper)
    {
        ...
        {
            return mapper.Map<List<CommandDto>>(commands);
        }
    }
}
