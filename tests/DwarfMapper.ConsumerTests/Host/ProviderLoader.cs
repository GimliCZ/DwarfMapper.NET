        ...
        internal static void EnsureLoaded()
        {
            lock (Gate)
            {
                ...
                foreach (var name in new[]
                         {
                             "ConsumerTests.ProviderA", "ConsumerTests.ProviderB"
                         })
                {
                    var path = Path.Combine(AppContext.BaseDirectory, name + ".dll");

                    Assert.True(File.Exists(path),
                        $"Provider assembly '{name}.dll' was not copied next to the host. The ProjectReference " + "must stay (with ReferenceOutputAssembly=\"false\") so it is BUILT and copied without " + "being visible to the compiler.");
