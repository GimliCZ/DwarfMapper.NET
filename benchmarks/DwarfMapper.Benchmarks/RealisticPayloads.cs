        ...
        public static void AssertRealistic<T>(IReadOnlyList<T> sample, string label)
        {
            ...
            if (nulls == 0 && edges == 0)
            {
                throw new InvalidOperationException(
                    $"{label}: payload contains neither a null nor a boundary value across {sample.Count} instances " + $"of {typeof(T).Name}. The generator has regressed to a uniform distribution, which is exactly " + "what this payload source replaced — investigate before trusting any number from this run.");
            }
        }
    }
}
