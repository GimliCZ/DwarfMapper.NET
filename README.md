...
Reference the **one** package — it bundles the attributes, the Roslyn source generator, and the IDE code fixes, so there is no separate analyzer wiring:

```xml
<PackageReference Include="DwarfMapper" Version="1.0.0-rc.1" />
```

> Pre-release: not yet on nuget.org. Until it ships, reference the project/source directly — see [Status](#status). Target framework must be `net10.0`.
...
If `CustomerDto` gains a `Region` property and you don't map or ignore it, **the build fails** with a diagnostic pointing at the unmapped member. **This is deliberate, not nagging** — the single most common real-world mapping bug is forgetting to wire up a new property, and here it can't ship. The fix is one attribute: map it, or `[MapIgnore(nameof(CustomerDto.Region))]` to drop it on purpose (audited, not silent). That's the whole bargain.
