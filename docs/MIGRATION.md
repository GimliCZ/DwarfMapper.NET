...
| `query.ProjectTo<Dto>(cfg)` | `partial IQueryable<Dto> Project(IQueryable<S>)` | **DIVERGENT (deliberately minimal)**: only direct members + renames + `[MapIgnore]`; a member needing conversion → `DWARF019` |
...
| `ProjectToType<Dst>()` (EF) | `partial IQueryable<Dst> Project(IQueryable<S>)` | minimal/translatable only (`DWARF019`) |
...
| `IQueryable` projection | `partial IQueryable<Dst> Project(…)` | minimal/translatable (`DWARF019`) |
