...
| `NullCollections` | `NullCollectionStrategy` | `AsEmpty` | Null source collection → `AsEmpty` (never throws) or `AsNull` (propagates null **only** when the target member is nullable — a nullable reference or a nullable value-type collection like `ImmutableArray<T>?`; a non-nullable target silently degrades to `AsEmpty`). The same answer holds through `Project`, degrade included: the projection emits `__s.M == null ? new List<T>() : …` (or `Array.Empty<T>()` / `Enumerable.Empty<T>()`, per target kind) rather than answering differently from `Map`. One bound: the projection guards only a source member that **may** be null (nullable-annotated or nullable-oblivious), so a member declared non-nullable that is null anyway throws from `Project` where `Map` returns empty — guarding unconditionally would put a null check into every correctly-annotated query for input that violates its own annotation. |
...
| `ImplicitConversions` | `bool` | `true` | `true`: non-lossless conversions are applied but surface `DWARF038` (a Warning for the lossy kinds — numeric narrowing/sign-change, `string ↔ T` parse/format and cross-category numeric; Info for a user-defined explicit operator). `false`: they become build errors (Mapperly-strict). **The same answer at every endpoint, `Project` included**: a lossy conversion that is nonetheless implicit in C# (`long → double`) is reported from the projection too, and at the same severity, so the option decides whether the build breaks rather than which method you happened to call. A `Nullable<>` wrapper changes nothing — `long? → double?` is the same question as `long → double`, and same-category widening (`int → long`, wrapped or not) stays silent because it loses nothing. |
