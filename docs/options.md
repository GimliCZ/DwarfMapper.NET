...
| `NullStrategy` | `NullStrategy` | `Throw` | Nullable-value source → **non-nullable** target when null: `Throw`, or `SetDefault` (use the destination default). It governs that case only: when the target **can hold the null** — a `Nullable<T>` or a nullable-annotated reference — the null is *lifted* (null in, null out) regardless of this setting, including across a nested pair whose two sides are declared with different kinds, and per element of a mapped collection or dictionary. |
