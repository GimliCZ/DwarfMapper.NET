...
### Task 16: Consumer proof — migrate one real mapper + verify

**Files:** a downstream consumer's mapper (not part of this repo). Verification only — no commit here.

**Interfaces:** none (integration verification).

- [ ] **Step 1: Build the Release generator + runtime and sync them into the consumer**

```bash
dotnet build src/DwarfMapper.Generator/DwarfMapper.Generator.csproj -c Release --nologo -v quiet
dotnet build src/DwarfMapper/DwarfMapper.csproj -c Release --nologo -v quiet
```
Both assemblies must be synced — the new `MapConfig` type lives in the runtime `DwarfMapper.dll`, not just the generator.

- [ ] **Step 2: Rewrite one attribute-heavy mapper to use `MapConfig`**

Pick a mapper whose config is dominated by `[MapProperty<…>(nameof(...), nameof(...))]` and replace those attributes with a `private static void Cfg(MapConfig<S, T> c) => c.Map(...)…` method using selector lambdas.

- [ ] **Step 3: Build + test the consumer**

Expected: 0 build errors and a fully green consumer suite. This proves the feature works end-to-end in a real codebase and that the migrated mapper behaves identically to its attribute form.

- [ ] **Step 4: Revert the consumer edit** — keep only the DwarfMapper.NET repo changes.

- [ ] **Step 5: Final full DwarfMapper suite green**

Run: `dotnet test DwarfMapper.NET.sln -c Debug --nologo`
...
## Post-MapConfig follow-up: repo strictness pass (separate plan, runs AFTER Task 16)

User decision (2026-07-01): finish all 16 MapConfig tasks first, then tighten strictness. This section is the committed scope so it is not lost; it becomes its own plan.

The repo is already at maximum analyzer strictness (`AnalysisMode=All`, `TreatWarningsAsErrors`, `EnforceCodeStyleInBuild`), so unused **locals/local-consts** (CS0168/CS0219) already fail. The gap is the **IDE** "unused member" rules, which default to *suggestion*:

1. **Raise `IDE0051` (unused private member), `IDE0052` (unread private member), `IDE0060` (unused parameter) to `error`** in the root `.editorconfig`, then sweep every finding across the solution.
2. **BLOCKING PREREQUISITE — do this step first, before flipping IDE0051 to error:** the MapConfig **convention method** is a `private static` method that is *never called*, so `IDE0051=error` would fail the build **in every consumer project** . Fix: after the generator collects a config method (Task 3), have it **emit a `nameof` reference to that method into the generated partial** (e.g. an assignment/field like `_ = nameof(<ConfigMethod>);`), which marks the user's method "used" for IDE0051 **without executing it** (generated files carry the `<auto-generated>` header so they are themselves exempt from IDE analysis, but a reference from them still counts as a use of the user's method). Test that a `[DwarfMapper]` class with only a MapConfig method compiles clean under `IDE0051=error`.
3. **`nameof` sweep where feasible:** runtime library (`src/DwarfMapper`), tests, and consumer code may use `nameof` for member/parameter-name strings and should. The **generator** (`src/DwarfMapper.Generator`, netstandard2.0) *cannot* `nameof` the runtime attribute types — that is why `KnownNames` holds string literals; leave those. Do check for generator self-references (its own types/members) that could use `nameof`.
4. Keep the full suite green and re-verify the downstream consumer, as in Task 16.
