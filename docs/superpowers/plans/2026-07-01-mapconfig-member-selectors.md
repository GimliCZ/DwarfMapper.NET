...
2. **BLOCKING PREREQUISITE — do this step first, before flipping IDE0051 to error:** the MapConfig **convention method** is a `private static` method that is *never called*, so `IDE0051=error` would fail the build **in every consumer project** (incl. FusedChat). Fix: after the generator collects a config method (Task 3), have it **emit a `nameof` reference to that method into the generated partial** (e.g. an assignment/field like `_ = nameof(<ConfigMethod>);`), which marks the user's method "used" for IDE0051 **without executing it** (generated files carry the `<auto-generated>` header so they are themselves exempt from IDE analysis, but a reference from them still counts as a use of the user's method). Test that a `[DwarfMapper]` class with only a MapConfig method compiles clean under `IDE0051=error`.
...
4. Keep the full suite green and re-verify FusedChat, as in Task 16.
