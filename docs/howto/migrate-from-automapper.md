...
| `.ForCtorParam("p", o => o.MapFrom(s => s.Y))` | `[MapProperty(nameof(S.Y), "p")]` (targets the ctor param by name). The source may be a **dotted path** — `[MapProperty("Window.Start", "start")]` — on the same terms as a member target (`DWARF043` unknown segment, `DWARF044` nullable hop), and `Use=` converts the leaf. The same attribute binds the same parameter under `Project`, so a `ForCtorParam` + `ProjectTo` pair converts once, not twice (`Use=` is the exception — a converter is `DWARF028` in a projection). |
