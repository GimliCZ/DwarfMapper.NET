...
namespace DwarfMapper.DocTooling
{
    ...
    public static class DocSnippetInjector
    {
        ...
        public static InjectionResult Inject(
            string markdown,
            IReadOnlyDictionary<string, SnippetRegion> regions,
            string docPath)
        {
            ...
            while (i < lines.Length)
            {
                ...
                if (i <= previous)
                {
                    throw new InvalidOperationException(
                        $"DocSnippetInjector stopped advancing at {docPath}:{i + 1} — injector bug, not a " + "document error. Every branch of the injection loop must advance the line index.");
                }
                ...
                if (!regions.TryGetValue(id, out var region))
                {
                    throw new DocToolingException(
                        $"{docPath}:{i + 1}: snippet '{id}' is referenced here but no sample defines it. Add a " + $"'// <snippet: {id}>' region to a file under samples/, or fix the id.");
                }
        ...
        private static int FindClose(string[] lines, int from, string docPath, string id, int openLine)
        {
            ...
            throw new DocToolingException(
                $"{docPath}:{openLine}: snippet '{id}' is never closed with '{CloseMarker}'. Refusing to treat " + "the rest of the file as its body.");
        }
    }
}
