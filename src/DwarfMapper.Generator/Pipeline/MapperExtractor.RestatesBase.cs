...
internal static partial class MapperExtractor
{
        ...
        List<DiagnosticInfo> diagnostics)
    {
        ...
        {
            if (GeneratedNames.IsAnySynthesized(m.MethodName)) continue;
            var src = ResolveByFqn(classSymbol, m.ParameterTypeFullName);
            var tgt = ResolveByFqn(classSymbol, m.ReturnTypeFullName);
            if (src is not null && tgt is not null) declaredPairs.Add((src, tgt, m));
        }
        ...
        foreach (var (derivedSrc, derivedTgt, overrides, loc) in declarations)
        {
            ...
            var derived = declaredPairs.Find(p =>
                SymbolEqualityComparer.Default.Equals(p.Src, derivedSrc)
                && SymbolEqualityComparer.Default.Equals(p.Tgt, derivedTgt));

            if (derived.Model is null)
            {
                diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.RestatesBaseUnresolved, location,
                    $"[RestatesBase<{derivedSrc.ToDisplayString()}, {derivedTgt.ToDisplayString()}>] names a "
                    + "pair this mapper does not declare. Add the pair it is about "
                    + $"([GenerateMap<{derivedSrc.ToDisplayString()}, {derivedTgt.ToDisplayString()}>] or a "
                    + "partial method for it), or fix the type arguments."));
                continue;
            }
            ...
            if (!TryFindBasePair(declaredPairs, derivedSrc, derivedTgt, out var basePair, out var ambiguous))
            {
                diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.RestatesBaseUnresolved, location,
                    ambiguous is null
                        ...
                        : $"[RestatesBase<{derivedSrc.ToDisplayString()}, {derivedTgt.ToDisplayString()}>] finds "
                          + $"more than one equally-close base pair ({ambiguous}). Only base CLASSES are walked, "
                          + "so this means two candidates sit at the same depth; declare the pair you mean and "
                          + "remove the other, or drop the attribute."));
                continue;
            }
    ...
    private static bool TryFindBasePair(
        ...
        ITypeSymbol derivedSrc, ITypeSymbol derivedTgt,
        ...
        out string? ambiguous)
    {
        ...
        foreach (var candidate in declaredPairs)
        {
            ...
            if (srcDepth <= 0 || tgtDepth <= 0) continue; // not a STRICT base on both sides
    ...
    private static int BaseDepth(ITypeSymbol derived, ITypeSymbol candidate)
    {
        ...
        {
            depth++;
            if (SymbolEqualityComparer.Default.Equals(t, candidate)) return depth;
        }
    ...
    private static void ReportDrift(
        MapMethodModel derived, MapMethodModel baseModel,
        ITypeSymbol derivedTgt, ITypeSymbol baseTgt,
        ...
        LocationInfo? location, List<DiagnosticInfo> diagnostics)
    {
        ...
        foreach (var baseMember in baseModel.Members)
        {
            ...
            if (!SameMemberType(baseTgt, derivedTgt, baseMember.TargetName)) continue;
            ...
            if (derivedMember != baseMember) drifted.Add(baseMember.TargetName);
        }

        if (drifted.Count == 0 && dropped.Count == 0) return;

        var parts = new List<string>();
        if (drifted.Count > 0) parts.Add("maps " + Join(drifted) + " differently from the base pair");
        if (dropped.Count > 0) parts.Add("does not map " + Join(dropped) + ", which the base pair maps");

        var first = drifted.Count > 0 ? drifted[0] : dropped[0];

        diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.RestatedBaseDrift, location,
            ...
            + "[RestatesBase(Overrides = new[] { \"" + first
    ...
    private static bool SameMemberType(ITypeSymbol baseTgt, ITypeSymbol derivedTgt, string memberName)
    {
        ...
        return baseType is not null && derivedType is not null
                                    && SymbolEqualityComparer.Default.Equals(baseType, derivedType);
    }
    ...
    private static ITypeSymbol? ResolveByFqn(INamedTypeSymbol classSymbol, string fqn)
    {
        ...
        {
            if (attr.AttributeClass is not { } ac || ac.TypeArguments.Length != 2) continue;
            foreach (var candidate in ac.TypeArguments)
                if (string.Equals(candidate.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), fqn,
        ...
        {
            if (m.MethodKind != MethodKind.Ordinary || m.Parameters.Length != 1) continue;
            if (string.Equals(m.Parameters[0].Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), fqn,
                    StringComparison.Ordinal))
                return m.Parameters[0].Type;
            if (string.Equals(m.ReturnType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), fqn,
                    StringComparison.Ordinal))
                return m.ReturnType;
        }
        ...
        ReadRestatesBase(INamedTypeSymbol classSymbol)
    {
        ...
        foreach (var attr in classSymbol.GetAttributes())
        {
            if (attr.AttributeClass is not { Name: KnownNames.RestatesBase } ac
            ...
            {
                if (named.Key != "Overrides") continue;
