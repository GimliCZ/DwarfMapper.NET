...
namespace DwarfMapper.Generator.Pipeline
{
    ...
    ///             <term>string → T</term>
    ///             <description>
    ///                 when T : IParsable&lt;T&gt; (and T is not an enum).
    ...
    ///                 Throws <c>FormatException</c> / <c>OverflowException</c> on bad input.
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <term>T → string</term>
    ///             <description>
    ///                 when T : IFormattable (and T is not an enum, not string itself).
    ///                 Emits <c>v.ToString(null, global::System.Globalization.CultureInfo.InvariantCulture)</c>.
    ///                 Uses InvariantCulture for culture-independence (e.g. decimal separator is always '.').
    ///             </description>
    ///         </item>
    ///     </list>
    ...
    internal static class ParsableConverter
    {
            ...
            Dictionary<string, SynthesizedMethod> synthesized)
        {
            ...
            if (src.SpecialType == SpecialType.System_String && tgt.TypeKind != TypeKind.Enum && TypeInterfaces.ImplementsIParsable(compilation, tgt))
            {
            ...
            {
                var isBoolOrChar = src.SpecialType is SpecialType.System_Boolean or SpecialType.System_Char;
        ...
        private static string AddStringToT(Dictionary<string, SynthesizedMethod> synth, ITypeSymbol tgt)
        {
            ...
            if (!synth.ContainsKey(name))
            {
                ...
                {
                    code =
                        $"    private static {fqTgt} {name}(string v) => {fqTgt}.Parse(v, global::System.Globalization.CultureInfo.InvariantCulture, global::System.Globalization.DateTimeStyles.RoundtripKind);\n";
                }
                ...
                {
                    code =
                        $"    private static {fqTgt} {name}(string v) => {fqTgt}.Parse(v, global::System.Globalization.CultureInfo.InvariantCulture);\n";
                }
                else
        ...
        private static bool HasPublicParseWithFormatProvider(ITypeSymbol t)
        {
            ...
            return t.GetMembers("Parse").OfType<IMethodSymbol>().Any(m =>
                m.IsStatic && m.DeclaredAccessibility == Accessibility.Public && m.Parameters.Length == 2 && m.Parameters[0].Type.SpecialType == SpecialType.System_String && (m.Parameters[1].Type.ToDisplayString() == "System.IFormatProvider" || m.Parameters[1].Type.ToDisplayString() == "System.IFormatProvider?"));
        }
            ...
            string format)
        {
            ...
            if (!synth.ContainsKey(name))
            {
                ...
                var code = $"    private static string {name}({Fq(src)} v) => " + $"v.ToString(\"{escaped}\", global::System.Globalization.CultureInfo.InvariantCulture);\n";
                synth[name] = new SynthesizedMethod(name, code);
            }
