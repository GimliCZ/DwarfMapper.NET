...
namespace DwarfMapper.Generator.Pipeline
{
    ...
    {
        /// <summary>
        ///     Dotted member path from a selector lambda `p => p.A.B`, or null when it is not a pure
        ///     member-access chain rooted at the lambda's single parameter.
        /// </summary>
        private static string? TryReadMemberPath(ExpressionSyntax arg)
        {
            ...
            switch (arg)
            {
                case SimpleLambdaExpressionSyntax { Body: ExpressionSyntax b } sl:
                    param = sl.Parameter.Identifier.ValueText;
                    body = b;
                    break;
        ...
        /// <summary>
        ///     Simple method name from a method-group argument (`Convert` or `Type.Convert`/`this.Convert`),
        ///     or null when the argument is not a method group (e.g. an inline lambda).
        /// </summary>
        private static string? TryReadMethodGroup(ExpressionSyntax arg)
        {
            return arg switch
            {
            ...
            List<DiagnosticInfo> diagnostics)
        {
            ...
            foreach (var method in classSymbol.GetMembers().OfType<IMethodSymbol>())
            {
                ...
                if (method.Parameters[0].Type is not INamedTypeSymbol pt)
                {
                    continue;
                }
                ...
                foreach (var inv in decl.DescendantNodes().OfType<InvocationExpressionSyntax>())
                {
                    ...
                    if (model.GetTypeInfo(ma.Expression).Type?.OriginalDefinition is not INamedTypeSymbol rt || !SymbolEqualityComparer.Default.Equals(rt, mapConfigDef))
                    {
                        continue;
                    }
                    ...
                    if (op == "Map" && args.Count >= 2)
                    {
                        ...
                        {
                            diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.MapConfigUnsupportedExpression,
                                loc,
                    ...
                    else if (op == "Ignore" && args.Count == 1)
                    {
                        ...
                        result.Ignores.Add(new PairIgnore
                        {
                            Target = tgt,
                            Member = tgtPath,
                            Loc = loc
                    ...
                    else if (op == "Construct" && args.Count == 1)
                    {
                        ...
                        result.Constructors.Add(new PairConstructor
                        {
                            Source = src,
                            Target = tgt,
                            Method = factory,
                    ...
                    else if (op == "Value" && args.Count == 2)
                    {
                        ...
                        {
                            result.Values.Add(new PairValue
                            {
                                Target = tgt,
                                Member = tgtPath,
                                Use = compute,
        ...
        /// <summary>
        ///     Flags a destination member configured more than once — by both an attribute and a MapConfig, or
        ///     twice within a MapConfig (DWARF069). <paramref name="attrProps" />/<paramref name="attrValues" /> are the
        ///     attribute-origin lists BEFORE the config entries are merged in.
        /// </summary>
        ...
        private static void ReportMapConfigConflicts(
            List<PairProp> attrProps,
            List<PairValue> attrValues,
            MapConfigResult config,
            List<DiagnosticInfo> diagnostics)
        {
            static string PropKey(PairProp p)
            {
                return p.Source.ToDisplayString() + "|" + p.Target.ToDisplayString() + "|" + p.TgtMember;
            }

            static string ValKey(PairValue v)
            {
                return v.Target.ToDisplayString() + "|" + v.Member;
            }
            ...
            foreach (var p in config.Props)
                ...
                {
                    diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.MapConfigConflict,
                        p.Loc,
