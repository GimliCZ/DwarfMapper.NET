...
internal static class MapEmitter
{
    ...
    private static void EmitMappingPlanDoc(StringBuilder sb, MapMethodModel method, string indent)
    {
        var summary = method.ReturnTypeFullName == "void"
            ? "Generated by DwarfMapper — maps from <c>" + XmlEscape(method.ParameterTypeFullName) + "</c> onto an existing target."
            : "Generated by DwarfMapper — maps <c>" + XmlEscape(method.ParameterTypeFullName) + "</c> to <c>" + XmlEscape(method.ReturnTypeFullName) + "</c>.";
        ...
        foreach (var line in lines)
            sb.Append(indent).Append("/// <item><description>").Append(XmlEscape(line)).AppendLine("</description></item>");
        sb.Append(indent).AppendLine("/// </list></remarks>");
    }

    private static void AddPlanLine(System.Collections.Generic.List<string> lines, MemberMap m, string? tag)
    {
