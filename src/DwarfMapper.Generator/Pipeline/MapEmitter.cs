...
internal static class MapEmitter
{
    ...
    private static void EmitMappingPlanDoc(StringBuilder sb, MapMethodModel method, string indent)
    {
        var summary = method.ReturnTypeFullName == "void"
            ? "Generated by DwarfMapper — maps from <c>" + XmlEscape(method.ParameterTypeFullName) +
              "</c> onto an existing target."
            : "Generated by DwarfMapper — maps <c>" + XmlEscape(method.ParameterTypeFullName) + "</c> to <c>" +
              XmlEscape(method.ReturnTypeFullName) + "</c>.";
        ...
        if (method.IsProjection || method.IsSpanMap || method.IsAsyncStreamMap) return;
    ...
    private static void EmitMethod(StringBuilder sb, MapMethodModel method, string indent)
    {
        ...
        if (hasUnflatten || hasWhen || hasSkipNull)
            EmitDeferredAssignments(sb, method, "__dwarf_target", method.ParameterName, ctxVarName, depthPassFwd,
                indent + "    ");
