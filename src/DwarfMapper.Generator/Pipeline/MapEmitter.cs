...
namespace DwarfMapper.Generator.Pipeline
{
    internal static partial class MapEmitter
    {
        ...
        // Emits an XML-doc mapping plan on a public method: a one-line summary (IDE hover) plus, for standard
        // member-assignment methods, a per-member <remarks> list (target ← source, via converter, = constant,
        // ?? substitute, when predicate). Deterministic (members are already in canonical order), so it does not
        // affect the run-twice determinism guarantee. Surfaces the resolved mapping for review without opening
        // the body — the readable face of the anti-mislinking story.
        private static void EmitMappingPlanDoc(StringBuilder sb, MapMethodModel method, string indent)
        {
            // A void UPDATE-INTO writes into the instance its caller passed. The model records the destination type in
            // ReturnTypeFullName and the void return in UpdateReturnsVoid, so asking the type name for "void" (the
            // test this summary was written with, before update-into existed) never matched.
            var summary = method.UpdateReturnsVoid
                ? "Generated by DwarfMapper — maps from <c>" +
                  XmlEscape(method.ParameterTypeFullName) +
                  "</c> onto an existing target."
                : "Generated by DwarfMapper — maps <c>" +
                  XmlEscape(method.ParameterTypeFullName) +
                  "</c> to <c>" +
                  XmlEscape(method.ReturnTypeFullName) +
                  "</c>.";
            sb.Append(indent).Append("/// <summary>").Append(summary).AppendLine("</summary>");

            // Projection/span/async use Members for element conversion, not destination members — skip the list.
            if (method.IsProjection || method.IsSpanMap || method.IsAsyncStreamMap)
            {
                return;
            }

            var lines = new List<string>();
            foreach (var arg in method.ConstructorArguments) AddPlanLine(lines, arg, "ctor");
            foreach (var m in method.Members) AddPlanLine(lines, m, null);
            if (lines.Count == 0)
            {
                return;
            }

            sb.Append(indent).AppendLine("/// <remarks>Mapping plan:<list type=\"bullet\">");
            foreach (var line in lines)
                sb.Append(indent).Append("/// <item><description>").Append(XmlEscape(line))
                    .AppendLine("</description></item>");
            sb.Append(indent).AppendLine("/// </list></remarks>");
        }
        ...
        private static void AddPlanLine(List<string> lines, MemberMap m, string? tag)
        {
            ...
            if (m.NullSubstituteLiteral is not null)
            {
                line += " (?? " + m.NullSubstituteLiteral + ")";
            }

            if (m.WhenPredicate is not null)
            {
                line += " (when " + m.WhenPredicate + ")";
            }
        ...
        private static void EmitMethod(StringBuilder sb, MapMethodModel method, string indent)
        {
            ...
            if (method.IsPartial)
            {
                EmitMappingPlanDoc(sb, method, indent);
            }
            ...
            var willUseSetNullPath = method.IsSetNullMode && method.ParameterIsReferenceType && (isSynthesizedRecursive || isPublicWithCtx);
            ...
            if (isSynthesizedRecursive && !method.IsPreserveMode && !willUseSetNullPath)
            {
                ...
                sb.Append(indent)
                    .Append("        throw new global::DwarfMapper.DwarfMappingDepthException(ctx.MaxDepth, depth);");
                sb.AppendLine();
            }
            ...
            {
                if (m.UnflattenIntermediateFqn is not null)
                {
                    hasUnflatten = true;
                }
            ...
            if (method.IsPreserveMode && method.ParameterIsReferenceType && (isSynthesizedRecursive || isPublicWithCtx))
            {
            ...
            if (hasCtorArgs)
            {
                ...
                if (hasInitMembers)
                {
                    ...
                    foreach (var member in method.Members)
                    {
                        if (member.UnflattenIntermediateFqn is not null ||
                            member.WhenPredicate is not null ||
        ...
        private static void EmitUpdateIntoMethod(StringBuilder sb, MapMethodModel method, string indent)
        {
            ...
            if (method.ParameterIsReferenceType)
            {
                sb.Append(indent).Append("    global::System.ArgumentNullException.ThrowIfNull(").Append(src)
                    .AppendLine(");");
            }
        ...
        /// </summary>
        private static void EmitCollectionKeyUpsert(
            StringBuilder sb,
            MemberMap member,
            string src,
            string dst,
            ...
            bool isPublicMethod)
        {
            ...
            if (!isPublicMethod)
            {
                ...
                sb.Append(indent)
                    .AppendLine(
                        "            throw new global::DwarfMapper.DwarfMappingDepthException(ctx.MaxDepth, depth);");
            }
            ...
            string indent)
        {
            ...
            foreach (var member in method.Members)
            {
                if (member.UnflattenIntermediateFqn is null)
                {
                    continue;
                }
            ...
            foreach (var member in method.Members)
            {
                if (member.WhenPredicate is null || member.UnflattenIntermediateFqn is not null)
                {
                    continue;
                }
            ...
            foreach (var member in method.Members)
            {
                ...
                if (!member.SkipIfSourceNull ||
                    member.WhenPredicate is not null ||
                    member.UnflattenIntermediateFqn is not null)
                {
                    continue;
                }
        ...
        /// </summary>
        /// <param name="sb">The buffer the expression is appended to.</param>
        /// <param name="member">
        ///     The resolved mapping to emit. It decides which of the forms below is written: a verbatim
        ///     <c>[MapValue]</c> expression, a <c>NullSubstitute</c> coalesce, a null-preserving ternary, a converter
        ///     call, or a plain source read.
        /// </param>
        /// <param name="paramName">
        ///     Name of the source parameter in scope; every source read is emitted as
        ///     <c>paramName.SourceMember</c>.
        /// </param>
            ...
            string depthArg = "0")
        {
            ...
            else
            {
                ...
                if (member.NullRefIntoNonNullable)
                {
                    sb.Append('!');
                }
            }
