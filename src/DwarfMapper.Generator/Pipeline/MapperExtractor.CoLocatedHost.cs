...
namespace DwarfMapper.Generator.Pipeline
{
    internal static partial class MapperExtractor
    {
        ...
        ///     Diagnostic sink; a directive the placement rule cannot bind is refused as <c>DWARF089</c> here rather
            ...
            List<DiagnosticInfo> diagnostics)
        {
            ...
            foreach (var member in host.GetMembers())
            {
                ...
                if (!DirectivesArePlaceable(host, member, directives, hostTargeted.Count, diagnostics))
                {
                    continue;
                }
        ...
        /// </summary>
        /// <param name="host">The co-located host class, named in every message raised here.</param>
        /// <param name="member">The annotated host member — the DESTINATION the directives on it configure.</param>
        /// <param name="directives">
        ///     Every member-placement directive on that member, in source order. Stacked ones bind POSITIONALLY, one
        ///     per host-targeted pair, which is what the count check at the end tests.
        /// </param>
        /// <param name="hostTargetedPairs">How many declared pairs have the host as their destination.</param>
        /// <param name="diagnostics">Diagnostic sink; one <c>DWARF089</c> per directive that cannot be placed.</param>
        private static bool DirectivesArePlaceable(
            INamedTypeSymbol host,
            ISymbol member,
            List<MemberDirective> directives,
            int hostTargetedPairs,
            List<DiagnosticInfo> diagnostics)
        {
            var placeable = true;
            ...
            foreach (var d in directives)
                ...
                if (!d.Ignore && d.ArgumentCount != 1)
                {
                    diagnostics.Add(new DiagnosticInfo(
                        DiagnosticDescriptors.MisplacedDirectiveOnCoLocatedHostMember,
                        d.Loc,
                ...
                else if (!d.Ignore && string.IsNullOrEmpty(d.Name))
                {
                    diagnostics.Add(new DiagnosticInfo(
                        DiagnosticDescriptors.MisplacedDirectiveOnCoLocatedHostMember,
                        d.Loc,
                        $"[MapProperty] on '{member.Name}' of the co-located host '{host.Name}' names no source " + "member — the argument is null, empty, or not a constant string. The member form's one " + $"argument is the SOURCE member '{member.Name}' is filled from, so there is nothing " + "here to bind to: [MapProperty(\"<source>\")]."));
                    placeable = false;
                }
            ...
            if (directives.Count > 1 && directives.Count != hostTargetedPairs)
            {
                diagnostics.Add(new DiagnosticInfo(
                    ...
                    (hostTargetedPairs == 1 ? "" : "s") +
                    " declared on it. Stacked directives bind " +
