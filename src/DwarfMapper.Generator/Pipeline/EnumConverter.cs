...
namespace DwarfMapper.Generator.Pipeline
{
    ...
    internal static class EnumConverter
    {
        ...
        private static string AddEnumByValue(
            ...
            INamedTypeSymbol tgt)
        {
            ...
            if (!synth.ContainsKey(name))
            {
                ...
                w.Line("private static " + Fq(tgt) + " " + name + "(" + Fq(src) + " v) => (" + Fq(tgt) + ")" + tgtUnderlying + ".CreateChecked((" + srcUnderlying + ")v);");
        ...
        /// </summary>
        private static string AddEnumToNum(
            Dictionary<string, SynthesizedMethod> synth,
            ...
            ITypeSymbol tgtNum)
        {
            ...
            if (!synth.ContainsKey(name))
            {
                ...
                var w = new CodeWriter(1);
                w.Line("private static " + Fq(tgtNum) + " " + name + "(" + Fq(srcEnum) + " v) => " + Fq(tgtNum) + ".CreateChecked((" + srcUnderlying + ")v);");
                synth[name] = new SynthesizedMethod(name, w.ToString());
            }
        ...
        /// </summary>
        private static string AddNumToEnum(
            Dictionary<string, SynthesizedMethod> synth,
            ITypeSymbol srcNum,
            INamedTypeSymbol tgtEnum)
        {
            ...
            if (!synth.ContainsKey(name))
            {
                ...
                var w = new CodeWriter(1);
                w.Line("private static " + Fq(tgtEnum) + " " + name + "(" + Fq(srcNum) + " v) => (" + Fq(tgtEnum) + ")" + tgtUnderlying + ".CreateChecked(v);");
                synth[name] = new SynthesizedMethod(name, w.ToString());
            }
        ...
        private static string SerializedName(IFieldSymbol member, EnumStringSource source)
        {
            ...
            if (source == EnumStringSource.Identifier)
            {
                return member.Name;
            }
            ...
            List<DiagnosticInfo> diagnostics)
        {
            ...
            diagnostics.Add(new DiagnosticInfo(
                DiagnosticDescriptors.EnumStringNameDiverges,
                location,
        ...
        private static string AddEnumToString(
            Dictionary<string, SynthesizedMethod> synth,
            INamedTypeSymbol src,
            EnumStringSource stringSource)
        {
            ...
            var name = GeneratedNames.EnumToStr + Sanitize(src) + "_" + StableHash.Fnv1a("EnumStr|" + stringSource + "|" + Fq(src));
            if (!synth.ContainsKey(name))
            {
        ...
        private static string AddStringToEnum(
            Dictionary<string, SynthesizedMethod> synth,
            INamedTypeSymbol tgt,
            EnumStringSource stringSource)
        {
            ...
            var name = GeneratedNames.StrToEnum + Sanitize(tgt) + "_" + StableHash.Fnv1a("StrEnum|" + stringSource + "|" + Fq(tgt));
            if (!synth.ContainsKey(name))
            {
                synth[name] = new SynthesizedMethod(name,
