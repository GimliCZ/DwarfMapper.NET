...
namespace DwarfMapper.Generator.Pipeline
{
    ...
    internal static class ConstructorSelector
    {
        ...
        ///     The compilation every accessibility test runs against — a constructor is a candidate only when the
        ///     generated code could actually call it.
        /// </param>
        ...
        /// </param>
        /// <param name="allowNonPublicConstructors">
        ///     <c>[DwarfMapper(AllowNonPublic = true)]</c>: widens every accessibility test here, so a non-public
        ///     constructor becomes selectable.
        /// </param>
        /// <param name="sourceType">
        ///     The mapping's source type, or <see langword="null" /> from callers that have none to give. When
        ///     present, Policy 5 first narrows the field to constructors every parameter of which has a source, so a
        ///     wide-but-unbindable constructor no longer outranks a fully mappable one.
        /// </param>
        /// <param name="explicitMaps">
        ///     The <c>[MapProperty]</c> renames, so that same narrowing counts a parameter fed only by a rename as
        ///     having a source.
        /// </param>
        /// <returns>
            ...
            IReadOnlyList<(string Source, string Target, string? Use)>? explicitMaps = null)
        {
            ...
            ReportUnusableAnnotatedConstructors(target,
                compilation,
            ...
            var hasExplicitNonParameterlessCtor = target.InstanceConstructors.Any(c =>
                !c.IsImplicitlyDeclared && IsAccessible(c, compilation, allowNonPublicConstructors) && !c.IsStatic && c.Parameters.Length > 0 && !IsObsolete(c));
            ...
            var anyParameterless = target.InstanceConstructors.FirstOrDefault(c =>
                IsAccessible(c, compilation, allowNonPublicConstructors) && !c.IsStatic && c.Parameters.Length == 0 && !IsObsolete(c) && (!c.IsImplicitlyDeclared || !skipImplicitParameterlessCtor));
            ...
            if (anyParameterless is not null)
            {
                ...
                var annotatedOverrides = target.InstanceConstructors
                    .Where(c => IsUsableCandidate(c, target, compilation, allowNonPublicConstructors) && IsAnnotated(c))
                    .ToList();
            ...
            if (sourceType is not null)
            {
                var satisfiable = candidates
                    .Where(c => AllParametersHaveASource(c,
                        sourceType,
                        explicitMaps,
                        compilation,
                        allowNonPublicConstructors))
            ...
            bool allowNonPublic)
        {
            ...
            foreach (var param in ctor.Parameters)
            {
                ...
                if (mapped is { Target: not null })
                {
                    ...
                    if (src.IndexOf('.') >= 0)
                    {
                        if (!MemberFacts.TryResolvePath(sourceType,
                                src,
                                compilation,
                                allowNonPublic,
                                out _,
        ...
        {
            return ctor.DeclaredAccessibility == Accessibility.Public || (allowNonPublic && compilation.IsSymbolAccessibleWithin(ctor, compilation.Assembly));
        }
            ...
            bool allowNonPublic)
        {
            ...
            if (ctor.Parameters.Any(p => p.RefKind == RefKind.Ref || p.RefKind == RefKind.Out))
            {
                return false;
            }
            ...
            List<DiagnosticInfo> diagnostics)
        {
            foreach (var ctor in target.InstanceConstructors)
            {
                ...
                if (IsUsableCandidate(ctor, target, compilation, allowNonPublic))
                {
                    continue;
                }
                ...
                diagnostics.Add(new DiagnosticInfo(
                    DiagnosticDescriptors.AnnotatedConstructorUnusable,
                    location,
                    $"[DwarfMapperConstructor] on '{target.Name}({Signature(ctor)})' is ignored: {reason} " + "The destination is constructed exactly as it would be with no annotation at all — the " + "fallback is deliberate (selecting this constructor would emit code the compiler rejects), " + "so the mapping is safe; the directive is not."));
            }
        }
            ...
            bool allowNonPublic)
        {
            ...
            if (ctor.Parameters.Length == 1 && SymbolEqualityComparer.Default.Equals(ctor.Parameters[0].Type, target))
            {
                return "it is a COPY constructor (its single parameter is the destination type itself), so it " + "cannot build the destination from the source. Annotate a constructor whose parameters " + "come from the source type.";
            }

            if (IsObsolete(ctor))
            {
                return "it is marked [Obsolete], and the mapper does not generate calls to obsolete members. " + "Drop the [Obsolete], or annotate a supported constructor.";
            }
            ...
            {
                return $"parameter '{byRef.Name}' is passed by " + $"{(byRef.RefKind == RefKind.Ref ? "ref" : "out")}, which cannot be written as a named " + "argument (CS1620). Take it by value or by 'in', or annotate a different constructor.";
            }
            ...
            return "the selector does not accept it (no specific reason could be determined — this is a gap " + "between the candidate filter and this message, please report it).";
        }
