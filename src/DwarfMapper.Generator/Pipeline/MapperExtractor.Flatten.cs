    ...
    internal static partial class MapperExtractor
    {
                ...
                IReadOnlyList<(INamedTypeSymbol Src, INamedTypeSymbol Tgt, bool WrittenGeneric)>? rawDerivedPairs = null)
        {
            ...
            foreach (var (srcNavName, tgtCollName) in rawDirectives)
            {
                ...
                {
                    diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.DuplicateFlattenGraphTarget,
                        location,
                        tgtCollName));
                    continue;
                }
                ...
                {
                    diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.InvalidFlattenGraph,
                        location,
                ...
                if (isHetero)
                {
                    ...
                    foreach (var (derivedSrc, derivedTgt, _) in effectiveDerivedPairs)
                    {
                        ...
                        {
                            diagnostics.Add(new DiagnosticInfo(DiagnosticDescriptors.InvalidMapDerivedType,
                                location,
                        ...
                        if (!synthesized.ContainsKey(perTypeHelperName))
                        {
                            ...
                            foreach (var leaf in derivedLeafMembers)
                            {
                                ...
                                var leafResolved = TryResolveConversion(compilation,
                                    leaf.Type,
                                    dtoMemberType,
                                    null,
                                    allMethods,
                ...
                foreach (var m in allMethods)
                    if (HasImplicitConversion(compilation, nodeType, m.ParamType) && HasImplicitConversion(compilation, m.ReturnType, nodeDtoType))
                    {
