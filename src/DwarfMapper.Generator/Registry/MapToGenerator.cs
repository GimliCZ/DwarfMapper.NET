    ...
    public sealed class MapToGenerator : IIncrementalGenerator
    {
        ...
        private static Model Extract(GeneratorAttributeSyntaxContext ctx)
        {
            ...
            foreach (var (srcSym, _, srcType) in MemberFacts.Readable(source, RegistryCompilation, RegistryAllowNonPublic))
            {
                ...
                {
                    diags.Add(new DiagnosticInfo(RegistryDiagnostics.MapPropertyArity,
                        location,
                        $"[MapProperty] on '{srcSym.Name}' uses the [DwarfMapper] class model's two-argument " + "method form; the registry's member form takes only the single destination name this " + "member supplies — drop the first argument."));
                    hasError = true;
                ...
                else if (directives.Count > 1 && targetCount > 0 && directives.Count != targetCount)
                {
                    diags.Add(new DiagnosticInfo(RegistryDiagnostics.MapPropertyArity,
                        location,
                        $"[MapProperty] on '{srcSym.Name}' must have either one value (all targets) or exactly " + "one value per [MapTo] target, in order."));
                    hasError = true;
                    directives = new List<MemberDirective>();
                }
                ...
                foreach (var d in directives)
                    if (d.Ignore && d.ArgumentCount != 0)
                    {
                        ...
                        diags.Add(new DiagnosticInfo(RegistryDiagnostics.MapIgnoreArgumentNotRead,
                            d.Loc ?? location,
            ...
            for (var ti = 0; ti < targetCount; ti++)
            {
                ...
                {
                    diags.Add(new DiagnosticInfo(RegistryDiagnostics.NoParameterlessConstructor,
                        location,
                        $"[MapTo] target '{target.ToDisplayString()}' has no public parameterless constructor; the " + "registry constructs the types it builds with an object initializer — add one, or map " + "the pair with the [DwarfMapper] class model, which supports constructor mapping"));
                    hasError = true;
                    continue;
                }
                ...
                foreach (var m in members)
                {
                    ...
                    {
                        diags.Add(new DiagnosticInfo(RegistryDiagnostics.AutoMatchDisabled,
                            location,
                    ...
                    {
                        diags.Add(new DiagnosticInfo(RegistryDiagnostics.ConflictingSource,
                            location,
                            $"'{destName}' on '{target.Name}'"));
                        hasError = true;
                    }
                ...
                foreach (var w in writables)
                {
                    if (!chosen.TryGetValue(w.Name, out var src))
                    {
                        ...
                        // same member.
                        if (autoMatchRefused.Contains(w.Name))
                        {
                            continue;
                        }

                        diags.Add(new DiagnosticInfo(RegistryDiagnostics.UnmappedDestination,
                            location,
                    ...
                    if (expr is null)
                    {
                        ...
                        {
                            diags.Add(new DiagnosticInfo(RegistryDiagnostics.NoConversion,
                                location,
                ...
                plans.Add(new TargetPlan(targetFqn,
                    "To" + target.Name,
            ...
            {
                diags.Add(new DiagnosticInfo(RegistryDiagnostics.DuplicateTargetMethodName,
                    location,
                    $"[MapTo] targets {string.Join(", ", dup.Select(p => p.TargetFqn))} share a simple name, so each " + $"would generate '{dup.Key}(this …)' — rename one target, or use the [DwarfMapper] class model " + "where every method is named explicitly"));
                hasError = true;
            }
            ...
            var isPublic = AssemblyConfiguration.PublicExtensions(compilation) && IsAccessiblePublic(source) && targets.All(IsAccessiblePublic);
            return new Model(
                source.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat),
        ...
        {
            return !SymbolEqualityComparer.Default.Equals(target, source) && (target.TypeKind == TypeKind.Class || target.TypeKind == TypeKind.Struct) && !target.IsAbstract;
        }
        ...
        private static void ReportUnreadConstructorDirective(
            ...
            List<DiagnosticInfo> diags,
            LocationInfo? location)
        {
            if (ConstructorSelector.HasAnnotatedConstructor(target))
            {
                diags.Add(new DiagnosticInfo(RegistryDiagnostics.ConstructorDirectiveNotRead,
                    location,
        ...
        {
            return directives.Exists(d => !d.Ignore && d.ArgumentCount != 1);
        }
        ...
        private static string Emit(Model model)
        {
            ...
            {
                var classHeader = (model.Public ? "public static class " : "internal static class ") + model.ExtClassName;
                using (w.Block(classHeader))
                {
                    using (w.Block("public static TTarget MapTo<TTarget>(this " + model.SourceFqn + " source)"))
                    {
                        ...
                        foreach (var t in model.Targets)
                            w.Line("if (typeof(TTarget) == typeof(" + t.TargetFqn + ")) return (TTarget)(object)" + t.MethodName + "(source);");
                        w.Line("throw new global::System.NotSupportedException(\"No [MapTo] mapping from " + model.SourceFqn + " to \" + typeof(TTarget).FullName + \".\");");
                    }

                    foreach (var t in model.Targets)
                    {
                        w.Line();
                        using (w.Block("public static " + t.TargetFqn + " " + t.MethodName + "(this " + model.SourceFqn + " source)"))
                        {
        ...
        private sealed class Resolver
        {
            ...
            ///     <para>
            ///         Without it the caller adds <c>DWARFR05</c> —
            ///         <i>
            ///             "the source and destination member types are
            ///             incompatible"
            ///         </i>
            ///         — on top, which is false: the types are perfectly compatible and the
            ...
            public string? Resolve(ITypeSymbol srcType, ITypeSymbol tgtType, string srcExpr, string targetName)
            {
                ...
                if (conv.IsImplicit)
                {
                    ...
                    {
                        _diags.Add(new DiagnosticInfo(RegistryDiagnostics.LossyImplicitConversion,
                            _loc,
                ...
                if (srcType is INamedTypeSymbol ns &&
                    tgtType is INamedTypeSymbol nt &&
                    IsObjectType(ns) &&
                    IsObjectType(nt))
                {
            ...
            private string? SynthNested(INamedTypeSymbol src, INamedTypeSymbol tgt, string targetName)
            {
                ...
                {
                    _diags.Add(new DiagnosticInfo(RegistryDiagnostics.RecursiveNesting,
                        _loc,
                        $"'{src.Name}' → '{tgt.Name}'"));
                ...
                {
                    _diags.Add(new DiagnosticInfo(RegistryDiagnostics.NoParameterlessConstructor,
                        _loc,
                ...
                foreach (var w in MemberFacts.Writable(tgt, RegistryCompilation, RegistryAllowNonPublic))
                {
                    ...
                    {
                        _diags.Add(new DiagnosticInfo(RegistryDiagnostics.UnmappedDestination,
                            _loc,
                            $"'{w.Name}' on '{tgt.Name}'"));
                        ok = false;
                        continue;
                    ...
                    if (expr is null)
                    {
                        ...
                        if (!RefusalReported)
                        {
                            _diags.Add(new DiagnosticInfo(RegistryDiagnostics.NoConversion,
                                _loc,
            ...
            private string? TryCollection(ITypeSymbol srcType, ITypeSymbol tgtType, string srcExpr, string targetName)
            {
                ...
                if (!CollectionConverter.TryGetEnumerableElement(srcType, out var sElem, out var sCount))
                {
                    return null;
                }
