...
namespace DwarfMapper.Generator.Diagnostics
{
    public static class DiagnosticDescriptors
    {
        ...
        public static readonly DiagnosticDescriptor OnCycleIgnoredUnderPreserve = new(
            ...
            "Mapper '{0}' sets OnCycle = SetNull together with ReferenceHandling = Preserve; " + "OnCycle only applies in None mode (Preserve reconstructs cycles faithfully), so the setting has no effect",
            Category,
        ...
        public static readonly DiagnosticDescriptor MapConfigUnsupportedExpression = new(
            "DWARF068",
            "Unsupported MapConfig expression",
        ...
        public static readonly DiagnosticDescriptor MapConfigConflict = new(
            "DWARF069",
            "Conflicting member configuration",
        ...
        public static readonly DiagnosticDescriptor NoCodeGenerated = new(
            ...
            "No code was generated for mapper '{0}' because it has unresolved DwarfMapper errors ({1}). Every " + "partial mapping method on it will now ALSO report CS8795 (\"must have an implementing part\") — " + "those are a cascade of this, not a separate problem, and they are not caused by a missing analyzer " + "reference. Fix the errors listed above.",
            Category,
            DiagnosticSeverity.Warning,
            true,
        ...
        public static readonly DiagnosticDescriptor IgnoredRequiredMember = new(
            ...
            "Destination member '{0}' is `required`, so ignoring it would emit an object initializer that omits " + "it — which does not compile (CS9035). Supply a placeholder with [MapValue(\"{0}\", …)], map it, or " + "drop `required` from the member.",
            Category,
            DiagnosticSeverity.Error,
            true,
        ...
        public static readonly DiagnosticDescriptor ProvidesMapInvalidShape = new(
            ...
            "Method '{0}' is marked [ProvidesMap] but does not have a registerable shape. It must be public, " + "take exactly one parameter, and return a value, with both types publicly nameable — the same shape " + "the generator's own maps register under, because the registry holds them the same way.",
            Category,
            DiagnosticSeverity.Error,
            true,
        ...
        public static readonly DiagnosticDescriptor DuplicateFlattenGraphTarget = new(
            ...
            "Destination collection '{0}' is filled by more than one [FlattenGraph] directive on this method, " + "which would emit an object initializer that assigns '{0}' twice (CS1912) — code that does not " + "compile. Keep exactly one [FlattenGraph] per destination collection; to flatten a second graph, " + "name a different collection member on the destination type.",
            Category,
            DiagnosticSeverity.Error,
            true,
