...
namespace DwarfMapper.Generator
{
    ...
    public sealed class DwarfGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(
                ...
                static (spc, data) =>
                {
                    ...
                    foreach (var (source, destination) in AmbientValidator.MissingRequires(own,
                                 req,
                                 root.Provided,
                                 root.Required))
                        spc.ReportDiagnostic(Diagnostic.Create(
                            DiagnosticDescriptors.RequiredMapNotProvided,
                            Location.None,
                            source,
                            destination));
                    ...
                    if (validate.Length != 0)
                    {
                        ...
                        if (root.DiAvailable)
                        {
                            ...
                            if (di.Length != 0)
                            {
                                spc.AddNormalizedSource("DwarfMapper.ValidateDi.g.cs", di);
                            }
                        }
                    }
                });
        ...
        private static void EmitRequiresManifest(
            ...
            List<MapperClassModel> models)
        {
            ...
            {
                if (method.Withheld || method.IsPartial || !GeneratedNames.IsObjectMap(method.MethodName))
                {
                    continue;
                }
                ...
                {
                    if (!existing.Owners.Contains(model.ClassName))
                    {
                        existing.Owners.Add(model.ClassName);
                    }
                }
                else
                {
            ...
            foreach (var pair in byName.OrderBy(static p => p.Key, StringComparer.Ordinal))
            {
                ...
                spc.ReportDiagnostic(Diagnostic.Create(
                    DiagnosticDescriptors.DivergentSynthesizedPair,
                    Location.None,
        ...
        private static void EmitAggregates(
            SourceProductionContext spc,
            ImmutableArray<MapperClassModel> models,
            bool diAvailable,
            ...
            string assemblyNamespace)
        {
            ...
            {
                spc.AddNormalizedSource("DwarfMapper.Extensions.g.cs", facade);
            }
            ...
            foreach (var (sourceType, extName) in facadeCollisions)
                ...
                spc.ReportDiagnostic(Diagnostic.Create(
                    DiagnosticDescriptors.DuplicateFacadeExtension,
                    Location.None,
                    $"more than one mapper maps from '{sourceType}', so the '{extName}(this {sourceType})' convenience " + "extension was not generated (it would be ambiguous) — call the mapper instance method, or " + "disable one mapper's extensions with [DwarfMapper(GenerateExtensions = false)]"));
            ...
            if (diAvailable)
            {
                ...
                if (di is not null)
                {
                    spc.AddNormalizedSource("DwarfMapper.ServiceCollectionExtensions.g.cs", di);
                }
            }
        ...
        private static void Execute(SourceProductionContext spc, MapperClassModel model)
        {
            ...
            {
                if (suppressScopedSignpost && (ReferenceEquals(diagnostic.Descriptor, DiagnosticDescriptors.ProjectionMethodNotGenerated) || ReferenceEquals(diagnostic.Descriptor, DiagnosticDescriptors.MappingMethodNotGenerated)))
                {
                    continue;
                }
