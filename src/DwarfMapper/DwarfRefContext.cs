...
public sealed class DwarfRefContext
{
    ...
    private readonly Dictionary<object, object>? _identity;
    ...
    public DwarfRefContext(int maxDepth, bool preserve = false, bool setNull = false)
    {
        ...
        if (preserve)
            _identity = new Dictionary<object, object>(
    ...
    /// </summary>
    /// <param name="src">
    ///     The source object about to be mapped. Must not be <c>null</c>
    ///     (the caller's null-guard runs first).
    /// </param>
    /// <returns>
    ///     <c>true</c> if freshly entered (proceed + pair with <see cref="ExitNode" />);
    ///     <c>false</c> if already on the stack (break the cycle with <c>null</c>).
    /// </returns>
