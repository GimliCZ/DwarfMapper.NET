...
public sealed class DwarfMapMissingException : InvalidOperationException
{
    ...
    /// <summary>Creates the exception for an unregistered <paramref name="sourceType"/> -&gt;
    /// <paramref name="destinationType"/> pair.</summary>
    ...
    private static string FormatMessage(Type? sourceType, Type? destinationType) =>
        $"No DwarfMapper map is registered for '{sourceType}' -> '{destinationType}'. " +
