...
public static class DwarfMapperRegistry
{
    private readonly struct Key : IEquatable<Key>
    {
        ...
        public Key(Type source, Type destination) { Source = source; Destination = destination; }
        public bool Equals(Key other) => Source == other.Source && Destination == other.Destination;
        public override bool Equals(object? obj) => obj is Key k && Equals(k);
        public override int GetHashCode() => unchecked((Source.GetHashCode() * 397) ^ Destination.GetHashCode());
    }
    ...
    public static bool IsProvided(Type source, Type destination)
        => Maps.ContainsKey(new Key(source, destination));
    ...
    public static bool IsAmbiguous(Type source, Type destination)
        => Ambiguous.ContainsKey(new Key(source, destination));
    ...
    public static bool TryGet(Type source, Type destination, out Func<object, object>? map)
        => Maps.TryGetValue(new Key(source, destination), out map);
