...
public sealed class RestateBaseConfigurationCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (attribute is null || classDecl is null) continue;

            if (TryReadPair(diagnostic, out var baseSource, out var baseTarget)
                && TryReadDerivedPair(attribute, out var derivedSource, out var derivedTarget))
            {
                ...
                context.RegisterCodeFix(
                    CodeAction.Create(
                        ...
                        _ => Task.FromResult(WithRestatement(
                            context.Document, root, capturedClass,
                            baseSource, baseTarget, derivedSource, derivedTarget)),
                        "DWARF085_Restate"),
                    diagnostic);
            }
    ...
    private static Document WithRestatement(
        ...
        string baseSource, string baseTarget, string derivedSource, string derivedTarget)
    {
        ...
        {
            if (!IsPairScoped(attribute, out var typeArgs)) continue;

            if (MatchesPair(typeArgs, derivedSource, derivedTarget))
            {
                var target = TargetMemberOf(attribute);
                if (target is not null) existingByTarget[target] = attribute;
            }
        ...
        if (replacements.Count == 0 && additions.Count == 0) return document;
    ...
    private static Document WithOverride(
        Document document, SyntaxNode root, AttributeSyntax attribute, string member)
    {
        var literal = SyntaxFactory.LiteralExpression(
            SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal(member));
    ...
    private static string Normalise(AttributeSyntax attribute) =>
        string.Concat(attribute.ToString().Where(c => !char.IsWhiteSpace(c)));
    ...
    private static bool IsPairScoped(AttributeSyntax attribute, out SeparatedSyntaxList<TypeSyntax> typeArgs)
    {
        ...
        if (!PairScoped.Contains(simple, StringComparer.Ordinal)
            && !PairScoped.Contains(simple.Replace("Attribute", string.Empty), StringComparer.Ordinal))
            return false;
    ...
    private static bool SameType(TypeSyntax syntax, string fullyQualified) =>
        string.Equals(Short(syntax.ToString()), Short(fullyQualified), StringComparison.Ordinal);
    ...
    {
        var name = attribute.Name is QualifiedNameSyntax q ? q.Right : attribute.Name;
        if (name is not GenericNameSyntax generic) return attribute;

        var replacements = generic.TypeArgumentList.Arguments.Count == 2
            ? new[] { SyntaxFactory.ParseTypeName(Short(derivedSource)), SyntaxFactory.ParseTypeName(Short(derivedTarget)) }
            : [SyntaxFactory.ParseTypeName(Short(derivedTarget))];

        var retargeted = generic.WithTypeArgumentList(
            SyntaxFactory.TypeArgumentList(SyntaxFactory.SeparatedList<TypeSyntax>(replacements)));
    ...
    private static bool TryReadDerivedPair(AttributeSyntax attribute, out string source, out string target)
    {
        ...
        var name = attribute.Name is QualifiedNameSyntax q ? q.Right : attribute.Name;
        if (name is not GenericNameSyntax { TypeArgumentList.Arguments.Count: 2 } generic) return false;
