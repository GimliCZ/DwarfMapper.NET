    ...
    public sealed class AddReverseMapInverseCodeFixProvider : CodeFixProvider
    {
        ...
        public override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                if (forward?.Parent is not ClassDeclarationSyntax || forward.ParameterList.Parameters.Count == 0 || forward.ParameterList.Parameters[0].Type is null)
                {
                    continue;
                }
        ...
        private static Document WithInverseMethod(Document document, SyntaxNode root, MethodDeclarationSyntax forward)
        {
            ...
            var newClass = classDecl.InsertNodesAfter(forward,
                new SyntaxNode[]
                {
                    inverse
                });
