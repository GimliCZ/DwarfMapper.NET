...
public sealed class AddMapIgnoreCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (member is null)
            {
                continue; // dotted/nested target, or unexpected message shape — don't offer a wrong fix
            }
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    ...
                    equivalenceKey: "DWARF001_AddMapIgnore"),
                diagnostic);
        }
    }

    private static Document WithMapIgnore(Document document, SyntaxNode root, MethodDeclarationSyntax method, string member)
    {
